...
class ErrorLogger:
    ...
    def log_error(error, context=None):
        ...
        current_app.logger.error(f"ERROR: {json.dumps(error_data, indent=2)}")
...
class MetricsCollector:
    ...
    def record_sale(amount, currency):
        """Record sale metric"""
        MetricsCollector.record_metric("sale_created", amount, {"currency": currency})

    @staticmethod
    def record_payment(amount, method):
        """Record payment metric"""
        MetricsCollector.record_metric("payment_received", amount, {"method": method})
