...
def generate_number(
    ...
    latest = q.order_by(getattr(model, field_name).desc()).first()
...
def get_next_number(
    ...
    last_record = (
        ...
        .filter(getattr(model_class, number_field).like(pattern))
        .order_by(getattr(model_class, number_field).desc())
        .first()
    )
