...
class ProductCategory(db.Model):
    ...
    parent_id = db.Column(
        db.Integer, db.ForeignKey("product_categories.id"), index=True
    )
    ...
    tenant = db.relationship(
        "Tenant", backref="product_categories", foreign_keys=[tenant_id]
    )
...
class ProductPartner(db.Model):
    ...
    tenant = db.relationship(
        "Tenant", backref="product_partners", foreign_keys=[tenant_id]
    )
...
class Product(db.Model):
    ...
    merchant_customer_id = db.Column(
        db.Integer, db.ForeignKey("customers.id"), index=True
    )
    ...
    merchant_share = db.Column(
        db.Numeric(5, 2), default=100.0
    )  # Percentage share for merchant owner
    ...
    has_serial_number = db.Column(
        db.Boolean, default=False, nullable=False
    )  # If true, each unit must have unique SN
    warranty_days = db.Column(
        db.Integer, default=0
    )  # Warranty duration in days (0 = No Warranty)
    ...
    )  # overrides tenant.business_type
    ...
    purchase_lines = db.relationship(
        "PurchaseLine", back_populates="product", lazy="dynamic"
    )
    stock_movements = db.relationship(
        "StockMovement", back_populates="product", lazy="dynamic"
    )
    partner_shares = db.relationship(
        "ProductPartner", back_populates="product", cascade="all, delete-orphan"
    )
    warehouse_stocks = db.relationship(
        "ProductWarehouseStock", back_populates="product", lazy="dynamic"
    )
    price_tiers = db.relationship(
        "ProductPriceTier", back_populates="product", lazy="dynamic"
    )
    ...
    def to_dict(self, include_cost=False):
        data = {
            ...
            "merchant_price": (
                float(self.merchant_price) if self.merchant_price else None
            ),
