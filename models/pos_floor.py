...
class PosFloor(db.Model):
    ...
    tables = db.relationship('PosTable', back_populates='floor', lazy='dynamic',
...
class PosTableOrder(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=lambda: __import__('datetime').datetime.now(__import__('datetime').timezone.utc), nullable=False)
