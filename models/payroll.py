...
class Employee(db.Model):
    ...
    employment_type = db.Column(
        db.String(20), default="salary"
    )  # salary (monthly), daily (miyawama)
...
class EmployeeLeave(db.Model):
    ...
    status = db.Column(
        db.String(20), default="approved", index=True
...
class SalaryAdvance(db.Model):
    ...
    is_deducted = db.Column(
        db.Boolean, default=False
    )  # True if fully deducted from salary
    fully_deducted_at = db.Column(db.DateTime, nullable=True)  # تاريخ الاكتمال

    # Link to GL
    gl_entry_id = db.Column(
        db.Integer, db.ForeignKey("gl_journal_entries.id"), nullable=True, index=True
    )
