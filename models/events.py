...
def register_stock_movement_listeners():
    ...
    def _h(mapper, connection, target):
        try:
            movement_type_ar = {
                'sale': 'بيع (خروج)', 'purchase': 'شراء (دخول)',
                'adjustment': 'تعديل', 'return': 'إرجاع', 'transfer': 'نقل'
            }.get(target.movement_type, target.movement_type)
            quantity_change = target.quantity or Decimal('0')
            logger.info(f"Stock Movement: {movement_type_ar} | Product #{target.product_id} | Qty: {abs(quantity_change)} | Ref: {target.reference_type}-{target.reference_id}")
        except Exception as e:
...
def register_advanced_sale_listener():
    if not _ADVANCED_SALE_LISTENER_ALLOWED:
        logger.warning("register_advanced_sale_listener() is legacy/disabled; use sale_service / payment_service for balance updates.")
        return
    from decimal import Decimal
