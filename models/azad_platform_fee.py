...
class AzadPlatformFee(db.Model):
    ...
    # Collection evidence: accounting accrues (status), the platform owner
    # confirms real-world receipt (collected_at + confirmed_by + vault txn).
    collected_at = db.Column(db.DateTime(timezone=True), nullable=True)
    confirmed_by = db.Column(db.Integer, db.ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
