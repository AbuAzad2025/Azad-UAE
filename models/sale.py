...
class Sale(db.Model):
    ...
    pos_session_id = db.Column(
        db.Integer, db.ForeignKey("pos_sessions.id", ondelete="RESTRICT"), nullable=True, index=True
    )
...
class SaleLine(db.Model):
    ...
    __table_args__ = (
        db.Index("idx_sale_lines_tenant_sale", "tenant_id", "sale_id"),
        db.Index("idx_sale_lines_tenant_product", "tenant_id", "product_id"),
    )
