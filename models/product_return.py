...
class ProductReturn(db.Model):
    ...
    reverses_invoice_id = db.Column(
        db.Integer, db.ForeignKey("sales.id", ondelete="RESTRICT"), nullable=True, index=True
    )
...
class ProductReturnLine(db.Model):
    ...
    return_id = db.Column(
        db.Integer, db.ForeignKey("product_returns.id", ondelete="RESTRICT"), nullable=False, index=True
    )
