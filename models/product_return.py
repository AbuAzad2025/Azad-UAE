...
class ProductReturn(db.Model):
    ...
    tenant = db.relationship(
        "Tenant", backref="product_returns", foreign_keys=[tenant_id]
    )
...
class ProductReturnLine(db.Model):
    ...
    return_id = db.Column(
        db.Integer, db.ForeignKey("product_returns.id"), nullable=False, index=True
    )
    ...
    tenant = db.relationship(
        "Tenant", backref="product_return_lines", foreign_keys=[tenant_id]
    )
