...
class FiscalPosition(db.Model):
    ...
    auto_apply = db.Column(
        db.Boolean, default=False
    )  # auto-assign to matching customers
    ...
    tenant = db.relationship(
        "Tenant", backref="fiscal_positions", foreign_keys=[tenant_id]
    )
    tax_rules = db.relationship(
    ...
    def map_tax(self, source_tax_id):
        ...
        rule = FiscalPositionTaxRule.query.filter_by(
            fiscal_position_id=self.id, source_tax_id=source_tax_id
        ).first()
    ...
    def map_account(self, source_account_id):
        ...
        rule = FiscalPositionTaxRule.query.filter_by(
            fiscal_position_id=self.id, source_account_id=source_account_id
        ).first()
...
class FiscalPositionTaxRule(db.Model):
    ...
    fiscal_position_id = db.Column(
        db.Integer, db.ForeignKey("fiscal_positions.id"), nullable=False, index=True
    )
    ...
    destination_tax_id = db.Column(
        db.Integer, db.ForeignKey("tax_calculation_rules.id")
    )
    destination_account_id = db.Column(db.Integer, db.ForeignKey("gl_accounts.id"))

    # Rule type
    rule_type = db.Column(
        db.String(20), nullable=False, default="tax"
    )  # 'tax' or 'account'

    tenant = db.relationship(
        "Tenant", backref="fiscal_position_tax_rules", foreign_keys=[tenant_id]
    )
    ...
    destination_tax = db.relationship(
        "TaxCalculationRule", foreign_keys=[destination_tax_id]
    )
    source_account = db.relationship("GLAccount", foreign_keys=[source_account_id])
    destination_account = db.relationship(
        "GLAccount", foreign_keys=[destination_account_id]
    )
