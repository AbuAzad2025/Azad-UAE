...
class CRMTeamMember(db.Model):
    ...
    team_id = db.Column(
        db.Integer,
        db.ForeignKey("crm_teams.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    user_id = db.Column(
...
class CRMLead(db.Model):
    ...
    stage_id = db.Column(
        db.Integer,
        db.ForeignKey("crm_stages.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    team_id = db.Column(
    ...
    def to_dict(self, lang="ar"):
        return {
            ...
            "stage_name": (
                self.stage.name_ar
                if self.stage and lang == "ar" and self.stage.name_ar
                else (self.stage.name if self.stage else "")
            ),
...
class CRMActivity(db.Model):
    ...
    lead_id = db.Column(
        db.Integer,
        db.ForeignKey("crm_leads.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    user_id = db.Column(
