...
- `scripts/ops/first_run_dev.py` — drops+recreates DBs, `flask db upgrade`, system_init (37 perms/8 roles/owner/3 currencies/76 industry fields/GL base), optional `--with-demo` (no seed-packages: SaaS packages are created by the platform owner from Owner panel `payment_vault/packages-management`)
