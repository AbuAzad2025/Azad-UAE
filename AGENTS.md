...
## 🔒 Financial & Security Integrity Rules (MANDATORY)

These rules were added after a comprehensive security audit of the Azadexa ERP.
**AI coding agents MUST follow these constraints. Violations can cause data leakage, financial inaccuracies, or audit failures.**

### 1. Tenant Isolation — NEVER trust client-side IDs

- **Every** API endpoint that accepts an `<id>` path parameter MUST validate that the resource belongs to `current_user.tenant_id` before returning data.
- Use `tenant_get_or_404(model, id)` or `assert_tenant_record(record)` from `utils/tenanting.py` — never use raw `db.session.get(model, id)` on user-facing routes.
- For route-level validation, use `@validate_tenant_ownership(model_class)` from `utils/tenant_security.py` (auto-detects ID parameter).
- **HARD RULE: It is strictly forbidden to write any SQLAlchemy query or Raw SQL dealing with tenant data without explicitly including a tenant_id filter or using the custom ownership decorator.**
- NEVER write queries that lack explicit `tenant_id` filtering unless the code is inside `without_tenant_scope()`.
- The User model is exempt from automatic ORM scoping (required by Flask-Login). Always use `scoped_user_query()` for user queries.

### 2. Inventory (MWAC) — Row-level locking is NON-NEGOTIABLE

- When updating `ProductWarehouseCost`, ALWAYS use `_safe_for_update(query, label)` from `services/stock_service.py`.
- **NEVER** wrap `with_for_update()` in `try/except Exception: fallback to no lock`. This creates race conditions where two concurrent transactions corrupt WAC.
- `ProductCostHistory` is the immutable audit trail — every MWAC change MUST log old/new cost, quantity, and value.

### 3. Journal Entries — State Machine is MANDATORY

- Journal entries flow: **Draft → Validated → Posted**. No entry may be posted without passing `validate_entry()` first.
- `validate_entry()` checks: (a) SUM(debit) == SUM(credit), (b) no header accounts used directly.
- Financial documents are **immutable after posting** — use `reverse_entry_advanced()` to reverse, never delete.
- **NEVER use `db.delete()` on journal entries or lines.** Use status='cancelled' for soft-delete.

### 4. Cheques — Strict lifecycle enforcement

- Cheques follow: `pending → deposited → cleared/bounced/cancelled`.
- Incoming cheques MUST be deposited before they can be bounced (prevents skipping bank tracking).
- Bounced cheques trigger a reversing GL entry + re-open AR/AP — never just flip a boolean.

### 5. Cancellations & Returns — No-Delete financial policy

- Sales, purchases, invoices, bills, and journal entries CANNOT be deleted after posting.
- Cancellations create reversal entries linked via `reverses_invoice_id`.
- Inventory reversals use the **original cost** from `ProductCostHistory` at the time of sale, NOT the current WAC.
...
`C:\Users\azad1\.openclaw-autoclaw\skills/<skill-name>/SKILL.md`
