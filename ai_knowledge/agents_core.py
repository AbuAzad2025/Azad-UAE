...
def intelligent_response(message: str, user_id: int = None, context: dict = None) -> str:
    """الحصول على رد ذكي - مع دعم العمليات والأخطاء"""
    try:
        # Try action dispatch first for operational messages
        ...
        if parsed:
            ...
            # On permission failure, still try local analysis
...
def _build_system_prompt(question: str, user_role: str = "user") -> str:
    ...
    from ai_knowledge.system_knowledge import search_knowledge, ACCOUNTING, SYSTEM_INFO, PERMISSIONS, ROLES
...
def ask_azad_enhanced(question: str, context: dict = None, user_id: int = None) -> dict:
    ...
    Returns dict with answer, source, confidence, thinking_steps.
    """
    result = {
        "answer": "",
        "source": "local",
    ...
    try:
        ...
        if result["source"] == "error":
            _log_ai_error("ask_azad_error", f"Source: {result['source']}", request_data={"question": question[:200]})
    except Exception:
        pass
