...
class ActionResult:
    ...
    def __init__(self, success: bool, message: str, data: Any = None,
                 action_type: str = "", needs_permission: str = ""):
...
class ActionDispatcher:
    ...
    def _register(self, action_type: str, handler: Callable,
                  permission: str = "", description: str = ""):
    ...
    def _register_all(self):
        ...
        def _list_customers(args: dict) -> ActionResult:
            try:
                ...
                return ActionResult(True, f"تم العثور على {len(data)} عميل",
                                    {"customers": data, "count": len(data)},
                                    "customer_list", "manage_customers")
            except Exception as e:
        ...
        def _get_customer_balance(args: dict) -> ActionResult:
            ...
            try:
                ...
                customer = Customer.query.filter_by(
                    tenant_id=tid, name=name, is_active=True).first()
                if not customer:
                    return ActionResult(False, f"العميل {name} غير موجود")
                balance = float(customer.balance or 0)
                status = "طبيعي" if balance < 1000 else \
                    ("تنبيه" if balance < 5000 else "خطير")
                return ActionResult(True,
                    f"رصيد العميل {name}: {balance:,.2f} درهم - {status}",
                    {"id": customer.id, "name": name, "balance": balance,
                     "credit_limit": float(customer.credit_limit or 0)},
                    "customer_balance", "manage_customers")
            except Exception as e:
        ...
        def _list_products(args: dict) -> ActionResult:
            try:
                ...
                return ActionResult(True, f"تم العثور على {len(data)} منتج",
                                    {"products": data, "count": len(data)},
                                    "product_list", "manage_products")
            except Exception as e:
                return ActionResult(False, f"خطأ: {str(e)[:100]}")
        ...
        def _check_stock(_args: dict) -> ActionResult:
            try:
                ...
                data = [{"name": p.name, "stock": float(p.current_stock or 0),
                         "min": float(p.min_stock_level or 0)} for p in low[:20]]
                if not data:
                    return ActionResult(True, "جميع المنتجات متوفرة بكميات جيدة",
                                        {"low_stock": []}, "stock_check", "manage_warehouse")
                return ActionResult(True,
                    f"يوجد {len(data)} منتجات منخفضة المخزون",
                    {"low_stock": data, "count": len(data)},
                    "stock_check", "manage_warehouse")
            except Exception as e:
                return ActionResult(False, f"خطأ: {str(e)[:100]}")
        ...
        def _list_sales(_args: dict) -> ActionResult:
            try:
                ...
                data = [{"id": s.id, "number": s.sale_number,
                         "total": float(s.total_amount or 0),
                         "status": s.payment_status,
                         "date": str(s.sale_date)[:10]} for s in sales]
                return ActionResult(True, f"آخر {len(data)} فواتير",
                                    {"sales": data}, "sale_list", "manage_sales")
            except Exception as e:
                return ActionResult(False, f"خطأ: {str(e)[:100]}")
        ...
        def _profit_summary(_args: dict) -> ActionResult:
            try:
                ...
                tid = _get_active_tenant_id()
                sales = db.session.query(func.coalesce(func.sum(Sale.total_amount), 0))\
                    .filter(Sale.tenant_id == tid, Sale.status == "active").scalar()
                lines = db.session.query(SaleLine).join(Sale).filter(
                    Sale.tenant_id == tid, Sale.status == "active").all()
                cost = Decimal("0")
                for line in lines:
                ...
                return ActionResult(True,
                    ...
                    {"revenue": float(sales or 0), "cost": float(cost),
                     "profit": float(profit), "margin_percent": margin},
                    "profit_summary", "view_reports")
            except Exception as e:
                return ActionResult(False, f"خطأ: {str(e)[:100]}")
        ...
        self._register("customer_balance", _get_customer_balance, "manage_customers", "عرض رصيد عميل")
    ...
    def dispatch(self, action_type: str, args: dict | None = None) -> ActionResult:
        ...
        if perm and not _is_owner() and not _has_permission(perm):
            _log_ai_error("permission_denied", f"Missing permission: {perm}",
        ...
        try:
            ...
            _log_ai_error("action_dispatch_error", str(e),
                          request_data={"action": action_type, "args": args})
