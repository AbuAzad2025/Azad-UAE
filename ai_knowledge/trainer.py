...
SEED_QA = [
    ...
    (
        "ملخص النظام",
        "هذا نظام أزاد لإدارة كراجات وورش المعدات الثقيلة.\n"
    ...
    (
        "ما هي صلاحياتي؟",
        "يمكنك معرفة صلاحياتك من صفحة المستخدم أو بسؤالي عن صلاحية معينة.",
    ),
    # Commands
    (
        "كيف أضيف عميلاً؟",
        "لإضافة عميل جديد استخدم الأمر:\n"
    ...
    (
        "أفضل العملاء",
        "يمكنني عرض العملاء حسب إجمالي مشترياتهم. أكتب:\n"
        "`ملخص المبيعات` لعرض التقرير الكامل.",
    ),
    (
        "منتجات منخفضة الربح",
        "يمكنني تحليل هوامش الربح لكل منتج. أكتب:\n`هوامش الربح` لتحليل تفصيلي.",
    ),
    (
        "مبيعات اليوم",
        "للاستعلام عن مبيعات اليوم، استخدم:\n`ملخص المبيعات` في قائمة التقارير.",
    ),
    # Inventory
    (
        "صحة المخزون",
        "يمكنني تقييم المخزون عبر:\n"
    ...
    (
        "الأرباح",
        "لتحليل الأرباح استخدم:\n`ملخص الأرباح` أو اسأل 'ما هي أرباح هذا الشهر؟'",
    ),
    (
        "التدفق النقدي",
        "يمكنني توقع التدفق النقدي بناءً على:\n"
        "- المبيعات المسجلة\n"
        "- المصروفات\n"
        "- المدفوعات المستحقة\n"
    ...
    (
        "الشيكات",
        "يمكنني مساعدتك في إدارة الشيكات:\n"
...
class Trainer:
    ...
    def seed(self) -> None:
        ...
        try:
            ...
            for path in glob.glob(
                os.path.join(training_dir, "GLOBAL", "expertise", "*.json")
            ):
                with open(path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                areas = (
                    data.get("expertise_areas", []) if isinstance(data, dict) else data
                )
                for area in areas:
    ...
    def get_stats(self) -> dict:
        ...
        total = (
            db.session.query(AiMemory)
            .filter(
                AiMemory.source.in_(
                    ["quick_learner", "system", "expertise", "learned", "corrected"]
                )
            )
            .count()
        )
        cat_rows = (
            db.session.query(AiMemory.category, db.func.count(AiMemory.id))
            .filter(
                AiMemory.source.in_(
                    ["quick_learner", "system", "expertise", "learned", "corrected"]
                )
