...
from routes.owner import (
    ...
    login_required, current_user, owner_required, company_admin_required,
    db, text,
)
...
def maintenance_fix_cost_centers():
    ...
    if confirm != 'FIX_COST_CENTERS':
        return jsonify({'success': False, 'error': 'يجب كتابة FIX_COST_CENTERS للتأكيد'}), 400
    ...
    try:
        ...
        return jsonify({'success': True, 'message': '✅ تم إصلاح فهارس مراكز التكلفة بنجاح'})
...
def maintenance_rebuild_gl_tree():
    ...
    if confirm != 'REBUILD_GL_TREE':
        return jsonify({'success': False, 'error': 'يجب كتابة REBUILD_GL_TREE للتأكيد'}), 400
...
def maintenance_fix_default_tenant():
    ...
    if confirm != 'FIX_DEFAULT_TENANT':
        return jsonify({'success': False, 'error': 'يجب كتابة FIX_DEFAULT_TENANT للتأكيد'}), 400
    ...
    try:
        ...
        if dry_run:
            return jsonify({'success': True, 'message': f'🔍 تجربة جافة: سيتم تصحيح {patched} عمود', 'result': result})
        return jsonify({'success': True, 'message': f'✅ تم تصحيح {patched} عمود في بيانات المستأجر الافتراضي', 'result': result})
    except Exception as e:
...
def maintenance_regenerate_default_backup():
    ...
    if confirm != 'REGENERATE_DEFAULT_BACKUP':
        return jsonify({'success': False, 'error': 'يجب كتابة REGENERATE_DEFAULT_BACKUP للتأكيد'}), 400
    ...
    try:
        ...
        if dry_run:
            return jsonify({'success': True, 'message': '🔍 تجربة جافة: ستتم إعادة إنشاء النسخة الاحتياطية', 'result': result})
        return jsonify({'success': True, 'message': f'✅ تم تجديد النسخة الاحتياطية: {result}', 'result': result})
    except Exception as e:
...
def maintenance_run_default_tenant_maintenance():
    ...
    if confirm != 'RUN_DEFAULT_TENANT_MAINTENANCE':
        return jsonify({'success': False, 'error': 'يجب كتابة RUN_DEFAULT_TENANT_MAINTENANCE للتأكيد'}), 400
    ...
    try:
        ...
        patched = len(result.get('patched', []))
        if dry_run:
            return jsonify({'success': True, 'message': f'🔍 تجربة جافة: سيتم تصحيح {patched} عمود وتجديد النسخة', 'result': result})
        backup = result.get('backup_regenerated', 'completed')
        return jsonify({'success': True, 'message': f'✅ صيانة كاملة: {patched} عمود مصحح، نسخة احتياطية: {backup}', 'result': result})
    except Exception as e:
...
def maintenance_cleanup_test_dbs():
    ...
    if confirm != 'CLEANUP_TEST_DBS':
        return jsonify({'success': False, 'error': 'يجب كتابة CLEANUP_TEST_DBS للتأكيد'}), 400
    ...
    try:
        ...
        if dry_run:
            return jsonify({'success': True, 'message': f'🔍 تجربة جافة: سيتم حذف {dropped} قاعدة بيانات', 'result': result})
        return jsonify({'success': True, 'message': f'✅ تم حذف {dropped} قاعدة بيانات اختبار', 'result': result})
    except Exception as e:
