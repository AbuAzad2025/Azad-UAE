...
def product_detail(slug, product_id):
    ...
    return render_template('shop/product.html', product=product, available=qty, wa_url=wa_url, related_products=related_products, recent_products=recent_products, variants=variants, loyalty_points=loyalty_points, reviews=reviews, review_count=review_count, avg_rating=avg_rating, **ctx)
...
def checkout(slug):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not address:
                ...
                raise ValueError('عنوان التوصيل مطلوب.' if ctx['lang'] == 'ar' else 'Delivery address is required.')

            if min_order > 0 and totals['subtotal'] < min_order:

                raise ValueError(

                    f'الحد الأدنى للطلب {min_order}' if ctx['lang'] == 'ar' else f'Minimum order is {min_order}'
    ...
    if not payment_methods:
        ...
        flash(
            ...
            'لا توجد طرق دفع متاحة حالياً.' if ctx['lang'] == 'ar' else 'No payment methods available.',
...
def reorder(slug, sale_id):
    ...
    cart = StoreService.get_cart(session, store.tenant_id)
    stock_map = StoreService.online_stock_map(store.tenant_id, [l.product_id for l in lines])
    for line in lines:
...
def order_confirmation(slug, token):
    ...
    sale = Sale.query.filter_by(id=int(payload['sale_id']), tenant_id=store.tenant_id, source='online_store').first_or_404()
