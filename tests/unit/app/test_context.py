...
def ctx_app():
    ...
    with (
        patch("app.context.LoggingCore.log_error"),
        patch(
            "utils.ai_access.get_ai_access_state",
            return_value={
                "allowed": True,
                "global_enabled": True,
                "tenant_enabled": True,
...
class TestContextProcessor:
    ...
    def test_utility_processor_invoice_fallback(self, ctx_app):
        inv = MagicMock(
            company_name_ar="من الفاتورة",
            company_name_en="From Inv",
            phone_1="111",
            email="i@b.com",
            address_ar="addr",
            logo_url="/inv.png",
    ...
    def test_utility_processor_tenant_exception(self, ctx_app):
        with (
            patch("models.Tenant.get_current", side_effect=RuntimeError("db fail")),
            patch(
                "models.system_settings.SystemSettings.get_current",
                side_effect=RuntimeError("sys fail"),
            ),
            patch("flask_login.current_user", MagicMock(is_authenticated=False)),
    ...
    def test_branding_name_fallback(self, ctx_app):
        ...
        with (
            ...
            patch(
                ...
                return_value={
                    "company_name_ar": "من العلامة",
                    "company_name_en": "Brand Co",
                },
            ),
            patch("models.system_settings.SystemSettings.get_current") as sys_set,
            patch("flask_login.current_user", MagicMock(is_authenticated=False)),
    ...
    def test_available_tenants_nested_log_failure(self, ctx_app):
        ...
        with (
            ...
            patch("app.context.current_user", user),
            patch(
                "app.context.LoggingCore.log_error",
