...
class TestSeedCommandsRemoved:
    """Guard: seed-demo / seed-packages CLI commands were removed.

    Tenants and SaaS packages are provisioned from the Owner panel, not
    from seed commands. The underlying services (provision_tenant_gl,
    saas_provisioning.seed_packages) remain covered by their own suites.
    """

    def test_seed_commands_absent(self, cli_app):
        runner = cli_app.test_cli_runner()
        assert runner.invoke(args=["seed-demo"]).exit_code != 0
        assert runner.invoke(args=["seed-packages"]).exit_code != 0
