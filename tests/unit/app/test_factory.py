...
class TestFactoryRoutes:
    ...
    def test_storefront_custom_domain_redirect(self, monkeypatch):
        ...
        extras = [
            patch('services.store_service.StoreService.get_store_by_host', return_value=store),
            patch('services.store_service.StoreService.is_store_publicly_available', return_value=True),
    ...
    def test_cli_commands_registration_warning(self, monkeypatch):
        monkeypatch.setenv('SKIP_SYSTEM_INTEGRITY', '1')
        with _minimal_app([patch('cli_commands.register_cli_commands', side_effect=RuntimeError('cli broken'))]) as app:
    ...
    def test_storefront_no_redirect_when_store_unavailable(self, monkeypatch):
        ...
        extras = [
            patch('services.store_service.StoreService.get_store_by_host', return_value=store),
            patch('services.store_service.StoreService.is_store_publicly_available', return_value=False),
        ]
        with _minimal_app(extras) as app:
            with app.test_request_context('/catalog'):
                for func in app.before_request_funcs[None]:
    ...
    def test_before_request_tenant_suspended(self, monkeypatch):
        ...
        extras = [
            ...
            patch('utils.tenanting.get_tenant_status', return_value={'ok': False, 'tenant': MagicMock(), 'reason': 'suspended'}),
