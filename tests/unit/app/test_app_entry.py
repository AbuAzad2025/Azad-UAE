...
class TestAppEntry:
    ...
    def test_main_masks_db_uri_and_runs_app(self, monkeypatch):
        ...
        with (
            patch.dict("os.environ", {"PORT": "59999", "HOST": "127.0.0.1"}),
            patch("services.backup_service.BackupService.initialize"),
            patch("services.auto_approval_service.schedule_auto_approval"),
    ...
    def test_auto_approval_failure_is_non_fatal(self):
        ...
        fake_app.logger = MagicMock()
        with (
            patch.dict("os.environ", {"PORT": "5002", "HOST": "127.0.0.1"}),
            patch("services.backup_service.BackupService.initialize"),
            patch(
                "services.auto_approval_service.schedule_auto_approval",
