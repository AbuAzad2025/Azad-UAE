...
"""
    ...
    errors, total_pages, total, stats = ErrorLogService.get_parsed_errors(error_file=str(log_file))
...
def test_get_parsed_errors_unknown_entry(tmp_path):
    ...
    errors, _, _, _ = ErrorLogService.get_parsed_errors(error_file=str(log_file))
...
"""
    ...
    # Filter by level
    errors, _, _, _ = ErrorLogService.get_parsed_errors(error_file=str(log_file), level_filter='ERROR')
    assert len(errors) == 1
    assert errors[0]['level'] == 'ERROR'
    
    # Filter by search
    errors, _, _, _ = ErrorLogService.get_parsed_errors(error_file=str(log_file), search='warning')
    assert len(errors) == 1
    assert errors[0]['level'] == 'WARNING'
...
def test_get_parsed_errors_pagination(tmp_path):
    ...
    # Page 1 (50 items)
    errors, total_pages, total, _ = ErrorLogService.get_parsed_errors(error_file=str(log_file), page=1, per_page=50)
    assert total == 60
    assert total_pages == 2
    assert len(errors) == 50
    
    # Page 2 (10 items)
    errors, _, _, _ = ErrorLogService.get_parsed_errors(error_file=str(log_file), page=2, per_page=50)
...
"""
    ...
    _, _, _, stats = ErrorLogService.get_parsed_errors(error_file=str(log_file))
...
def test_get_parsed_errors_file_not_exists():
    errors, _, total, _ = ErrorLogService.get_parsed_errors(error_file='nonexistent.log')
    assert total == 0
    assert errors == []
