...
class TestMonitoringHealth:
    def test_health_healthy(self, monitoring_client):
        with patch('routes.monitoring.LoggingCore.get_system_health', return_value={'status': 'healthy'}):
            resp = monitoring_client.get('/monitoring/health')
        assert resp.status_code == 200
        assert resp.get_json()['status'] == 'healthy'

    def test_health_unhealthy(self, monitoring_client):
        with patch('routes.monitoring.LoggingCore.get_system_health', return_value={'status': 'degraded'}):
            resp = monitoring_client.get('/monitoring/health')
        assert resp.status_code == 503

    def test_health_no_auth_required(self, monitoring_client):
        with unauthenticated_client(monitoring_client), \
             patch('routes.monitoring.LoggingCore.get_system_health', return_value={'status': 'healthy'}):
            resp = monitoring_client.get('/monitoring/health')
        assert resp.status_code == 200
...
class TestMonitoringMetrics:
    def test_metrics_json(self, monitoring_client):
        with patch('routes.monitoring.LoggingCore.get_app_metrics', return_value={'requests': 10}):
