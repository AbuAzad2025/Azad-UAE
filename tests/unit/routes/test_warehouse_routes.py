...
def warehouse_mocks():
    ...
    patches = [
        patch(
            "routes.warehouse.StockService.get_visible_products_query",
            return_value=visible_query,
        ),
        patch(
            "routes.warehouse.StockService.get_low_stock_products",
            return_value=[products[1]],
        ),
        patch(
            "routes.warehouse.StockService.get_out_of_stock_products",
        ...
        patch(
            "routes.warehouse.get_branch_stock_map",
            return_value={
                1: Decimal("20"),
                2: Decimal("3"),
                3: Decimal("0"),
        ...
        patch(
            ...
            return_value=_chain_query(all=[_branch()]),
        ),
...
class TestWarehouseMovements:
    ...
    def test_movements_with_branch_scope_empty_warehouses(self, warehouse_client):
        ...
        with (
            patch("routes.warehouse.branch_scope_id", return_value=3),
            patch("routes.warehouse.Warehouse.query", wh_query),
...
class TestWarehouseExtended:
    ...
    def test_create_post_online_warehouse_success(self, warehouse_admin_client, warehouse_mocks):
        ...
        wh_query.filter_by.return_value.first.return_value = None
        with (
            patch("routes.warehouse.Warehouse") as _wh_cls,
            ...
            patch(
                "services.store_service.StoreService.get_tenant_store",
                return_value=MagicMock(warehouse_id=None),
            ),
            patch("routes.warehouse.db.session"),
        ):
            resp = warehouse_admin_client.post(
    ...
    def test_create_post_tenant_limit(self, warehouse_admin_client):
        ...
        with (
            patch(
                "utils.tenant_limits.check_warehouses_limit",
                side_effect=TenantLimitError("warehouses", 5, 5),
            ),
    ...
    def test_create_post_value_error(self, warehouse_admin_client):
        with (
            patch("routes.warehouse.atomic_transaction", side_effect=ValueError("bad")),
            patch("routes.warehouse.render_template", return_value="create"),
        ):
            resp = warehouse_admin_client.post(
                "/warehouse/create",
    ...
    def test_create_post_invalid_warehouse_type_defaults(self, warehouse_admin_client, warehouse_mocks):
        ...
        wh_query.filter_by.return_value.first.return_value = None
        with (
            patch("routes.warehouse.Warehouse") as wh_cls,
            patch("routes.warehouse.Warehouse.WAREHOUSE_TYPES", ("physical", "online")),
            patch("routes.warehouse.Warehouse.TYPE_PHYSICAL", "physical"),
            patch("routes.warehouse.Warehouse.TYPE_ONLINE", "online"),
            patch("routes.warehouse.db.session"),
