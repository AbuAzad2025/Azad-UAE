...
class TestApiHelpers:
    ...
    def test_scoped_customer_balance_scoped(self, mocker):
        ...
        mocker.patch('routes.api.PaymentService.get_customer_balance_scoped', return_value=Decimal('42.5'))
    ...
    def test_validate_telemetry_missing_origin_production(self, app, mocker):
        ...
        mocker.patch('routes.api._trusted_telemetry_origins', return_value=frozenset({'https://app.test'}))
        with app.test_request_context():
            resp, code = _validate_public_telemetry_origin()
            assert code == 403
    ...
    def test_supplier_balance_scoped(self, mocker):
        mocker.patch('routes.api.branch_scope_id', return_value=2)
        mocker.patch('routes.api.PaymentService.get_supplier_balance_scoped', return_value=Decimal('7'))
...
class TestApiEndpoints:
    ...
    def test_currency_rate_success(self, api_client, mocker):
        mocker.patch(
            ...
            return_value={'rate': Decimal('3.67'), 'source': 'ecb', 'cached': True, 'age_seconds': 5},
        )
    ...
    def test_products_low_stock_failure(self, api_client, mocker):
        mocker.patch('routes.api.StockService.get_low_stock_products', side_effect=RuntimeError('db'))
        resp = api_client.get('/api/products/low-stock')
    ...
    def test_industry_fields(self, api_client, mocker):
        field = SimpleNamespace(
            field_code='vat', field_name_ar='ضريبة', field_name_en='VAT',
            field_type='text', is_required=True,
        )
        mocker.patch('services.industry_service.IndustryService.get_fields_for', return_value=[field])
    ...
    def test_search_products(self, api_client, mocker):
        product = SimpleNamespace(
            id=1, name='Widget', sku='W1', barcode='111',
            regular_price=Decimal('10'), merchant_price=None, partner_price=None,
            cost_price=Decimal('5'), unit='pc', current_stock=Decimal('3'),
    ...
    def test_search_suppliers(self, api_client, mocker):
        supplier = SimpleNamespace(
            id=2, name='Sup', company_name='Co', phone='050', email='a@t.com',
            supplier_type='local', rating=5, is_verified=True,
        )
    ...
    def test_search_customers(self, api_client, mocker):
        customer = SimpleNamespace(
            id=3, name='Cust', phone='050', email='c@t.com', customer_type='retail',
        )
        base_q = MagicMock()
        base_q.filter.return_value = base_q
        base_q.order_by.return_value = base_q
    ...
    def test_product_info(self, api_client, mocker):
        product = SimpleNamespace(
            id=9, tenant_id=1, name='P', sku='S', barcode='B',
            regular_price=Decimal('20'), current_stock=Decimal('4'),
            unit='pc', min_stock_alert=Decimal('1'),
        )
    ...
    def test_products_list(self, api_client, mocker):
        product = SimpleNamespace(
            id=1, name='Item', sku='SKU', barcode=None,
            regular_price=Decimal('5'), current_stock=Decimal('2'),
        )
        products_q = MagicMock()
        products_q.filter.return_value = products_q
        products_q.order_by.return_value = products_q
    ...
    def test_search_products_purchase_purpose(self, api_client, mocker):
        product = SimpleNamespace(
            id=2, name='Buy', sku='B1', barcode=None,
            regular_price=Decimal('1'), merchant_price=None, partner_price=None,
            cost_price=Decimal('1'), unit='pc', current_stock=Decimal('1'),
            has_serial_number=False,
        )
        product.is_low_stock = lambda: False
        products_q = MagicMock()
