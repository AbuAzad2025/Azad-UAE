...
def _payments_patches(**cfg):
    ...
    with patch('routes.payments.tenant_query', side_effect=tq_side), \
         patch('routes.payments.tenant_get_or_404', side_effect=cfg.get('get_or_404', lambda m, i: cfg.get('receipt') or cfg.get('payment'))), \
         patch('routes.payments.tenant_get', side_effect=cfg.get('tenant_get', lambda m, i, or_404=True: cfg.get('receipt'))), \
         ...
         patch('routes.payments.PaymentService.get_customer_balance_scoped', return_value=Decimal('500')), \
...
class TestViewAndPrint:
    def test_view_payment_branch_forbidden(self, payments_client):
        ...
        with _payments_patches(payment=payment, branch_scope=1), \
    ...
    def test_print_payment_with_qr(self, payments_client):
        ...
        with _payments_patches(payment=payment, branch_scope=None), \
             patch('routes.payments.tenant_get_or_404', return_value=payment), \
             patch('routes.payments.InvoiceSettings.company_print_context', return_value=(tenant, settings, {'name_ar': 'Co'})), \
             ...
             patch('routes.payments.generate_qr_data_url', return_value='data:image/png;base64,x'), \
             patch('routes.payments.resolve_default_currency', return_value='AED'), \
             patch('routes.payments.get_system_default_currency', return_value='AED'), \
             patch('routes.payments.render_template', return_value='ok') as render, \
...
class TestVoucherFlows:
    def test_create_voucher_get(self, payments_client):
        ...
        with patch('routes.payments._scoped_customers_query', return_value=cq), \
    ...
    def test_voucher_outgoing_supplier_cash(self, payments_client, mocker):
        ...
        mocker.patch('services.gl_posting.post_or_fail')
        mocker.patch('services.gl_service.GLService.ensure_core_accounts')
        mocker.patch('services.gl_service.GLService.get_payment_credit_account', return_value='1101')
        mocker.patch('services.gl_service.GLService.get_payment_credit_concept', return_value='CASH')
        with _payments_patches(supplier=supplier), \
             patch('routes.payments.atomic_transaction', return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())), \
             patch('models.Payment', MagicMock), \
             patch('routes.payments.db.session') as session:
...
class TestReceiptCrud:
    ...
    def test_view_receipt_seller_forbidden(self, payments_client, bypass_permission_auth):
        ...
        with patch('routes.payments.tenant_get', return_value=receipt), \
    ...
    def test_delete_receipt_with_links_archives(self, payments_client):
        ...
        with patch('routes.payments.tenant_get_or_404', return_value=receipt), \
             ...
             patch('routes.payments.db.session') as session:
            sale = MagicMock(paid_amount=Decimal('100'), paid_amount_aed=Decimal('100'), amount_aed=Decimal('200'), balance_due=Decimal('100'))
...
class TestApiCustomerBalance:
    ...
    def test_customer_balance_json(self, payments_client):
        ...
        sale = MagicMock(
            id=10, sale_number='S1', sale_date=datetime(2026, 1, 1),
            total_amount=Decimal('200'), balance_due=Decimal('50'),
            exchange_rate=Decimal('1'), currency='AED',
        )
        with _payments_patches(customer=customer), \
...
class TestCreateFromSaleAndPurchase:
    def test_create_from_sale_get_redirect(self, payments_client):
        sale = MagicMock(branch_id=1, customer_id=1, customer=MagicMock(), balance_due=Decimal('100'), exchange_rate=1, currency='AED')
    ...
    def test_create_payment_get_redirect(self, payments_client, mocker):
        purchase = MagicMock(
            branch_id=1, supplier_id=2, supplier_name='S', tenant_id=1,
            amount_aed=Decimal('500'), exchange_rate=1, currency='AED', id=8,
        )
...
class TestPaymentsExtendedCoverage:
    ...
    def test_print_receipt_template_fallback(self, payments_client):
        ...
        with patch('routes.payments.tenant_get', return_value=receipt), \
             ...
             patch('routes.payments.InvoiceSettings.company_print_context', return_value=(MagicMock(name_ar='Co'), settings, {})), \
             patch('utils.tenant_branding.get_print_header_context', return_value={}), \
    ...
    def test_create_from_sale_post_success(self, payments_client, mocker):
        sale = MagicMock(
            branch_id=1, customer_id=1, customer=MagicMock(),
            balance_due=Decimal('100'), exchange_rate=Decimal('1'), currency='AED',
            id=5, tenant_id=1,
        )
    ...
    def test_voucher_outgoing_customer_cash(self, payments_client, mocker):
        ...
        mocker.patch('services.gl_service.GLService.get_payment_credit_account', return_value='1101')
        mocker.patch('services.gl_service.GLService.get_payment_credit_concept', return_value='CASH')
        mocker.patch('services.gl_service.GLService.get_customer_credit_account', return_value='1130')
        mocker.patch('services.gl_service.GLService.get_customer_credit_concept', return_value='AR')
        with _payments_patches(customer=customer), \
             patch('routes.payments.atomic_transaction', return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())), \
             patch('models.Payment', MagicMock), \
             patch('routes.payments.db.session') as session:
    ...
    def test_api_customer_balance_foreign_sale(self, payments_client):
        customer = MagicMock(id=1)
        sale = MagicMock(
            id=10, sale_number='S-FX', sale_date=datetime(2026, 1, 1),
            total_amount=Decimal('200'), balance_due=Decimal('100'),
            exchange_rate=Decimal('3.67'), currency='USD',
        )
        with _payments_patches(customer=customer), \
             patch('routes.payments._scoped_customer_unpaid_sales', return_value=[sale]), \
             patch('routes.payments.get_system_default_currency', return_value='AED'):
...
class TestPaymentsDeepCoverage:
    ...
    def test_create_from_sale_currency_exception_fallback(self, payments_client, mocker):
        sale = MagicMock(
            branch_id=1, customer_id=1, customer=MagicMock(),
            balance_due=Decimal('100'), exchange_rate=Decimal('1'), currency='',
            id=5, tenant_id=1,
        )
    ...
    def test_delete_receipt_updates_sale_paid_status(self, payments_client):
        ...
        with patch('routes.payments.tenant_get_or_404', return_value=receipt), \
             ...
             patch('routes.payments.db.session') as session:
            sale = MagicMock(
                paid_amount=Decimal('100'), paid_amount_aed=Decimal('100'),
                amount_aed=Decimal('200'), balance_due=Decimal('100'),
            )
            sale_model.query.filter_by.return_value.first.return_value = sale
            arch_cls.return_value.archive_record = MagicMock()
            session.commit = MagicMock()
    ...
    def test_delete_receipt_marks_sale_paid(self, payments_client):
        ...
        with patch('routes.payments.tenant_get_or_404', return_value=receipt), \
            ...
            sale = MagicMock(
                paid_amount=Decimal('100'), paid_amount_aed=Decimal('100'),
                amount_aed=Decimal('50'), balance_due=Decimal('0'),
            )
            sale_model.query.filter_by.return_value.first.return_value = sale
            arch_cls.return_value.archive_record = MagicMock()
            session.commit = MagicMock()
    ...
    def test_create_payment_card_and_ewallet(self, payments_client, mocker):
        purchase = MagicMock(
            ...
            amount_aed=Decimal('500'), exchange_rate=Decimal('1'), currency='AED',
        )
        ...
        mocker.patch('services.gl_posting.post_or_fail')
        mocker.patch('services.gl_service.GLService.ensure_core_accounts')
        mocker.patch('services.gl_service.GLService.get_payment_credit_account', return_value='1120')
        mocker.patch('services.gl_service.GLService.get_payment_credit_concept', return_value='BANK')
        payment_inst = MagicMock(
            id=72, amount=Decimal('30'), currency='AED', exchange_rate=Decimal('1'),
            amount_aed=Decimal('30'), branch_id=1, supplier_name='S', payment_number='PAY-CARD',
            notes='', cheque_id=None,
        )
    ...
    def test_create_payment_ewallet(self, payments_client, mocker):
        ...
        mocker.patch('services.gl_service.GLService.get_payment_credit_concept', return_value='BANK')
        payment_inst = MagicMock(
            id=73, amount=Decimal('20'), currency='AED', exchange_rate=Decimal('1'),
            amount_aed=Decimal('20'), branch_id=1, supplier_name='S', payment_number='PAY-EW',
            cheque_id=None,
        )
        mocker.patch('models.Payment', return_value=payment_inst)
        with patch('utils.decorators.branch_scope_id', return_value=None), \
    ...
    def test_print_payment_currency_fallback(self, payments_client):
        ...
        with _payments_patches(payment=payment, branch_scope=None), \
             ...
             patch('utils.tenant_branding.get_print_header_context', return_value={}), \
             patch('routes.payments.number_to_arabic_words', return_value='خمسون'), \
             patch('routes.payments.resolve_default_currency', side_effect=RuntimeError('curr')), \
             patch('routes.payments.get_system_default_currency', return_value='AED'), \
             patch('routes.payments.render_template', return_value='ok'), \
             patch('models.Branch') as branch_q:
            branch_q.query.filter_by.return_value.first.return_value = MagicMock(name='Branch')
    ...
    def test_voucher_outgoing_supplier_cheque(self, payments_client, mocker):
        ...
        payment_inst = MagicMock(id=55, amount=Decimal('200'), currency='AED', exchange_rate=Decimal('1'), amount_aed=Decimal('200'), branch_id=1)
        mocker.patch('models.Payment', return_value=payment_inst)
        with _payments_patches(supplier=supplier), \
             patch('routes.payments.atomic_transaction', return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())), \
    ...
    def test_create_payment_post_bank_transfer(self, payments_client, mocker):
        ...
        mocker.patch('services.gl_service.GLService.get_payment_credit_account', return_value='1120')
        mocker.patch('services.gl_service.GLService.get_payment_credit_concept', return_value='BANK')
        payment_inst = MagicMock(id=70, amount=Decimal('100'), currency='AED', exchange_rate=Decimal('1'), amount_aed=Decimal('100'), branch_id=1, supplier_name='S', payment_number='PAY-PUR')
        mocker.patch('models.Payment', return_value=payment_inst)
        with patch('utils.decorators.branch_scope_id', return_value=None), \
             patch('routes.payments.atomic_transaction', return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())), \
             patch('routes.payments.url_for', return_value='/purchases/8'), \
    ...
    def test_delete_receipt_exception(self, payments_client, mocker):
        ...
        mocker.patch('services.gl_service.GLService.reverse_entry', side_effect=RuntimeError('gl fail'))
        with patch('routes.payments.tenant_get_or_404', return_value=receipt), \
             patch('utils.decorators.branch_scope_id', return_value=None):
            resp = payments_client.post('/payments/receipts/1/delete', follow_redirects=False)
        assert resp.status_code == 302
    ...
    def test_create_from_sale_missing_payment_method(self, payments_client, mocker):
        sale = MagicMock(
            branch_id=1, customer_id=1, customer=MagicMock(), balance_due=Decimal('100'),
            exchange_rate=1, currency='AED', id=5,
        )
        mocker.patch('routes.payments.tenant_get_or_404', return_value=sale)
    ...
    def test_voucher_outgoing_customer_cheque(self, payments_client, mocker):
        ...
        mocker.patch('services.cheque_service.process_cheque_issue')
        payment_inst = MagicMock(id=56, amount=Decimal('80'), currency='AED', exchange_rate=Decimal('1'), amount_aed=Decimal('80'), branch_id=1)
        mocker.patch('models.Payment', return_value=payment_inst)
        with _payments_patches(customer=customer), \
             patch('routes.payments.atomic_transaction', return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())), \
             patch('models.Cheque', MagicMock), \
    ...
    def test_create_payment_exceeds_balance(self, payments_client, mocker):
        purchase = MagicMock(
            id=8, branch_id=1, supplier_id=2, supplier_name='S', tenant_id=1,
            amount_aed=Decimal('100'), exchange_rate=Decimal('1'), currency='AED',
        )
        mock_query = MagicMock()
        mock_query.filter.return_value.scalar.return_value = Decimal('0')
        mocker.patch('routes.payments.db.session.query', return_value=mock_query)
    ...
    def test_view_receipt_sale_branch_scope(self, payments_client):
        ...
        with patch('routes.payments.tenant_get', side_effect=[receipt, sale]), \
    ...
    def test_create_payment_cheque_path(self, payments_client, mocker):
        ...
        mocker.patch('services.gl_service.GLService.get_payment_credit_account', return_value='1120')
        mocker.patch('services.gl_service.GLService.get_payment_credit_concept', return_value='BANK')
        payment_inst = MagicMock(id=71, amount=Decimal('50'), currency='AED', exchange_rate=Decimal('1'), amount_aed=Decimal('50'), branch_id=1, supplier_name='S', payment_number='PAY-CHQ-P', cheque_id=None)
        mocker.patch('models.Payment', return_value=payment_inst)
        with patch('utils.decorators.branch_scope_id', return_value=None), \
             patch('routes.payments.atomic_transaction', return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())), \
             patch('routes.payments.url_for', return_value='/purchases/8'), \
