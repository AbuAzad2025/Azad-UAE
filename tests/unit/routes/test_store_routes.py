...
def store_service_mocks():
    ...
    patches = [
        ...
        patch(
            ...
            return_value={5: Decimal("12")},
        ),
        patch("routes.store.StoreService.stores_globally_enabled", return_value=True),
        patch(
            "routes.store.StoreService.get_catalog_products",
            return_value=([_mock_product()], {5: Decimal("4")}),
        ),
        patch(
        ...
        patch(
            "routes.store.StoreService.ensure_unique_slug",
        ...
        patch(
            "routes.store.StoreOrderService.STATUS_LABELS_AR",
        ...
        patch(
            "routes.store.StorePaymentMethodService.get_by_code",
        ...
        patch(
            "services.store_notification_service.StoreNotificationService.whatsapp_admin_link",
...
class TestStoreAdminIndex:
    ...
    def test_admin_index_without_online_warehouse(self, store_client):
        with (
            patch("routes.store.StoreService.get_online_warehouse", return_value=None),
...
class TestStoreAdminOrders:
    ...
    def test_order_confirm_value_error(self, store_client):
        with patch(
            ...
            side_effect=ValueError("stock issue"),
        ):
