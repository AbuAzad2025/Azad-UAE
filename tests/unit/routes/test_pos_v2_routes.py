...
class TestPosPages:
    def test_index(self, pos_client):
        ...
        with (
            _pos_api_patches(warehouses=[wh]),
...
class TestPosCatalogApi:
    ...
    def test_product_lookup_out_of_stock_warning(self, pos_client):
        product = _mock_product()
        with (
            _pos_api_patches(lookup_result=(product, {1: 0})),
            patch(
                "routes.pos.serialize_pos_product",
                return_value={"is_out_of_stock": True, "name": "Item"},
            ),
        ):
            resp = pos_client.get("/pos/api/product?code=BC1")
...
class TestPosWalkin:
    ...
    def test_walkin_failure(self, pos_client):
        with (
            _pos_api_patches(),
            patch("routes.pos.get_pos_walkin_customer", side_effect=RuntimeError("db")),
        ):
            resp = pos_client.get("/pos/api/walkin-customer")
        assert resp.status_code == 400
...
class TestPosCheckout:
    ...
    def test_checkout_invalid_product(self, pos_client):
        with _pos_api_patches(locked_products=[]):
            resp = pos_client.post(
                "/pos/api/checkout",
                json=self._checkout_payload(customer_id=None, quick_customer=False),
            )
        assert resp.status_code == 400
    ...
    def test_checkout_serial_validation(self, pos_client):
        ...
        with (
            _pos_api_patches(locked_products=[product]),
            patch(
                "routes.pos.merge_checkout_lines",
                return_value=[
                    {
    ...
    def test_checkout_missing_payment_method(self, pos_client):
        with _pos_api_patches():
            resp = pos_client.post(
                "/pos/api/checkout",
                json=self._checkout_payload(paid_amount=50, payment_method=""),
            )
        assert resp.status_code == 400
    ...
    def test_checkout_invalid_paid_amount_defaults_zero(self, pos_client):
        with _pos_api_patches():
            resp = pos_client.post(
                "/pos/api/checkout",
                json=self._checkout_payload(paid_amount="not-a-number"),
            )
        assert resp.get_json()["success"] is True
...
class TestPosSessionApi:
    ...
    def test_session_open_error(self, pos_client):
        with (
            ...
            patch("routes.pos.create_pos_session", side_effect=RuntimeError("fail")),
        ):
            resp = pos_client.post("/pos/api/session/open", json={"opening_balance": 0})
        assert resp.status_code == 400
...
class TestPosKds:
    ...
    def test_kds_orders(self, pos_client):
        order = MagicMock(
            ...
            order_number="K1",
            status="pending",
            created_at=MagicMock(isoformat=lambda: "2026-06-01"),
            items_json="[]",
        )
        chain = MagicMock()
