...
def _crm_patches(**kwargs):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch('routes.crm.Customer.query', _chain_query(all=kwargs.get('customers', []))))
...
class TestCrmAuth:
    ...
    def test_pipeline_forbidden_without_permission(self, crm_client, bypass_permission_auth):
        ...
        with _crm_patches(), patch('utils.decorators.is_global_owner_user', return_value=False):
            resp = crm_client.get('/crm/pipeline')
...
class TestCrmCreateLead:
    ...
    def test_create_post_success(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.create_lead', return_value=_mock_lead()):
    ...
    def test_create_post_error(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.create_lead', side_effect=ValueError('bad')):
...
class TestCrmLeadDetail:
    def test_detail_success(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.get_lead', return_value=_mock_lead()):
            resp = crm_client.get('/crm/leads/5')
        assert resp.status_code == 200

    def test_detail_not_found_redirects(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.get_lead', side_effect=ValueError('missing')):
...
class TestCrmEditLead:
    ...
    def test_edit_post_error(self, crm_client):
        with _crm_patches(), \
             patch('routes.crm.CRMLeadService.get_lead', return_value=_mock_lead()), \
             patch('routes.crm.CRMLeadService.update_lead', side_effect=RuntimeError('fail')):
            resp = crm_client.post('/crm/leads/3/edit', data={'name': 'X'})
        assert resp.status_code == 200

    def test_edit_missing_lead_redirects(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.get_lead', side_effect=ValueError('gone')):
...
class TestCrmApi:
    def test_move_stage_success(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.move_stage', return_value=_mock_lead()):
            resp = crm_client.post('/crm/api/move-stage', json={'lead_id': 1, 'stage_id': 2})
        assert resp.status_code == 200
        assert resp.get_json()['success'] is True

    def test_move_stage_bad_request(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.move_stage', side_effect=ValueError('invalid')):
    ...
    def test_stats(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.get_pipeline_stats', return_value=[{'count': 1}]):
            resp = crm_client.get('/crm/api/stats')
        assert resp.status_code == 200
        assert resp.get_json()[0]['count'] == 1

    def test_add_activity_success(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.add_activity', return_value=MagicMock()):
            resp = crm_client.post('/crm/api/activities', json={'lead_id': 1, 'summary': 'call'})
        assert resp.status_code == 200
        assert resp.get_json()['success'] is True

    def test_add_activity_error(self, crm_client):
        with _crm_patches(), patch('routes.crm.CRMLeadService.add_activity', side_effect=KeyError('lead_id')):
