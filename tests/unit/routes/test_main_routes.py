    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                ...
                return_value=_chain_query(count=10),
            )
        )
        stack.enter_context(
            patch(
        ...
        stack.enter_context(
            patch(
                "routes.main.StockService.get_out_of_stock_products",
                return_value=kwargs.get("out_of_stock", []),
            )
        )
...
class TestMainDashboard:
    ...
    def test_dashboard_low_stock_error(self, main_client):
        with (
            ...
            patch(
                "routes.main.StockService.get_low_stock_products",
                side_effect=RuntimeError("stock fail"),
            ),
            patch(
                "routes.main.db.session.query",
                side_effect=_dashboard_query_side_effect(),
    ...
    def test_dashboard_out_of_stock_error(self, main_client):
        with (
            ...
            patch(
                "routes.main.StockService.get_out_of_stock_products",
                side_effect=RuntimeError("oos fail"),
            ),
            patch(
                "routes.main.db.session.query",
                side_effect=_dashboard_query_side_effect(),
    ...
    def test_dashboard_liquidity_exception_swallowed(
        ...
        with (
            ...
            patch("routes.main.Sale.query", _scalar_query_result([])),
            patch(
                "utils.gl_tenant.active_tenant_id",
    ...
    def test_dashboard_failure_returns_500(self, main_client):
        with (
            _main_patches(),
            patch("utils.tenanting.tenant_query", side_effect=RuntimeError("boom")),
        ):
            resp = main_client.get("/dashboard")
...
class TestMainProfile:
    def test_my_profile_renders(self, main_client, bypass_permission_auth):
        ...
        with (
            ...
            patch(
                "routes.main.db.session.query",
                return_value=_scalar_query_result((2, Decimal("500"))),
            ),
    ...
    def test_profile_update_full_name_ar(self, main_client, bypass_permission_auth):
        with (
            patch("utils.sanitizer.InputSanitizer.sanitize_text", return_value="اسم"),
            patch("routes.main.db.session") as sess,
        ):
            resp = main_client.post(
                "/my-profile/update",
                data={"full_name_ar": "اسم"},
                follow_redirects=False,
            )
        assert resp.status_code == 302
        sess.commit.assert_called()
    ...
    def test_profile_update_phone(self, main_client, bypass_permission_auth):
        with (
            patch(
                ...
                return_value="+971500000001",
            ),
            patch("routes.main.db.session") as sess,
        ):
            resp = main_client.post(
                "/my-profile/update",
                data={"phone": "0500000001"},
                follow_redirects=False,
            )
        assert resp.status_code == 302
...
class TestTenantPublicProfile:
    def test_tenant_profile_active(self, main_client):
        ...
        with (
            patch("models.tenant.Tenant.query") as tq,
