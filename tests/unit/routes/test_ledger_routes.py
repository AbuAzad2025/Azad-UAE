    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                ...
                return_value=kwargs.get("can_access_branch", True),
            )
        )
        stack.enter_context(
            patch(
                "utils.gl_tenant.scope_gl_accounts",
                side_effect=lambda q, **kw: _scoped_accounts_query(accounts),
            )
        )
        stack.enter_context(
            patch(
                "utils.gl_tenant.scope_journal_entries",
                side_effect=lambda q, **kw: _chain_query(all=[]),
            )
        )
        stack.enter_context(
            patch(
                "utils.gl_tenant.gl_entry_query",
                return_value=_chain_query(all=[], first=entry),
            )
        )
        stack.enter_context(
            patch(
                "utils.gl_tenant.gl_account_query",
                return_value=_scoped_accounts_query(accounts),
            )
        )
        stack.enter_context(
            patch(
                "utils.gl_tenant.scoped_model_query",
                return_value=_chain_query(all=[], count=0),
            )
        )
        stack.enter_context(
            patch(
                "utils.tenanting.tenant_query",
                return_value=_chain_query(all=[], count=0),
            )
        )
        ...
        stack.enter_context(
            patch(
                "routes.ledger.CashFlowService.generate_cash_flow",
                return_value={"sections": []},
            )
        )
        stack.enter_context(
            patch(
                "routes.ledger.AgingAnalysisService.get_receivables_aging",
                return_value={},
            )
        )
        stack.enter_context(
        ...
        stack.enter_context(
            patch(
                "routes.ledger.AgingAnalysisService.verify_payables_with_gl",
                return_value={},
            )
        )
        stack.enter_context(
            patch(
                "services.depreciation_service.DepreciationService.run_monthly",
...
class TestLedgerPages:
    ...
    def test_run_depreciation_with_errors(self, ledger_client):
        with (
            ...
            patch(
                "services.depreciation_service.DepreciationService.run_monthly",
                return_value={"posted": 0, "skipped": 0, "errors": ["fail"]},
            ),
        ):
            resp = ledger_client.post(
                "/ledger/run-depreciation",
    ...
    def test_income_statement(self, ledger_client):
        ...
        with (
            ...
            patch(
                "routes.ledger.db.session.query",
                side_effect=_ledger_query_side_effect(),
            ),
            patch("utils.tenanting.get_active_tenant_id", return_value=1),
        ):
            resp = ledger_client.get(
    ...
    def test_balance_sheet(self, ledger_client):
        ...
        with (
            _ledger_patches(branch_scope=1),
            patch("utils.gl_tenant.scope_gl_accounts", side_effect=scope),
            patch(
                "routes.ledger.db.session.query",
                side_effect=_ledger_query_side_effect(
                    credit_val=Decimal("800"),
                    debit_val=Decimal("100"),
                ),
            ),
        ):
    ...
    def test_cash_flow_error_redirects(self, ledger_client):
        with (
            ...
            patch(
                "routes.ledger.CashFlowService.generate_cash_flow",
                side_effect=RuntimeError("bad period"),
            ),
        ):
...
class TestLedgerManualEntry:
    ...
    def test_manual_entry_post_success(self, ledger_client):
        ...
        with (
            _ledger_patches(),
            patch("routes.ledger.GLService.create_manual_entry", return_value=entry),
        ):
            resp = ledger_client.post(
                "/ledger/manual-entry",
                data={
...
class TestLedgerEntryViews:
    ...
    def test_view_entry_branch_forbidden(self, ledger_client):
        ...
        eq.filter_by.return_value.first_or_404.return_value = entry
        with (
            _ledger_patches(entry=entry, branch_scope=2),
            patch("utils.gl_tenant.gl_entry_query", return_value=eq),
        ):
            resp = ledger_client.get("/ledger/entry/1")
    ...
    def test_reverse_entry_branch_forbidden(self, ledger_client):
        ...
        eq.filter_by.return_value.first_or_404.return_value = entry
        with (
            _ledger_patches(entry=entry, branch_scope=1),
            patch("utils.gl_tenant.gl_entry_query", return_value=eq),
        ):
...
class TestLedgerAdmin:
    ...
    def test_admin_add_account_currency_fallback(self, ledger_admin_client):
        ...
        with (
            ...
            patch("utils.gl_tenant.gl_account_query", return_value=q),
            patch(
                "routes.ledger.resolve_default_currency",
