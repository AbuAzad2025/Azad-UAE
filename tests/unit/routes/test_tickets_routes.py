...
def _ticket_patches(**kwargs):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch('routes.tickets.TicketCategory.query', _chain_query(all=kwargs.get('categories', []))))
        stack.enter_context(patch('routes.tickets.TicketPriority.query', _chain_query(all=kwargs.get('priorities', []))))
        stack.enter_context(patch('routes.tickets.Customer.query', _chain_query(all=kwargs.get('customers', []))))
...
class TestTicketsAuth:
    ...
    def test_list_forbidden_without_permission(self, tickets_client, bypass_permission_auth):
        ...
        with _ticket_patches(), patch('utils.decorators.is_global_owner_user', return_value=False):
            resp = tickets_client.get('/tickets/')
...
class TestTicketsCreate:
    ...
    def test_create_post_success(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.create_ticket', return_value=_mock_ticket()):
    ...
    def test_create_post_error(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.create_ticket', side_effect=ValueError('required')):
...
class TestTicketsDetail:
    def test_detail_success(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.get_ticket', return_value=_mock_ticket()):
            resp = tickets_client.get('/tickets/7')
        assert resp.status_code == 200

    def test_detail_cross_tenant_redirects(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.get_ticket', side_effect=ValueError('tenant')):
...
class TestTicketsWorkflow:
    def test_add_comment_success(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.add_comment', return_value=MagicMock()):
    ...
    def test_add_comment_error(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.add_comment', side_effect=ValueError('empty')):
            resp = tickets_client.post('/tickets/1/comment', data={}, follow_redirects=False)
        assert resp.status_code == 302

    def test_assign_success(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.assign_ticket', return_value=_mock_ticket()):
            resp = tickets_client.post('/tickets/1/assign', data={'assigned_user_id': '5'}, follow_redirects=False)
        assert resp.status_code == 302

    def test_assign_error(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.assign_ticket', side_effect=ValueError('bad')):
            resp = tickets_client.post('/tickets/1/assign', data={}, follow_redirects=False)
        assert resp.status_code == 302

    def test_resolve_success(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.resolve_ticket', return_value=_mock_ticket(status='resolved')):
            resp = tickets_client.post('/tickets/1/resolve', follow_redirects=False)
        assert resp.status_code == 302

    def test_resolve_error(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.resolve_ticket', side_effect=ValueError('gone')):
            resp = tickets_client.post('/tickets/1/resolve', follow_redirects=False)
        assert resp.status_code == 302

    def test_close_success(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.close_ticket', return_value=_mock_ticket(status='closed')):
            resp = tickets_client.post('/tickets/1/close', follow_redirects=False)
        assert resp.status_code == 302

    def test_close_error(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.close_ticket', side_effect=ValueError('gone')):
            resp = tickets_client.post('/tickets/1/close', follow_redirects=False)
        assert resp.status_code == 302

    def test_reopen_success(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.reopen_ticket', return_value=_mock_ticket(status='open')):
            resp = tickets_client.post('/tickets/1/reopen', follow_redirects=False)
        assert resp.status_code == 302

    def test_reopen_error(self, tickets_client):
        with _ticket_patches(), patch('routes.tickets.TicketService.reopen_ticket', side_effect=ValueError('gone')):
            resp = tickets_client.post('/tickets/1/reopen', follow_redirects=False)
        assert resp.status_code == 302
