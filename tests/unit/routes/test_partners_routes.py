...
def partners_client(app_factory, bypass_permission_auth):
    with patch("routes.partners.render_template", return_value="ok") as render, \
         patch("routes.partners.tenant_query", side_effect=lambda model: _tenant_query_chain(all=[])), \
...
class TestPartnersEdit:
    ...
    def test_edit_post_success_redirects(self, partners_client):
        partner = _partner_mock(2)
        with patch("routes.partners.tenant_query", return_value=_tenant_query_chain(first=partner)):
            resp = partners_client.post("/partners/2/edit", data={
...
class TestPartnersDistribute:
    ...
    def test_distribute_post_success(self, partners_client):
        partners_client._partners_mocks["service"].create_distributions.return_value = [1, 2]
        resp = partners_client.post("/partners/distribute", data={
...
class TestPartnersAddTransaction:
    def test_add_transaction_deposit(self, partners_client):
        ...
        resp = partners_client.post("/partners/3/tx", data={
...
class TestPartnersAuth:
    ...
    def test_forbidden_add_transaction_without_manage_payments(self, partners_client, mock_user):
        ...
        with patch("utils.decorators.is_global_owner_user", return_value=False), \
             patch("utils.auth_helpers.is_global_owner_user", return_value=False):
            resp = partners_client.post("/partners/1/tx", data={
                "transaction_type": "deposit",
                "amount": "10",
            })
        assert resp.status_code == 403
