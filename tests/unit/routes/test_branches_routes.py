...
def _base_patches(branches=None, first=None):
    ...
    return [
        ...
        patch("routes.branches.tenant_query", return_value=_chain_query(all=branches, first=first)),
        patch("routes.branches.tenant_get_or_404", side_effect=lambda model, bid: _mock_branch(bid)),
...
class TestBranchesIndex:
    def test_index_returns_200(self, branches_client):
        with patch("routes.branches.render_template", return_value="index") as render, \
             patch("routes.branches.tenant_query", return_value=_chain_query(all=[_mock_branch()])):
...
class TestBranchesCreate:
    ...
    def test_create_post_success(self, branches_client):
        ...
        with patches[0], patches[1], patches[2], patches[3] as mock_session, patches[4] as gl_sync, patches[5]:
    ...
    def test_create_missing_name_redirects(self, branches_client):
        with patch("routes.branches.flash") as flash, \
    ...
    def test_create_duplicate_code_redirects(self, branches_client):
        ...
        with patch("routes.branches.flash") as flash, \
             patch("routes.branches.tenant_query", return_value=_chain_query(first=existing)), \
             patch("utils.tenant_limits.check_branches_limit"):
            resp = branches_client.post(
                "/branches/create",
...
class TestBranchesEdit:
    def test_edit_get_renders_form(self, branches_client):
        branch = _mock_branch()
        with patch("routes.branches.tenant_get_or_404", return_value=branch), \
