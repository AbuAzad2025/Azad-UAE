    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                ...
                return_value=[_mock_warehouse()],
            )
        )
        stack.enter_context(
            patch(
                "routes.sales.get_accessible_warehouses",
                return_value=[_mock_warehouse()],
            )
        )
        ...
        stack.enter_context(
            patch(
                "routes.sales.SaleService.has_inventory_posted",
                return_value=kwargs.get("has_gl", False),
            )
        )
        stack.enter_context(
            patch(
                "routes.sales.StockService.get_product_stock",
        ...
        stack.enter_context(
            patch(
                "routes.sales.InvoiceSettings.get_active",
...
class TestSalesCreate:
    ...
    def test_create_post_invalid_line_skipped(self, sales_client):
        ...
        # Route returns 302 redirect for invalid lines
        assert resp.status_code == 302
    ...
    def test_create_post_currency_fallback(self, sales_client):
        with (
            _sales_patches(),
            patch(
                "routes.sales.resolve_default_currency",
    ...
    def test_create_post_currency_fallback_logs_error(self, sales_client):
        with (
            ...
            patch(
                "routes.sales.LoggingCore.log_error",
...
class TestSalesEdit:
    ...
    def test_edit_discount_update(self, sales_client):
        with _sales_patches():
            resp = sales_client.post(
                "/sales/1/edit",
                data={"notes": "n", "discount_amount": "10"},
                follow_redirects=False,
            )
        assert resp.status_code == 302
...
class TestSalesApiPrice:
    ...
    def test_api_get_price_not_found_guard(self, sales_client):
        with (
            _sales_patches(),
            patch("routes.sales.tenant_get_or_404", return_value=None),
        ):
            resp = sales_client.get("/sales/api/get-price?product_id=10&customer_id=5")
...
class TestSalesArchived:
    ...
    def test_archived_skips_missing_sale(self, sales_client):
        ...
        with (
            _sales_patches(archived=[archived_rec]),
            patch("routes.sales.tenant_get", return_value=None),
        ):
            resp = sales_client.get("/sales/archived")
        assert resp.status_code == 200
...
class TestSalesDeleteArchive:
    ...
    def test_delete_gl_warning_continues(self, sales_owner_client):
        with (
            _sales_patches(sale=_mock_sale(status="draft")),
            patch(
    ...
    def test_delete_error(self, sales_owner_client):
        ...
        with (
            _sales_patches(),
            patch("services.archive_service.ArchiveService", return_value=mock_archive),
        ):
            resp = sales_owner_client.post("/sales/1/delete", follow_redirects=False)
        assert resp.status_code == 302
    ...
    def test_restore_rollback(self, sales_client):
        archived_rec = MagicMock()
        with (
            _sales_patches(archived_record=archived_rec),
