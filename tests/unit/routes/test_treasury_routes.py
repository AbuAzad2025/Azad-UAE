...
class TestTreasuryDashboard:
    def test_treasury_returns_200(self, treasury_client):
        ...
        with patches["build_dashboard"], patches["branches"], patches["render"] as render, \
             patches["scope"], patches["access"]:
    ...
    def test_treasury_with_branch_id_arg(self, treasury_client):
        patches = _base_treasury_patches()
        with patches["build_dashboard"] as build, patches["branches"], patches["render"], \
             patches["scope"], patches["access"]:
    ...
    def test_treasury_branch_scope_mismatch_403(self, treasury_client):
        patches = _base_treasury_patches()
        with patches["build_dashboard"], patches["branches"], \
             patch("routes.treasury.report_branch_scope_id", return_value=2), \
             patches["render"] as render, patches["access"]:
    ...
    def test_treasury_branch_not_accessible_403(self, treasury_client):
        patches = _base_treasury_patches()
        with patches["build_dashboard"], patches["branches"], patches["scope"], \
...
class TestTreasuryExport:
    def test_export_xlsx_default(self, treasury_client):
        patches = _base_treasury_patches()
        with patches["build_dashboard"], patches["scope"], patches["access"], \
             patch("services.export_service.ExportService.export_to_xlsx", return_value=_export_io()) as xlsx, \
    ...
    def test_export_csv_explicit(self, treasury_client):
        patches = _base_treasury_patches()
        with patches["build_dashboard"], patches["scope"], patches["access"], \
             patch("services.export_service.ExportService.export_to_csv", return_value=_export_io()) as csv, \
    ...
    def test_export_branch_scope_mismatch_403(self, treasury_client):
        patches = _base_treasury_patches()
        with patches["build_dashboard"], \
             patch("routes.treasury.report_branch_scope_id", return_value=1), \
             patches["access"], patches["render"] as render:
...
class TestTreasuryVatReturn:
    def test_vat_return_returns_200(self, treasury_client):
        ...
        with patch("services.tax_service.TaxService.get_vat_return", return_value=report) as tax, \
...
class TestTreasuryWpsExport:
    def test_wps_export_success(self, treasury_client):
        ...
        with patch("utils.localization.get_strategy", return_value=strategy), \
