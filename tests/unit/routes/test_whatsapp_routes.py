...
class TestSendInvoice:
    ...
    def test_service_failure(self, whatsapp_client):
        ...
        with (
            _whatsapp_patches(sale=sale) as mocks,
            patch(
                "utils.error_messages.ErrorMessages.whatsapp_failed",
                return_value="failed",
            ),
        ):
            mocks["wa_svc"].send_invoice.return_value = {
                "success": False,
                "error": "gateway down",
            }
            resp = whatsapp_client.post("/whatsapp/send-invoice/5")
...
class TestSendReminder:
    def test_success(self, whatsapp_client):
        customer = MagicMock(
            phone="+971500000002",
            name="Ali",
            get_balance_aed=MagicMock(return_value=Decimal("150.50")),
        )
    ...
    def test_service_failure(self, whatsapp_client):
        customer = MagicMock(
            phone="+971500000002",
            name="Ali",
            get_balance_aed=MagicMock(return_value=Decimal("10")),
        )
        with (
            _whatsapp_patches(customer=customer) as mocks,
            patch(
                "utils.error_messages.ErrorMessages.whatsapp_failed",
                return_value="failed",
            ),
        ):
            mocks["wa_svc"].send_payment_reminder.return_value = {
                "success": False,
                "error": "rate limit",
            }
            resp = whatsapp_client.post("/whatsapp/send-reminder/3")
