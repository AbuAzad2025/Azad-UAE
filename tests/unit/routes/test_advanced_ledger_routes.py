...
from tests.unit.routes.conftest import _chain_query, app_factory, bypass_admin_auth, unauthenticated_client
...
def _advanced_ledger_patches(**kwargs):
    ...
    cheque_q = MagicMock()
    cheque_q.filter_by.return_value.order_by.return_value.limit.return_value.all.return_value = kwargs.get('cheques', [])
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch('routes.advanced_ledger.AdvancedFinancialAnalytics.get_trend_analysis', return_value=[{'month': 'Jan', 'revenue': 100, 'expenses': 40, 'profit': 60}]))
        stack.enter_context(patch('routes.advanced_ledger.AdvancedFinancialAnalytics.get_expense_breakdown', return_value=[]))
        stack.enter_context(patch('routes.advanced_ledger.AdvancedFinancialAnalytics.get_revenue_breakdown', return_value=[]))
        stack.enter_context(patch('routes.advanced_ledger.AdvancedFinancialAnalytics.get_dashboard_summary', return_value={
            'ratios': {}, 'trends': [], 'expense_breakdown': [], 'revenue_breakdown': [], 'forecast': [],
        }))
        stack.enter_context(patch('routes.advanced_ledger.AdvancedFinancialAnalytics.get_financial_ratios', return_value={'current_ratio': 1.5}))
        stack.enter_context(patch('routes.advanced_ledger.AdvancedFinancialAnalytics.get_forecasting_data', return_value={'months': []}))
        stack.enter_context(patch('routes.advanced_ledger.accounting_event_stream.get_recent_events', return_value=[{'type': 'sale', 'id': 1}]))
        stack.enter_context(patch('routes.advanced_ledger.accounting_event_stream.get_events_by_type', return_value=[{'type': 'sale'}]))
        stack.enter_context(patch('routes.advanced_ledger.ChequeAccountingIntegration.get_cheque_accounting_summary', return_value={'status': 'pending'}))
        stack.enter_context(patch('models.invoice_settings.InvoiceSettings.get_active', return_value=MagicMock(active_template='modern', enable_qr_code=False)))
...
class TestAdvancedLedgerPages:
    ...
    def test_add_customs_tax_post_success(self, advanced_ledger_client):
        ...
        with _advanced_ledger_patches(accounts_q=_accounts_query([acct])) as mocks:
            resp = advanced_ledger_client.post('/ledger/advanced/customs-taxes/add', data={
                'gl_account_id': '1',
                'name': 'Import',
                'name_ar': 'استيراد',
                'tax_type': 'customs',
    ...
    def test_add_customs_tax_post_exception(self, advanced_ledger_client):
        ...
        with _advanced_ledger_patches(accounts_q=accounts_q) as mocks, \
             patch('routes.advanced_ledger.db.session.commit', side_effect=RuntimeError('db fail')), \
    ...
    def test_add_expense_category_post_success(self, advanced_ledger_client):
        acct = _mock_account()
        with _advanced_ledger_patches(accounts_q=_accounts_query([acct])) as mocks:
            resp = advanced_ledger_client.post('/ledger/advanced/expense-categories/add', data={
                'gl_account_id': '1',
                'name': 'Travel',
                'name_ar': 'سفر',
            }, follow_redirects=False)
    ...
    def test_add_expense_category_post_exception(self, advanced_ledger_client):
        acct = _mock_account()
        with _advanced_ledger_patches(accounts_q=_accounts_query([acct])), \
             patch('routes.advanced_ledger.db.session.commit', side_effect=RuntimeError('fail')), \
             patch('utils.error_messages.ErrorMessages.unexpected_error', return_value='err'):
            resp = advanced_ledger_client.post('/ledger/advanced/expense-categories/add', data={
                'gl_account_id': '1',
                'name': 'Travel',
...
class TestAdvancedLedgerActions:
    def test_reverse_journal_entry(self, advanced_ledger_client):
        ...
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.AdvancedJournalEntryManager.reverse_entry_advanced', return_value=reversal):
    ...
    def test_reverse_journal_entry_error(self, advanced_ledger_client):
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.AdvancedJournalEntryManager.reverse_entry_advanced', side_effect=RuntimeError('fail')), \
             patch('utils.error_messages.ErrorMessages.unexpected_error', return_value='error'):
    ...
    def test_delete_journal_entry_error(self, advanced_ledger_client):
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.AdvancedJournalEntryManager.delete_entry', side_effect=RuntimeError('fail')), \
             patch('utils.error_messages.ErrorMessages.unexpected_error', return_value='err'):
    ...
    def test_receive_cheque(self, advanced_ledger_client):
        ...
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.ChequeAccountingIntegration.receive_cheque', return_value=entry):
    ...
    def test_clear_cheque(self, advanced_ledger_client):
        ...
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.ChequeAccountingIntegration.clear_cheque', return_value=entry):
            resp = advanced_ledger_client.post('/ledger/advanced/cheque-integration/1/clear', data={
                'bank_charges': '5',
    ...
    def test_approve_journal_entry_error(self, advanced_ledger_client):
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.AdvancedJournalEntryManager.approve_entry', side_effect=RuntimeError('fail')), \
             patch('utils.error_messages.ErrorMessages.unexpected_error', return_value='err'):
            resp = advanced_ledger_client.post('/ledger/advanced/journal-management/2/approve')
        assert resp.status_code == 302
    ...
    def test_receive_cheque_error(self, advanced_ledger_client):
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.ChequeAccountingIntegration.receive_cheque', side_effect=RuntimeError('fail')), \
             patch('utils.error_messages.ErrorMessages.unexpected_error', return_value='err'):
            resp = advanced_ledger_client.post('/ledger/advanced/cheque-integration/1/receive')
        assert resp.status_code == 302
    ...
    def test_clear_cheque_error(self, advanced_ledger_client):
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.ChequeAccountingIntegration.clear_cheque', side_effect=RuntimeError('fail')), \
             patch('utils.error_messages.ErrorMessages.unexpected_error', return_value='err'):
...
class TestAdvancedLedgerAPI:
    ...
    def test_cheque_summary_api_error(self, advanced_ledger_client):
        with _advanced_ledger_patches(), \
             patch('routes.advanced_ledger.ChequeAccountingIntegration.get_cheque_accounting_summary', side_effect=ValueError('missing')):
            resp = advanced_ledger_client.get('/ledger/advanced/api/cheque/9/accounting-summary')
        assert resp.status_code == 400

    def test_api_financial_ratios(self, advanced_ledger_client):
        with _advanced_ledger_patches():
            resp = advanced_ledger_client.get('/ledger/advanced/api/financial-ratios?date_from=2026-01-01&date_to=2026-06-01')
