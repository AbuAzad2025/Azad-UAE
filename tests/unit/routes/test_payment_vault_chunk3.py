...
def mock_analytics(mocker):
    mocker.patch('services.analytics_service.AnalyticsService.get_daily_stats', return_value={'today_revenue': 0, 'today_transactions': 0})
    mocker.patch('services.analytics_service.AnalyticsService.get_revenue_by_period', return_value={'labels': [], 'purchases': [], 'donations': []})
    mocker.patch('services.analytics_service.AnalyticsService.get_payment_method_stats', return_value={})
    mocker.patch('services.analytics_service.AnalyticsService.get_customer_behavior', return_value={})
    mocker.patch('services.analytics_service.AnalyticsService.get_package_performance', return_value={})
...
class TestVaultSecurityHelpers:
    ...
    def test_trusted_origins_from_config(self, app_factory):
        ...
        app = app_factory(payment_vault_bp, {'PAYMENT_VAULT_TRUSTED_ORIGINS': ['https://shop.example.com/']})
        with app.app_context():
    ...
    def test_trusted_origins_dev_defaults(self, app_factory, monkeypatch):
        from routes.payment_vault import payment_vault_bp, _payment_vault_trusted_origins, _DEV_VAULT_ORIGINS
    ...
    def test_validate_public_api_origin_no_trusted(self, app_factory, mocker):
        ...
        mocker.patch('routes.payment_vault._payment_vault_trusted_origins', return_value=frozenset())
        app = app_factory(payment_vault_bp)
    ...
    def test_validate_public_api_origin_bad_origin(self, app_factory, mocker):
        ...
        mocker.patch('routes.payment_vault._payment_vault_trusted_origins', return_value=frozenset({'http://localhost:5000'}))
        app = app_factory(payment_vault_bp)
...
class TestUnlockVaultRoutes:
    ...
    def test_unlock_post_empty_password(self, vault_owner_client, mocker):
        mocker.patch('routes.payment_vault._get_vault_for_current_tenant', return_value=MagicMock())
    ...
    def test_unlock_post_create_new_vault(self, vault_owner_client, mocker, mock_db):
        ...
        resp = vault_owner_client.post('/payment-vault/unlock', data={'vault_password': 'Secret123!'}, follow_redirects=False)
        assert resp.status_code == 302

    def test_unlock_post_success(self, vault_owner_client, mock_unlocked_vault, mocker):
        mocker.patch('routes.payment_vault.PaymentLog.log_action')
        resp = vault_owner_client.post('/payment-vault/unlock', data={'vault_password': 'good'}, follow_redirects=False)
        assert resp.status_code == 302
...
class TestVaultPageRoutes:
    ...
    def test_dashboard_unlocked(self, vault_owner_client, mock_unlocked_vault, mocker, mock_analytics):
        ...
        mocker.patch('services.notification_service.SecurityService.get_security_status', return_value={'security_level': 'ok'})
    ...
    def test_create_package_missing_names(self, vault_owner_client, mock_unlocked_vault):
        resp = vault_owner_client.post('/payment-vault/package/create', data={'name_ar': ''}, follow_redirects=False)
        assert resp.status_code == 302
    ...
    def test_create_package_duplicate_slug(self, vault_owner_client, mock_unlocked_vault, mocker):
        ...
        resp = vault_owner_client.post('/payment-vault/package/create', data={
            'name_ar': 'باقة', 'name_en': 'Pack', 'slug': 'basic',
        }, follow_redirects=False)
        assert resp.status_code == 302
    ...
    def test_create_package_success(self, vault_owner_client, mock_unlocked_vault, mocker, mock_db):
        ...
        mocker.patch('routes.payment_vault.LoggingCore.log_audit')
        resp = vault_owner_client.post('/payment-vault/package/create', data={
            'name_ar': 'باقة', 'name_en': 'Pack', 'price': '99', 'features': 'a\nb',
        }, follow_redirects=False)
        assert resp.status_code == 302
    ...
    def test_edit_package_post(self, vault_owner_client, mock_unlocked_vault, mocker, mock_db):
        ...
        resp = vault_owner_client.post('/payment-vault/package/1/edit', data={
            'name_ar': 'تعديل', 'name_en': 'Edit', 'price': '50',
        }, follow_redirects=False)
        assert resp.status_code in (200, 302)
    ...
    def test_health_check(self, vault_owner_client, mocker):
        mocker.patch('services.health_service.HealthCheckService.run_full_health_check', return_value={'overall_status': 'healthy'})
        resp = vault_owner_client.get('/payment-vault/health')
        assert resp.status_code == 200

    def test_health_check_unhealthy(self, vault_owner_client, mocker):
        mocker.patch('services.health_service.HealthCheckService.run_full_health_check', return_value={'overall_status': 'degraded'})
        resp = vault_owner_client.get('/payment-vault/health')
        assert resp.status_code == 503

    def test_metrics(self, vault_owner_client, mocker):
        mocker.patch('services.health_service.HealthCheckService.get_system_metrics', return_value={'cpu': 10})
        resp = vault_owner_client.get('/payment-vault/metrics')
        assert resp.status_code == 200

    def test_v2_stats(self, vault_owner_client, mocker):
        mocker.patch('services.analytics_service.AnalyticsService.get_daily_stats', return_value={'today_revenue': 0})
...
class TestStripeWebhook:
  ...
  def test_stripe_missing_secret(self, vault_owner_client, mocker):
      ...
      resp = vault_owner_client.post('/payment-vault/webhook/stripe', data=self.PAYLOAD, content_type='application/json', headers={'Stripe-Signature': 'sig'})
      assert resp.status_code == 503

  def test_stripe_missing_signature(self, vault_owner_client):
      resp = vault_owner_client.post('/payment-vault/webhook/stripe', data=self.PAYLOAD, content_type='application/json')
      assert resp.status_code == 400

  def test_stripe_invalid_signature(self, vault_owner_client, mocker):
      mocker.patch('services.webhook_service.WebhookService.verify_stripe_signature', return_value=False)
      resp = vault_owner_client.post('/payment-vault/webhook/stripe', data=self.PAYLOAD, content_type='application/json', headers={'Stripe-Signature': 'bad'})
  ...
  def test_stripe_success(self, vault_owner_client, mocker):
      ...
      mocker.patch('services.webhook_service.WebhookService.process_stripe_webhook', return_value={'success': True})
      resp = vault_owner_client.post('/payment-vault/webhook/stripe', data=self.PAYLOAD, content_type='application/json', headers={'Stripe-Signature': 'good'})
      assert resp.status_code == 200
...
class TestVaultExports:
    def test_export_purchases(self, vault_owner_client, mock_unlocked_vault, mocker):
        ...
        mocker.patch('services.export_service.ExportService.export_purchases_to_csv', return_value=MagicMock())
    ...
    def test_export_donations(self, vault_owner_client, mock_unlocked_vault, mocker):
        ...
        mocker.patch('routes.payment_vault.Donation.query', q)
        mocker.patch('services.export_service.ExportService.export_donations_to_csv', return_value=MagicMock())
        mocker.patch('flask.send_file', return_value=MagicMock(status_code=200))
    ...
    def test_export_cards(self, vault_owner_client, mock_unlocked_vault, mocker):
        mocker.patch('routes.payment_vault.CardPayment.query').order_by.return_value.all.return_value = []
        mocker.patch('services.export_service.ExportService.export_cards_to_csv', return_value=MagicMock())
        mocker.patch('flask.send_file', return_value=MagicMock(status_code=200))
    ...
    def test_export_report_pdf(self, vault_owner_client, mock_unlocked_vault, mocker):
        ...
        mocker.patch('routes.payment_vault.Donation.query', q)
        mocker.patch('services.export_service.ExportService.generate_pdf_report', return_value='<html></html>')
