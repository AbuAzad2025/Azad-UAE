...
def _supplier_patches(**kwargs):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                "routes.suppliers._supplier_in_scope",
                return_value=kwargs.get("in_scope", True),
            )
        )
        stack.enter_context(
            patch(
                "routes.suppliers._supplier_scoped_totals",
...
class TestSuppliersHelpers:
    ...
    def test_attach_branch_labels(self):
        ...
        with (
            patch("routes.suppliers.db.session") as sess,
...
class TestSuppliersIndex:
    ...
    def test_index_branch_columns(self, suppliers_client):
        with (
            _supplier_patches(show_branches=True),
...
class TestSuppliersCreate:
    ...
    def test_create_tenant_limit_error(self, suppliers_client):
        ...
        with (
            _supplier_patches(),
            patch(
                "utils.tenant_limits.check_suppliers_limit",
                side_effect=TenantLimitError("suppliers", 10, 10),
            ),
        ):
            resp = suppliers_client.post(
                "/suppliers/create",
...
class TestSuppliersView:
    def test_view_success(self, suppliers_client):
        ...
        with (
            ...
            patch(
                "routes.suppliers._supplier_scoped_totals",
                return_value=([], Decimal("100"), Decimal("40")),
            ),
        ):
...
class TestSuppliersApiSearch:
    def test_api_search_with_query(self, suppliers_client):
        with (
            ...
            patch(
                "routes.suppliers._supplier_scoped_totals",
                return_value=([], Decimal("10"), Decimal("3")),
            ),
        ):
    ...
    def test_api_search_error(self, suppliers_client):
        with (
            _supplier_patches(),
            patch(
                "routes.suppliers._scoped_supplier_query",
