...
class TestSecurityHelperGaps:
    ...
    def test_validate_public_api_origin_trusted_origin_header(
        self, app_factory, mocker
    ):
    ...
    def test_validate_public_api_origin_untrusted_referer(self, app_factory, mocker):
        ...
        with app.test_request_context(
            "/", headers={"Referer": "https://evil.example/pay"}
        ):
...
class TestLockedVaultErrorPaths:
    def test_delete_package_locked_returns_403(
        self, vault_owner_client, mock_locked_vault
    ):
        resp = vault_owner_client.post("/payment-vault/package/1/delete")
        assert resp.status_code == 403

    def test_decrypt_card_locked_returns_403(
        self, vault_owner_client, mock_locked_vault
    ):
        resp = vault_owner_client.post("/payment-vault/card/1/decrypt")
        assert resp.status_code == 403

    def test_purchase_detail_locked_redirects(
        self, vault_owner_client, mock_locked_vault
    ):
        resp = vault_owner_client.get(
            "/payment-vault/purchase/1", follow_redirects=False
        )
        assert resp.status_code == 302

    def test_purchases_locked_redirects(self, vault_owner_client, mock_locked_vault):
        resp = vault_owner_client.get(
            "/payment-vault/purchases", follow_redirects=False
        )
        assert resp.status_code == 302

    def test_donation_detail_locked_redirects(
        self, vault_owner_client, mock_locked_vault
    ):
        resp = vault_owner_client.get(
            "/payment-vault/donation/1", follow_redirects=False
        )
        assert resp.status_code == 302
...
class TestWebhookEdgePaths:
    ...
    def test_nowpayments_duplicate_webhook(
    ...
    def test_nowpayments_stale_timestamp_rejected(
        self, vault_owner_client, mock_unlocked_vault, mocker
    ):
    ...
    def test_stripe_duplicate_webhook(
        self, vault_owner_client, mock_unlocked_vault, mocker
    ):
        mocker.patch("routes.payment_vault._is_duplicate_webhook", return_value=True)
        mocker.patch(
...
class TestPublicApiErrorPaths:
    ...
    def test_purchase_rejects_untrusted_origin(
        self, vault_owner_client, app_factory, mocker
    ):
        mocker.patch(
            "routes.payment_vault._payment_vault_trusted_origins",
            return_value=frozenset({"http://localhost:5000"}),
    ...
    def test_purchase_crypto_nowpayments_success(
...
class TestListFilterBranches:
    def test_donations_crypto_filter(
        self, vault_owner_client, mock_unlocked_vault, mocker
    ):
        item = MagicMock()
...
class TestLockedVaultPageRedirects:
    def test_donations_locked_redirects(self, vault_owner_client, mock_locked_vault):
        resp = vault_owner_client.get(
            "/payment-vault/donations", follow_redirects=False
        )
        assert resp.status_code == 302

    def test_packages_management_locked_redirects(
        self, vault_owner_client, mock_locked_vault
    ):
        resp = vault_owner_client.get(
            "/payment-vault/packages-management", follow_redirects=False
        )
        assert resp.status_code == 302

    def test_create_package_locked_redirects(
        self, vault_owner_client, mock_locked_vault
    ):
        resp = vault_owner_client.post(
            "/payment-vault/package/create",
    ...
    def test_edit_package_locked_redirects(self, vault_owner_client, mock_locked_vault):
        resp = vault_owner_client.get(
            "/payment-vault/package/1/edit", follow_redirects=False
        )
        assert resp.status_code == 302
    ...
    def test_change_password_locked_redirects(
        self, vault_owner_client, mock_locked_vault
    ):
        resp = vault_owner_client.get(
            "/payment-vault/change-password", follow_redirects=False
        )
        assert resp.status_code == 302
...
class TestPackageEditHelperPaths:
    ...
    def test_edit_package_invalid_numeric_fields(
        ...
        pkg = MagicMock(
            price=10, max_users=1, max_branches=1, support_duration_months=3
        )
        _patch_package_query(mocker, get_or_404=pkg)
        mocker.patch.object(pv_mod, "LoggingCore")
        resp = vault_owner_client.post(
...
class TestDonationApiAndPurchasesFilter:
    ...
    def test_view_purchases_status_filter(
        self, vault_owner_client, mock_unlocked_vault, mocker
    ):
