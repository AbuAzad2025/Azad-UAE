    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                ...
                return_value=kwargs.get("in_scope", True),
            )
        )
        stack.enter_context(
            patch(
                "routes.cheques._scoped_customers_query",
                return_value=_chain_query(all=[]),
            )
        )
        stack.enter_context(
            patch(
                "routes.cheques._scoped_suppliers_query",
                return_value=_chain_query(all=[]),
            )
        )
        stack.enter_context(
...
class TestChequesHelpers:
    def test_resolve_transaction_rate(self):
        ...
        with patch(
            "routes.cheques.ExchangeRateService.resolve_exchange_rate_for_transaction",
    ...
    def test_scoped_customers_with_branch(self):
        ...
        cq.filter.return_value = cq
        with (
            patch("routes.cheques.Customer.query", cq),
            patch("routes.cheques.get_active_tenant_id", return_value=1),
            patch("routes.cheques.branch_scope_id", return_value=2),
            patch(
                "sqlalchemy.select",
    ...
    def test_scoped_suppliers_with_branch(self):
        ...
        with (
            patch("routes.cheques.Supplier.query", sq),
            patch("routes.cheques.get_active_tenant_id", return_value=1),
            patch("routes.cheques.branch_scope_id", return_value=2),
            patch(
                "sqlalchemy.select",
    ...
    def test_scoped_cheques_branch_filter(self):
        ...
        with (
            patch("routes.cheques.Cheque.query", q),
            patch("routes.cheques.get_active_tenant_id", return_value=1),
...
class TestChequesListPages:
    ...
    def test_alerts(self, cheques_client):
        ...
        with (
            _cheque_patches(),
            patch("routes.cheques._scoped_cheques_query", return_value=bounced_q),
        ):
...
class TestChequesCreate:
    ...
    def test_create_invalid_customer_scope(self, cheques_client):
        ...
        with (
            _cheque_patches(),
            patch("routes.cheques._scoped_customers_query", return_value=customers_q),
        ):
            resp = cheques_client.post(
                "/cheques/create",
                data={
    ...
    def test_create_invalid_supplier_scope(self, cheques_client):
        ...
        with (
            _cheque_patches(),
            patch("routes.cheques._scoped_suppliers_query", return_value=suppliers_q),
        ):
            resp = cheques_client.post(
                "/cheques/create",
                data={
    ...
    def test_create_currency_log_error_inner_failure(self, cheques_client):
        ...
        with (
            ...
            patch(
                "routes.cheques.LoggingCore.log_error",
    ...
    def test_clear_currency_fallback(self, cheques_client):
        with (
            ...
            resp = cheques_client.post(
                "/cheques/1/clear",
                data={"clearance_exchange_rate": "3.67"},
                follow_redirects=False,
            )
        assert resp.status_code == 302
    ...
    def test_create_exception(self, cheques_client):
        with (
            _cheque_patches(),
            patch("routes.cheques.generate_number", side_effect=RuntimeError("boom")),
        ):
            resp = cheques_client.post(
                "/cheques/create",
                data={
...
class TestChequesViewEdit:
    ...
    def test_edit_post_error(self, cheques_client):
        with (
            _cheque_patches(),
            patch(
                "routes.cheques._resolve_transaction_rate",
...
class TestChequesActions:
    ...
    def test_deposit_value_error(self, cheques_client):
        with (
            _cheque_patches(),
            patch(
                "routes.cheques.process_cheque_deposit",
                side_effect=ValueError("bad date"),
            ),
        ):
            resp = cheques_client.post(
                "/cheques/1/deposit",
                data={"deposit_date": "bad"},
                follow_redirects=False,
            )
        assert resp.status_code == 302
    ...
    def test_clear_with_gain(self, cheques_client):
        ...
        with _cheque_patches(cheque=cheque):
            resp = cheques_client.post(
                "/cheques/1/clear",
                data={"clearance_date": "2026-02-05"},
                follow_redirects=False,
            )
        assert resp.status_code == 302
    ...
    def test_clear_value_error(self, cheques_client):
        with (
            _cheque_patches(),
            patch(
                "routes.cheques.process_cheque_clear",
    ...
    def test_bounce_value_error(self, cheques_client):
        with (
            _cheque_patches(),
            patch(
                "routes.cheques.process_cheque_bounce",
