...
def _hr_patches(**kwargs):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch('routes.hr.tenant_query', side_effect=lambda model: _chain_query(all=kwargs.get('leave_types', []))))
...
class TestHrAuth:
    ...
    def test_attendance_forbidden_without_permission(self, hr_client, bypass_permission_auth):
        ...
        with _hr_patches(), patch('utils.decorators.is_global_owner_user', return_value=False):
            resp = hr_client.get('/hr/attendance')
...
class TestHrAttendance:
    ...
    def test_clock_in_success(self, hr_client):
        with _hr_patches():
            resp = hr_client.post('/hr/attendance/clock-in', data={'branch_id': '2'}, follow_redirects=False)
        assert resp.status_code == 302

    def test_clock_in_error(self, hr_client):
        with _hr_patches(), patch('routes.hr.HRService.clock_in', side_effect=ValueError('already')):
    ...
    def test_clock_out_error(self, hr_client):
        with _hr_patches(), patch('routes.hr.HRService.clock_out', side_effect=ValueError('no session')):
            resp = hr_client.post('/hr/attendance/clock-out')
        assert resp.status_code == 302
...
class TestHrLeaves:
    ...
    def test_request_leave_post_error(self, hr_client):
        with _hr_patches(), patch('routes.hr.HRService.request_leave', side_effect=ValueError('overlap')):
            resp = hr_client.post('/hr/leaves/request', data={})
        assert resp.status_code == 200
    ...
    def test_approve_leave_cross_tenant(self, hr_client):
        with _hr_patches(), patch('routes.hr.tenant_get_or_404', side_effect=NotFound()):
            resp = hr_client.post('/hr/leaves/99/approve')
        assert resp.status_code == 404

    def test_approve_leave_value_error(self, hr_client):
        with _hr_patches(), patch('routes.hr.HRService.approve_leave', side_effect=ValueError('not draft')):
            resp = hr_client.post('/hr/leaves/5/approve')
        assert resp.status_code == 302
    ...
    def test_refuse_leave_value_error(self, hr_client):
        with _hr_patches(), patch('routes.hr.HRService.refuse_leave', side_effect=ValueError('gone')):
...
class TestHrDepartments:
    ...
    def test_create_department_error(self, hr_client):
        with _hr_patches(), patch('routes.hr.HRService.create_department', side_effect=ValueError('dup')):
    ...
    def test_create_contract_error(self, hr_client):
        with _hr_patches(), patch('routes.hr.HRService.create_contract', side_effect=KeyError('user_id')):
            resp = hr_client.post('/hr/contracts/create', data={})
        assert resp.status_code == 302
