...
class TestQueryFilters:
    ...
    def test_get_dropdowns(self, mocker):
        ...
        cat_q = MagicMock()
        cat_q.distinct.return_value.order_by.return_value.all.return_value = [
            ("BACKEND",),
            ("FRONTEND",),
        ]
        lvl_q = MagicMock()
        lvl_q.distinct.return_value.order_by.return_value.all.return_value = [
            ("ERROR",),
            ("CRITICAL",),
        ]
...
class TestRequestId:
    ...
    def test_persist_user_lookup_failure(self, app, mocker):
        ...
        mocker.patch(
            "services.error_audit_service.current_user",
            MagicMock(get_id=MagicMock(side_effect=RuntimeError("user"))),
        )
        conn = MagicMock()
    ...
    def test_persist_app_config_failure(self, app, mocker):
        ...
        mocker.patch(
            "services.error_audit_service.current_app",
            MagicMock(config=MagicMock(get=MagicMock(side_effect=RuntimeError("cfg")))),
        )
        conn = MagicMock()
