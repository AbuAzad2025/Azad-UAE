...
class TestTrainer:
    ...
    def test_seed_from_expertise_json(self, knowledge_path, tmp_path):
        ...
        trainer = Trainer()
        trainer.quick_learner = ql
        with patch('ai_knowledge.trainer.os.path.dirname', return_value=str(tmp_path)), \
    ...
    def test_seed_expertise_read_error(self, knowledge_path, tmp_path):
        ...
        with patch('glob.glob', return_value=[str(bad)]), \
    ...
    def test_learn_learning_system_error(self):
        ...
        trainer.quick_learner = ql
        trainer._seeded = True
        with patch('ai_knowledge.core.learning_system.learning_system', side_effect=RuntimeError()):
...
class TestExpansionCore:
    ...
    def test_knowledge_expander_fetch_failure(self, knowledge_path):
        from ai_knowledge.expansion_core import KnowledgeExpander
        import requests
        with patch('ai_knowledge.expansion.knowledge_expansion.requests.get', side_effect=requests.RequestException('timeout')):
    ...
    def test_knowledge_source_manager(self):
        from ai_knowledge.expansion_core import KnowledgeSourceManager, get_learning_resources
        ...
        with patch('ai_knowledge.expansion.knowledge_sources.requests.get', return_value=mock_resp):
...
class TestGenerationCore:
    ...
    def test_code_generator_all(self):
        ...
        gen = CodeGenerator()
        assert 'SELECT' in gen.generate_sql_query('select', 'sales', {'where': {'id': 1}, 'order_by': 'id', 'limit': 10})
...
class TestLearningEngineConsolidated:
    ...
    def test_auto_retraining(self, knowledge_path):
        ...
        with patch.object(AutoRetrainingScheduler, 'get_last_training_info', return_value=None), \
            ...
            with patch.object(AutoRetrainingScheduler, 'get_last_training_info', return_value={
                'timestamp': (datetime.now() - timedelta(days=8)).isoformat(),
                'sales_count': 5,
            }):
                assert AutoRetrainingScheduler.should_retrain() is True
        ...
        with patch('ai_knowledge.neural.neural_engine.get_neural_engine') as gne, \
             patch('models.Sale') as MockSale:
        ...
        with patch('ai_knowledge.neural.neural_engine.get_neural_engine', side_effect=RuntimeError()):
    ...
    def test_continuous_learner(self, knowledge_path):
        from ai_knowledge.learning_engine import ContinuousLearner, get_continuous_learner
...
class TestNeuralNetworkConsolidatedDeep:
    ...
    def test_semantic_and_transformers(self):
        from ai_knowledge.neural_network import (
            SemanticMatcher, TransformersBrain, get_confidence, get_intent,
            get_transformers_brain, semantic_matcher, understand_message,
        )
...
class TestActionDispatcherHandlers:
    @pytest.fixture
    def permitted(self):
        return patch('ai_knowledge.action_dispatcher._get_active_tenant_id', return_value=1), \
    ...
    def test_list_products_and_stock(self, permitted, mock_ai_user):
        from ai_knowledge.action_dispatcher import action_dispatcher
        product = MagicMock(id=1, name='Bolt', sku='B1', selling_price=Decimal('10'), current_stock=Decimal('5'))
        with permitted[0], permitted[1], permitted[2], permitted[3], \
    ...
    def test_sales_payment_expense(self, permitted, mock_ai_user):
        ...
        with permitted[0], permitted[1], permitted[2], permitted[3], \
             ...
             patch('models.Sale') as Sale, patch('models.Expense') as Expense, \
             patch('ai_knowledge.action_dispatcher.db.session'):
            Ex.return_value.create_sale.return_value = {'success': True, 'sale_id': 5, 'message': 'ok', 'total': 100}
            assert action_dispatcher.dispatch('create_sale', {
                'customer_name': 'Ali', 'product_name': 'Bolt', 'quantity': 2,
            }).success is True
            Ex.return_value.create_sale.return_value = {'success': False, 'error': 'fail'}
            assert action_dispatcher.dispatch('create_sale', {
            ...
            Ex.return_value.receive_payment.return_value = {'success': True, 'message': 'ok', 'payment_id': 1}
            assert action_dispatcher.dispatch('receive_payment', {'customer_name': 'Ali', 'amount': 100}).success is True
    ...
    def test_supplier_reports_employee(self, permitted, mock_ai_user):
        ...
        with permitted[0], permitted[1], permitted[2], permitted[3], \
            ...
            Ex.return_value.create_employee.return_value = {'success': True, 'message': 'ok', 'id': 2}
            assert action_dispatcher.dispatch('create_employee', {'name': 'Emp', 'salary': 3000}).success is True
            Ex.return_value.create_purchase.return_value = {'success': True, 'message': 'ok', 'purchase_id': 9}
            assert action_dispatcher.dispatch('create_purchase', {
                'supplier_name': 'S', 'product_name': 'P', 'quantity': 1,
            }).success is True

    def test_create_user_and_helpers(self, permitted, mock_ai_user):
        from ai_knowledge.action_dispatcher import action_dispatcher, _get_active_tenant_id, _is_owner
        ...
        with permitted[0], permitted[1], permitted[2], permitted[3], \
            ...
            assert action_dispatcher.dispatch('create_user', {
                'username': 'u1', 'password': 'secret', 'role': 'seller',
            }).success is True
...
class TestPersonalityCoreDeep:
    ...
    def test_azad_responses_handlers(self):
        ...
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si, \
             patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': 'greeting', 'confidence': 0.9}), \
...
class TestCoreEngineExpanded:
    def test_conversation_manager(self, knowledge_path):
        ...
        import ai_knowledge.core_engine as ce
        from ai_knowledge.core_engine import ConversationManager, get_conversation_manager
