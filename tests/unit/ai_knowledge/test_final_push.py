...
def _sale_mock():
    ...
    line = MagicMock(
        ...
        product=MagicMock(name='Oil Filter'), sale_id=42,
    )
...
class TestNeuralEngineFinal:
    ...
    def test_train_price_optimizer_paths(self, knowledge_path):
        ...
        with patch.object(engine, '_train_price_internal', return_value={'success': True, 'r2_score': 0.8}) as inner:
    ...
    def test_classify_customer_all_branches(self, knowledge_path):
        ...
        with patch('extensions.db') as mock_db, patch('models.Customer'), patch('models.Sale'):
            ...
            mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.first.return_value = _customer_row(120000)
            with patch.object(engine, '_load_model', return_value=False):
                assert engine.classify_customer_intelligence(1)['classification'] == 'vip'
            mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.first.return_value = _customer_row(60000)
            with patch.object(engine, '_load_model', return_value=False):
                assert engine.classify_customer_intelligence(2)['classification'] == 'premium'
            mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.first.return_value = _customer_row(1000, days_ago=120)
            with patch.object(engine, '_load_model', return_value=False):
            ...
            with patch.object(engine, '_load_model', return_value=True), \
                 ...
                 patch.object(engine.scalers['customer_classifier'], 'transform', return_value=np.array([[1.0] * 5])), \
                 patch.object(engine.models['customer_classifier'], 'predict', return_value=np.array([1])), \
                 patch.object(engine.models['customer_classifier'], 'predict_proba', return_value=np.array([[0.1, 0.9]])), \
...
class TestNeuralNetworkConsolidated:
    def test_neural_engine_from_consolidated(self, knowledge_path):
        ...
        with patch.object(engine, 'detect_fraud', return_value={'is_fraud': False, 'risk_score': 0.1, 'reasons': []}):
...
class TestCoreEngineConsolidated:
    def test_reasoning_engine_from_core_engine(self):
        ...
        engine.think('تسعير المنتج', {'cost_price': 100, 'customer_type': 'partner', 'quantity': 20, 'margin': 1.15, 'discount': 5})
    ...
    def test_context_engine_from_core_engine(self):
        ...
        with patch('ai_knowledge.core.context_engine.data_analyzer') as mock_da, \
            ...
            mock_ls.get_learning_insights.return_value = {'total_interactions': 20, 'top_topics': [{'topic': 'sales'}]}
    ...
    def test_system_integrator_from_core_engine(self):
        ...
        supplier = MagicMock(
            id=3, name='SupplierX', supplier_type='local', phone='050', email='s@test.com',
...
class TestAnalyticsEngineConsolidated:
    ...
    def test_sales_analytics_from_analytics_engine(self):
        from ai_knowledge.analytics_engine import SalesAnalytics, InventoryAnalytics, ProfitAnalytics, get_analytics
        ...
        assert InventoryAnalytics.calculate_reorder_point({'avg_daily_sales': 5, 'lead_time_days': 7, 'current_stock': 50})['status'] == 'ok'
...
class TestDocumentGeneratorModule:
    def test_more_document_methods(self):
        ...
        with patch('models.Sale') as MockSale, patch('models.Customer') as MockC, patch('models.Product') as MockP:
            ...
            MockC.query.all.return_value = [MagicMock(
                id=1, name='Ali', customer_type='regular', phone='', email='',
                get_balance_aed=lambda: Decimal('0'), created_at=datetime.now(),
            )]
            MockP.query.all.return_value = [MagicMock(
...
class TestAgentsCoreFinal:
    def test_get_llm_response_groq(self):
        ...
        with patch.dict('os.environ', {'GROQ_API_KEY': 'test-key', 'GEMINI_API_KEY': ''}, clear=False), \
    ...
    def test_get_llm_response_gemini_fallback(self):
        ...
        with patch.dict('os.environ', {'GROQ_API_KEY': '', 'GEMINI_API_KEY': 'g-key'}, clear=False), \
    ...
    def test_ask_azad_enhanced_llm_path(self):
        ...
        with patch('ai_knowledge.system_knowledge.search_knowledge', return_value=[]), \
             patch('ai_knowledge.system_knowledge.FAQ', {}), \
             patch('ai_knowledge.agents_core._check_llm_availability', return_value=True), \
             patch('ai_knowledge.agents_core._get_llm_response', return_value='شرح محاسبي مفصل'), \
...
class TestAzadResponsesHandlers:
    ...
    def _common_mocks(self):
        customer_payload = {
            ...
            'customer': {
                ...
                'email': 'a@test.com', 'balance_aed': 500.0, 'total_sales': 3, 'last_sale_date': '2025-01-01',
            },
        }
        debt_payload = {
            'success': True,
            'debt_analysis': {
                'unpaid_sales_count': 1, 'avg_debt_amount': 200, 'max_debt_amount': 200,
                'overdue_count': 0,
            },
        }
        product_payload = {
            'success': True,
            'product': {
                'id': 2, 'name': 'Filter', 'sku': 'F1', 'current_stock': 5,
                'alert_limit': 10, 'unit_price': 25.0, 'category': 'Parts', 'status': 'منخفض',
            },
        }
        summary_payload = {
            'success': True,
    ...
    def test_handle_methods_direct(self, responses):
        ...
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as mock_si, \
            ...
            mock_si.get_customer_sales_summary.return_value = {'success': True, 'summary': {
                'total_sales': 2, 'total_amount': 1000, 'paid_amount': 800, 'balance_due': 200,
            ...
            mock_si.get_financial_summary.return_value = {
                'success': True,
                'financial': {
                    'total_sales': 50000.0, 'total_payments': 40000.0, 'total_receivables': 10000.0,
                    'today_sales': 500.0, 'today_payments': 300.0,
                },
            }
            mock_si.search_data.return_value = {'success': True, 'results': {'customers': [], 'products': [], 'sales': []}}
    ...
    def test_smart_response_handler_routes(self, responses):
        ...
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as mock_si, \
            ...
            mock_si.get_supplier_balance.return_value = {
                'success': True,
                'supplier': {'id': 1, 'name': 'مورد', 'balance_aed': 100.0, 'total_purchases': 2,
                             'last_purchase_date': '2025-01-01', 'supplier_type': 'local', 'phone': '', 'email': ''},
            }
            mock_da.analyze_customer_debt.return_value = debt_payload
            ...
            mock_ke.search_knowledge.return_value = {'success': True, 'results': [{'title': 'نتيجة'}]}
...
class TestKnowledgeBaseFinal:
    def test_module_help_and_search(self):
        from ai_knowledge.knowledge_base import get_module_help, search_knowledge, get_welcome_message
...
class TestSystemKnowledgeFinal:
    def test_role_info_and_search_edge_cases(self):
        from ai_knowledge.system_knowledge import get_role_info, search_knowledge, get_permission_info
...
class TestSystemIntegrationPaths:
    ...
    def test_supplier_and_product_stock(self, integrator):
        supplier = MagicMock(
            id=7, name='PartsCo', supplier_type='import', phone='050', email='p@c.com',
        ...
        product = MagicMock(
            id=9, name='Brake Pad', sku='BP01', current_stock=3, min_stock_alert=5,
            unit_price=Decimal('80'), category=MagicMock(name='Brakes'),
        )
        with patch('models.Supplier') as MockSup, patch('models.Purchase'), patch('models.Product') as MockP:
            MockSup.query.get.return_value = supplier
...
class TestSecurityRulesFinal:
    def test_security_rules_full_coverage(self):
        ...
        owner = MagicMock(is_authenticated=True, is_owner=True, username='owner', role=MagicMock(slug='super_admin'))
        seller = MagicMock(is_authenticated=True, is_owner=False, username='seller', role=MagicMock(slug='seller'))
        ...
        with patch('ai_knowledge.specialized.security_rules.current_user', seller):
            filtered = SecurityRules.filter_sensitive_data({
                'password': 'secret', 'email': 'user@example.com', 'phone': '0501234567', 'name': 'Ali',
            })
