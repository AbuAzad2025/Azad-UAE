...
class TestNeuralEngineDeep:
    ...
    def test_classify_regular_stale(self, knowledge_path):
        ...
        with patch.object(engine, '_load_model', return_value=False), \
             patch('extensions.db') as mock_db, patch('models.Customer'), patch('models.Sale'):
            mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.first.return_value = self._customer_row(5000, days_ago=120)
    ...
    def test_optimize_stock_high_urgency(self, knowledge_path):
        ...
        with patch.object(engine, '_load_model', return_value=False), \
            ...
            mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.first.return_value = self._product_row(3, 20, 60)
    ...
    def test_optimize_stock_low_urgency(self, knowledge_path):
        ...
        with patch.object(engine, '_load_model', return_value=False), \
             patch('extensions.db') as mock_db, patch('models.Product'), patch('models.SaleLine'):
            mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.first.return_value = self._product_row(500, 10, 10)
            result = engine.optimize_stock_level(2)
            assert result['urgency'] == 'low'

    def test_optimize_stock_missing_product(self, knowledge_path):
        engine = self._engine(knowledge_path)
        with patch('extensions.db') as mock_db, patch('models.Product'), patch('models.SaleLine'):
            mock_db.session.query.return_value.outerjoin.return_value.filter.return_value.group_by.return_value.first.return_value = None
...
class TestContextEngineEnhance:
    ...
    def test_enhance_data_query(self):
        ...
        with patch('ai_knowledge.core.context_engine.system_integrator') as mock_si, \
             patch('ai_knowledge.core.context_engine.learning_system') as mock_ls:
            mock_si.get_system_summary.return_value = {'success': True, 'summary': {'total_customers': 10, 'total_products': 5, 'today_sales': 2000}}
    ...
    def test_enhance_prediction_create_search(self):
        ...
        with patch('ai_knowledge.core.context_engine.knowledge_expander') as mock_ke, \
             patch('ai_knowledge.core.context_engine.learning_system') as mock_ls:
            mock_ke.search_knowledge.return_value = {'success': True, 'results': [{'title': 'دليل الضريبة'}]}
            mock_ls.get_learning_insights.return_value = {'total_interactions': 20, 'top_topics': [{'topic': 'مبيعات'}]}
...
class TestSystemIntegrationDeep:
    ...
    def test_get_product_stock(self, integrator):
        ...
        product = MagicMock(id=1, name='Oil', sku='O1', current_stock=2, min_stock_alert=10, unit_price=Decimal('40'), category=category)
...
class TestDataAnalyzerDeep:
    def test_customer_debt_overdue(self):
        ...
        sale = MagicMock(id=10, total_amount=Decimal('1000'), paid_amount=Decimal('100'), created_at=datetime.now() - timedelta(days=45))
        with patch('extensions.db') as mock_db, patch('models.Sale.query') as mock_sale_q:
    ...
    def test_product_performance_single(self):
        ...
        line = MagicMock(quantity=4, line_total=Decimal('400'), unit_price=Decimal('100'), sale_id=5, sale=MagicMock(created_at=datetime.now()))
        with patch('models.Product') as MockP, patch('models.SaleLine') as MockSL, patch('models.Sale'):
            ...
            MockSL.query.filter.return_value.join.return_value.order_by.return_value.limit.return_value.all.return_value = [line]
    ...
    def test_payment_patterns_and_ratios(self):
        ...
        mock_db = MagicMock()
        mock_db.func.sum.return_value = MagicMock()
        mock_db.session.query.return_value.scalar.side_effect = [Decimal('2000'), Decimal('1500')]
        with patch('extensions.db', mock_db), patch('models.Customer') as MockC, patch('models.Payment') as MockPay, \
             patch('models.Sale'), patch('models.Product') as MockP:
...
class TestAzadPersonalityDeep:
    ...
    @pytest.mark.parametrize('mood,emoji', [
        ('happy', '😊'), ('excited', '🚀'), ('proud', '🌟'), ('smart', '💡'), ('love', '💚'), ('other', '😄'),
    ])
