...
def _patch_model_cols(*models):
    ...
    for mod in models:
        ...
        for attr in ('status', 'sale_date', 'cost_price', 'unit_price', 'tenant_id',
                     ...
                     'min_stock_alert', 'is_active', 'paid_amount', 'total_amount', 'customer_id',
                     'created_at', 'name'):
            setattr(m, attr, _Col())
        ctxs.append(p)
    return ctxs
...
class TestLearningSystemWave6:
    ...
    def test_failed_response_and_groq_paths(self, knowledge_path):
        ...
        ls.learn_from_groq_feedback({
            'question': 'q', 'local_answer': 'a', 'improved_answer': 'better',
...
class TestActionDispatcherWave6:
    def test_runtime_error_paths(self):
        ...
        with patch('ai_knowledge.action_dispatcher.current_user', MagicMock(is_owner=False, is_authenticated=True)), \
             patch('ai_knowledge.action_dispatcher._has_permission', side_effect=RuntimeError()):
    ...
    def test_remaining_handler_branches(self, mock_ai_user):
        ...
        with patch('ai_knowledge.action_dispatcher._get_active_tenant_id', return_value=1), \
            ...
            ex = Ex.return_value
            ex.create_sale.return_value = {'success': True, 'message': 'ok', 'sale_id': 1, 'total': 50}
            r = action_dispatcher.dispatch('create_sale', {
                'customer_name': 'A', 'product_name': 'P', 'unit_price': 25,
            })
            assert r.success is True
            ex.receive_payment.return_value = {'success': False, 'message': 'no'}
            assert action_dispatcher.dispatch('receive_payment', {
                'customer_name': 'A', 'amount': 50,
            }).success is False
            ...
            assert action_dispatcher.dispatch('create_employee', {'name': 'E'}).success is False
            ex.create_purchase.return_value = {'success': True, 'message': 'ok', 'purchase_id': 3}
        ...
        with patch('ai_knowledge.action_dispatcher._get_active_tenant_id', return_value=1), \
             patch('ai_knowledge.action_dispatcher._has_permission', return_value=True), \
             patch('ai_knowledge.action_dispatcher._is_owner', return_value=True), \
             patch('models.User') as User, patch('models.Role') as Role, \
             patch('ai_knowledge.action_dispatcher.db.session') as sess:
        ...
        with patch.object(reg['handler'], '__call__', side_effect=RuntimeError('boom')):
            pass
        with patch('ai_knowledge.action_dispatcher._has_permission', return_value=True), \
...
class TestDataAnalyzerWave6:
    def test_empty_debt_and_sales_trends(self):
        ...
        sale = MagicMock(
            id=10, total_amount=Decimal('1000'), paid_amount=Decimal('100'),
        ...
        sale_simple = MagicMock(
            total_amount=Decimal('100'), created_at=datetime.now(),
        ...
        with patch('models.Sale') as Sale:
            ...
            assert up.get('analysis', {}).get('trend') in ('تصاعدي', 'تنازلي', 'مستقر', 'غير محدد')
            Sale.query.filter.return_value.all.return_value = []
    ...
    def test_product_payment_ratios(self):
        from ai_knowledge.analytics.data_analyzer import DataAnalyzer
        da = DataAnalyzer()
        with patch('models.Product') as Product, patch('models.SaleLine') as SL, patch('models.Sale'):
            ...
            line = MagicMock(quantity=1, line_total=Decimal('10'), unit_price=Decimal('10'),
                             sale_id=1, sale=MagicMock(created_at=datetime.now()))
            SL.query.filter.return_value.all.return_value = [line]
            SL.query.filter.return_value.join.return_value.order_by.return_value.limit.return_value.all.return_value = [line]
        ...
        with patch('extensions.db') as mock_db, patch('models.Sale'), patch('models.Payment'), \
             patch('models.Customer') as Customer, patch('models.Product') as Product:
...
class TestSystemIntegrationWave6:
    ...
    def test_supplier_balance_errors(self, integrator):
        supplier = MagicMock(
            id=1, name='S', supplier_type='local', phone='', email='',
    ...
    def test_product_stock_and_summaries(self, integrator):
        ...
        product = MagicMock(
            id=1, name='P', sku='S', current_stock=5, min_stock_alert=10,
            unit_price=Decimal('20'), category=category,
        )
        with patch('models.Product') as Product:
        ...
        with patch('models.Customer') as Customer, patch('models.Sale') as Sale, \
             patch('models.Product') as Product, patch('models.Payment') as Payment:
            ...
            recent_c = MagicMock(id=1, name='A', customer_type='VIP', get_balance_aed=lambda: Decimal('0'))
        ...
        with patch('extensions.db') as mock_db, patch('models.Sale') as Sale, patch('models.Payment') as Payment:
            setattr(Sale, 'created_at', _Col())
            setattr(Payment, 'created_at', _Col())
            mock_db.func.sum.return_value = MagicMock()
            q = MagicMock()
...
class TestDocumentGeneratorWave6:
    def test_all_generators(self):
        ...
        line = MagicMock(
            quantity=1, unit_price=Decimal('100'), line_total=Decimal('100'),
            product=SimpleNamespace(name='Filter Oil'),
        )
        sale = MagicMock(
            id=1, created_at=datetime.now(), paid_amount=Decimal('100'),
            balance_due=Decimal('0'), subtotal=Decimal('100'),
            discount_amount=Decimal('0'), shipping_cost=Decimal('0'),
            tax_amount=Decimal('5'), total_amount=Decimal('105'),
        ...
        with patch('models.Sale') as Sale, patch('models.Customer') as Customer, \
            ...
            Customer.query.get.return_value = MagicMock(
                id=1, name='Ali', phone='', email='', address='',
            )
            stmt_sale = MagicMock(
                id=1, total_amount=Decimal('100'), created_at=datetime.now(), payments=[],
            )
            ...
            Product.query.all.return_value = [
                MagicMock(name='P', sku='S', current_stock=5, min_stock_alert=10, unit_price=Decimal('10'), category=None),
            ]
...
class TestIntelligentAssistantWave6:
    def test_analysis_branches(self, knowledge_path):
        ...
        assistant._neural_engine.predict_next_week_sales = MagicMock(return_value={'success': True, 'predicted_amount': 5000})
        ...
        cust = {'customer_data': {'success': True, 'debt_analysis': {
            'total_debt': 0, 'overdue_count': 0, 'unpaid_sales_count': 0,
        }}}
        assert assistant._analyze_and_reason('customer_balance', cust, {})['insights']
        cust2 = {'customer_data': {'success': True, 'debt_analysis': {
            'total_debt': 2000, 'overdue_count': 2, 'unpaid_sales_count': 3,
        }}}
        ...
        cd = {'customer_data': {'success': True, 'customer': {'name': 'A'}, 'debt_analysis': {
            'total_debt': 100, 'unpaid_sales_count': 1, 'overdue_count': 1,
        }}}
        ...
        with patch.object(assistant, '_generate_dynamic_response', wraps=assistant._generate_dynamic_response):
            pass
    ...
    def test_collect_partial_failures(self):
        ...
        try:
            with patch('models.Sale') as Sale, patch('models.Customer') as Customer, \
                 patch('models.Product') as Product, patch('models.Payment'), \
    ...
    def test_learn_with_tenant(self, knowledge_path):
        from ai_knowledge.agents.intelligent_assistant import IntelligentAssistant
        assistant = IntelligentAssistant()
        with patch.object(assistant.memory_system, 'remember_conversation'), \
...
class TestAzadResponsesWave6:
    ...
    def test_smart_response_edges(self, responses):
        with patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': None, 'confidence': 0}), \
             ...
             patch('ai_knowledge.personality.azad_responses.get_welcome_message', return_value='w'), \
             ...
             patch('ai_knowledge.personality.azad_responses.get_system_guide', return_value='g'), \
             patch('ai_knowledge.personality.azad_responses.get_market_insights', return_value='m'):
...
class TestNeuralEngineWave6:
    ...
    def test_remaining_neural_paths(self, engine, knowledge_path):
        ...
        with patch('extensions.db') as mock_db, patch('models.Sale'), patch('models.Purchase'), \
             patch('models.Expense'), patch('models.Receipt'), \
...
class TestSecondaryModulesWave6:
    ...
    def test_semantic_transformers_vision(self):
        from ai_knowledge.neural.semantic_matcher import SemanticMatcher, understand_message
    ...
    def test_memory_quick_learner_trainer(self, knowledge_path):
        ...
        ql = QuickLearner()
        with patch('models.ai.AiMemory') as AiMemory, patch('extensions.db.session') as sess:
...
class TestWave6FinalPush:
    ...
    def test_azad_smart_routes_extended(self, responses):
        ...
        with patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': None, 'confidence': 0}), \
             ...
             patch('ai_knowledge.personality.azad_responses.get_market_insights', return_value='market info'), \
             patch('ai_knowledge.personality.azad_responses.get_help_for_task', return_value='لم أجد'), \
             patch('ai_knowledge.personality.azad_responses.search_knowledge', return_value=[]), \
             ...
             patch('services.ai_service.AIService.analyze_profit_margins', return_value={
                 'success': True, 'overall': {'revenue': 1000, 'cost': 600, 'profit': 400, 'margin': 40},
            ...
            si.get_customer_sales_summary.return_value = {'success': True, 'summary': {}}
            ...
            si.get_financial_summary.return_value = {
                'success': True,
                'financial': {'total_sales': 1000.0, 'total_payments': 800.0, 'total_receivables': 200.0,
                                'today_sales': 50.0, 'today_payments': 30.0},
            }
            si.get_product_stock.return_value = {
                'success': True,
                'product': {'name': 'F', 'id': 1, 'sku': 'S', 'category': 'C', 'unit_price': 10.0,
                            'current_stock': 5, 'alert_limit': 10},
            }
            si.search_data.return_value = {'success': True, 'results': {'customers': [], 'products': [], 'sales': []}}
            dg.generate_invoice.return_value = ('inv', 'ok')
            ke.search_knowledge.return_value = {'success': True, 'total_found': 1, 'results': [{'title': 'T'}]}
            for msg in [
                'كيف استخدم النظام guide', 'سوق market منافسة', 'ربح profit هامش margin',
                ...
                'نكتة joke ضحك laugh', 'ضريبة vat', 'جمارك customs',
            ]:
    ...
    def test_code_and_document_remaining(self):
        ...
        assert gen.generate_report_query('profit', {}) or gen.generate_report_query('sales', {'start_date': '2025-01-01'})
        sale = MagicMock(
            id=1, created_at=datetime.now(), total_amount=Decimal('100'),
            paid_amount=Decimal('50'), balance_due=Decimal('50'),
            customer=MagicMock(name='Ali'),
        )
        with patch('models.Sale') as Sale:
            Sale.query.filter.return_value.all.return_value = [sale]
    ...
    def test_neural_semantic_batch(self, knowledge_path):
        ...
        from ai_knowledge.neural.neural_engine import AzadNeuralEngine
        matcher = SemanticMatcher()
        for msg in ('مبيعات اليوم', 'رصيد العميل', 'مخزون ناقص', 'توقع المبيعات', 'ضريبة vat'):
