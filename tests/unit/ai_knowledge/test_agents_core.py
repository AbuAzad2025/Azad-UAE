...
class TestAgentsCore:
    ...
    def test_intelligent_response_greeting(self):
        with patch('ai_knowledge.trainer.trainer.seed'), \
             patch('ai_knowledge.action_dispatcher.current_user', MagicMock(full_name='Ali')):
    ...
    def test_intelligent_response_error(self):
        with patch('ai_knowledge.trainer.trainer.seed', side_effect=RuntimeError()), \
             patch('ai_knowledge.action_dispatcher.action_dispatcher.parse_chat_action', side_effect=RuntimeError('x')), \
    ...
    def test_ask_azad_enhanced_local(self):
        ...
        with patch('ai_knowledge.agents_core._check_llm_availability', return_value=False), \
    ...
    def test_ask_azad_enhanced_faq(self):
        with patch('ai_knowledge.agents_core._check_llm_availability', return_value=False), \
             patch('ai_knowledge.system_knowledge.search_knowledge', return_value=[{'type': 'faq', 'answer': 'إجابة'}]):
