...
class TestConsolidatedImports:
    ...
    def test_personality_core_reexports(self):
        from ai_knowledge.personality_core import AzadPersonality, AzadResponses, azad_responses
    ...
    def test_core_engine_reexports(self):
        from ai_knowledge.core_engine import ContextEngine, ReasoningEngine, get_memory_system
    ...
    def test_learning_and_expansion_reexports(self):
        from ai_knowledge.learning_engine import ContinuousLearner, QuickLearner, evaluate_and_learn
...
class TestNeuralEngineTrainWave2:
    ...
    def test_train_price_optimizer_public_api(self, knowledge_path):
        engine = self._engine(knowledge_path)
        with patch.object(engine, '_train_price_internal', return_value={'success': True, 'r2_score': 0.85}) as inner:
    ...
    def test_consolidated_neural_engine_paths(self, knowledge_path):
        ...
        with patch.object(engine, '_load_model', return_value=False):
            ...
            assert engine.detect_fraud({'amount_aed': 5000, 'discount_amount': 0, 'subtotal': 5000})['is_fraud'] is False
...
class TestAzadResponsesWave2:
    ...
    @pytest.mark.parametrize('message', [
        ...
        'مورد جديد للتوريد', 'فلتر بحث المنتجات', 'طريقة دفع كاش',
        'حلل المبيعات الشهرية', 'تحسين الأداء والتعلم', 'حالة النظام والأداء',
        'توقع المبيعات القادمة', 'مخزون المنتجات منخفض', 'هامش الربح الصافي',
        'دليل استخدام النظام', 'سوق المنافسة والاستراتيجية',
        'رصيد العميل أحمد', 'بيانات العميل علي', 'مخزون منتج فلتر',
        'ملخص النظام الكلي', 'أنشئ عميل جديد', 'ابحث عن منتج زيت',
        'أضف موقع معرفة جديد', 'روابط النظام', 'مصادر موثوقة للتعلم',
        'أين أجد معلومات الضريبة', 'ابحث في المعرفة عن الجمارك',
        'فاتورة بيع جديدة', 'سند قبض جديد', 'ولد فاتورة للعميل',
        'صدر بيانات المبيعات excel', 'تقرير المبيعات الشهري',
        'قانون ضريبة فلسطين', 'شحن بحري وقوانين التخليص', 'معايير جودة الطعام',
    ])
    ...
    def test_analytical_intent_path(self, responses):
        with patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': 'sales_analysis', 'confidence': 0.9}), \
             ...
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})):
            ia.process.return_value = {'success': True, 'data_used': True, 'response': 'تحليل المبيعات'}
    ...
    def test_handler_methods(self, responses):
        summary_payload = {
            'success': True,
            'summary': {
                'customers': {'total': 10, 'vip': 2, 'recent': [{'name': 'Ali', 'type': 'vip', 'balance': 100}]},
                'sales': {'total': 100, 'today': 5, 'recent': [{'id': 1, 'customer': 'Ali', 'amount': 500, 'date': '2025-06-01'}]},
...
class TestIntelligentAssistantWave2:
    ...
    def test_full_sales_analysis_path(self, assistant, mock_ai_user):
        ...
        with patch('ai_knowledge.learning.quick_learner.quick_learner.get_answer', return_value=None), \
             ...
             patch('models.Sale') as MockSale, patch('models.Customer') as MockCustomer, patch('models.Product') as MockProduct, \
...
class TestAgentsCoreWave2:
    def test_intelligent_response_help_action(self, mock_ai_user):
        ...
        with patch('ai_knowledge.action_dispatcher.action_dispatcher.parse_chat_action', return_value=('help', {})), \
             patch('ai_knowledge.action_dispatcher.action_dispatcher.format_help', return_value='مساعدة'):
    ...
    def test_ask_azad_enhanced_faq(self):
        ...
        assert result['source'] in ('faq', 'system_knowledge', 'master_brain', 'llm', 'local')

    def test_check_llm_and_build_prompt(self):
        from ai_knowledge.agents_core import _build_system_prompt, _check_llm_availability
...
class TestContinuousLearnerWave2:
    def test_evaluate_and_learn(self):
        ...
        tests = [{'question': 'ما الضريبة', 'expected_keywords': ['5%', 'vat'], 'context': {}}]
