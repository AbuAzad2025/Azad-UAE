...
def _sale_mock():
    ...
    sale = MagicMock(
        ...
        tax_amount=Decimal('5'), payments=[MagicMock(payment_method='cash')],
    )
    return sale
...
class TestGlobalKnowledgeWave5:
    def test_fetch_exception_paths(self):
        ...
        mock_dt = MagicMock()
        mock_dt.isoformat.side_effect = [RuntimeError('time fail'), '2025-01-01T00:00:00'] * 10
    ...
    def test_currency_rates_non_200_and_exception(self):
        ...
        with patch('ai_knowledge.expansion.global_knowledge.requests.get', return_value=mock_resp):
            result = conn.fetch_currency_rates()
            assert result['success'] is True
            assert result['source'] == 'Default Rates'
        with patch('ai_knowledge.expansion.global_knowledge.requests.get', side_effect=RuntimeError('net')):
...
class TestKnowledgeExpansionWave5:
    def test_add_website_exception(self, knowledge_path):
        ...
        with patch.object(exp, '_fetch_website_content', return_value={'success': True, 'title': 'T', 'content': 'c'}), \
    ...
    def test_fetch_parse_exception(self, knowledge_path):
        from ai_knowledge.expansion.knowledge_expansion import KnowledgeExpander
        exp = KnowledgeExpander()
        mock_resp = MagicMock(status_code=200, content=b'<html><script>x</script><style>s</style><nav>n</nav><footer>f</footer><header>h</header><body><p>text</p></body></html>')
        mock_resp.raise_for_status = MagicMock()
        with patch('ai_knowledge.expansion.knowledge_expansion.requests.get', return_value=mock_resp):
            result = exp._fetch_website_content('https://example.com')
            assert result['success'] is True
        with patch('ai_knowledge.expansion.knowledge_expansion.requests.get', side_effect=RuntimeError('parse')):
    ...
    def test_search_with_website_and_category(self, knowledge_path):
        ...
        exp.sources['websites'] = [{
            'url': 'https://x.com', 'filename': 'website_1.json',
            'category': 'parts', 'description': '', 'added_date': datetime.now().isoformat(),
        }]
        site_file = knowledge_path / 'expanded_knowledge' / 'website_1.json'
        site_file.parent.mkdir(parents=True, exist_ok=True)
        site_file.write_text(json.dumps({
            'title': 'Parts Site', 'content': 'heavy equipment parts catalog',
            'url': 'https://x.com', 'category': 'parts',
        }), encoding='utf-8')
    ...
    def test_summary_exception(self, knowledge_path):
        from ai_knowledge.expansion.knowledge_expansion import KnowledgeExpander
        exp = KnowledgeExpander()
        with patch.object(exp, 'sources', property(lambda self: (_ for _ in ()).throw(RuntimeError('boom')))):
            pass
        exp.sources = None
...
class TestAutomotiveECUWave5:
    def test_all_public_methods(self):
        ...
        for prefix, cat in [('P0999', 'Powertrain'), ('C0123', 'Chassis'), ('B0456', 'Body'), ('U0100', 'Network'), ('X0000', 'Unknown')]:
...
class TestMasterBrainWave5:
    def test_init_import_failures(self):
        ...
        with patch.dict('sys.modules', {'ai_knowledge.knowledge.automotive_ecu_knowledge': None}), \
    ...
    def test_neural_and_synthesize_paths(self):
        from ai_knowledge.agents.master_brain import MasterBrain
        brain = MasterBrain()
        with patch('services.ai_service.AIService.predict_price_with_neural', return_value={
            'predicted_price': 150.0, 'margin_percent': 25.0, 'confidence': 0.9,
        }):
            neural = brain._use_neural_if_needed('pricing', {'product_id': 1})
            assert neural['predicted_price'] == 150.0
        with patch('services.ai_service.AIService.forecast_sales_neural', return_value={
            'forecast': [{'day_name': 'Mon', 'amount': 1000}],
        }):
            neural = brain._use_neural_if_needed('prediction', {})
            assert 'forecast' in neural
        with patch('services.ai_service.AIService.forecast_sales_neural', side_effect=RuntimeError('fail')):
            assert brain._use_neural_if_needed('prediction', {}) is None
        synth = brain._synthesize_answer(
            'ضريبة VAT', {'steps': []}, None,
            brain.knowledge_base.get('taxes', brain.knowledge_base.get('accounting', {})),
            'question',
        )
        assert synth['text']
        synth2 = brain._synthesize_answer('random', {'steps': []}, {
            'predicted_price': 99.0, 'margin_percent': 10, 'confidence': 0.8,
        }, {}, 'general')
        assert '99' in synth2['text']
        synth3 = brain._synthesize_answer('random', {'steps': []}, {
...
class TestReasoningEngineWave5:
    ...
    def test_financial_and_technical(self):
        from ai_knowledge.core.reasoning_engine import ReasoningEngine
        engine = ReasoningEngine()
        fin = engine.financial_reasoning('تحليل', {
            'sales': 10000, 'costs': 4000, 'expenses': 1000,
            'assets': 50000, 'liabilities': 10000,
        })
...
class TestActionDispatcherWave5:
    def test_tenant_and_permission_helpers(self):
        from ai_knowledge.action_dispatcher import _get_active_tenant_id, _has_permission, _is_owner, _audit
        ...
        with patch('ai_knowledge.action_dispatcher.current_user', MagicMock(is_authenticated=True, has_permission=MagicMock(side_effect=RuntimeError()))):
            assert _has_permission('x') is False
        with patch('ai_knowledge.action_dispatcher.current_user', SimpleNamespace(is_owner=False, is_authenticated=True, has_permission=MagicMock(return_value=False))):
    ...
    def test_dispatch_handlers(self, mock_ai_user):
        ...
        with patch('ai_knowledge.action_dispatcher._get_active_tenant_id', return_value=1), \
            ...
            ex = Ex.return_value
            ex.create_sale.return_value = {'success': True, 'message': 'ok', 'sale_id': 1, 'sale_number': 'S1', 'total': 100}
            ...
            assert action_dispatcher.dispatch('create_sale', {
                'customer_name': 'Ali', 'product_name': 'Bolt',
            }).success is False
        ...
        with patch('ai_knowledge.action_dispatcher._get_active_tenant_id', return_value=1), \
            ...
            Sale.query.filter_by.return_value.order_by.return_value.limit.return_value.all.return_value = [
                MagicMock(id=1, sale_number='S1', total_amount=100, payment_status='paid', sale_date=datetime.now()),
            ]
        ...
        with patch('ai_knowledge.action_dispatcher._get_active_tenant_id', return_value=1), \
             ...
             patch('services.ai_executor.AIExecutor') as Ex:
            ex = Ex.return_value
            ex.receive_payment.return_value = {'success': True, 'message': 'ok', 'payment_id': 5}
        ...
        with patch('ai_knowledge.action_dispatcher._get_active_tenant_id', return_value=1), \
            ...
            assert action_dispatcher.dispatch('add_expense', {
...
class TestAnalyticsWave5:
    def test_sales_analytics_branches(self):
        ...
        assert InventoryAnalytics.calculate_reorder_point({
    ...
    def test_data_analyzer_full(self):
        ...
        old_sale = MagicMock(
            id=1, total_amount=Decimal('1000'), paid_amount=Decimal('200'),
...
class TestSystemKnowledgeWave5:
    def test_get_model_info_paths(self):
        from ai_knowledge.system_knowledge import get_model_info, get_permission_info, get_role_info, search_knowledge
...
class TestLearningSystemWave5:
    def test_corrupt_loads_and_failure_paths(self, knowledge_path):
        ...
        for fname in ('learned_knowledge.json', 'interactions_log.json', 'patterns.json', 'feedback_log.json'):
    ...
    def test_classify_all_types(self, knowledge_path):
        from ai_knowledge.core.learning_system import AzadLearningSystem
        sys = AzadLearningSystem()
        for q in ['ضريبة vat', 'جمارك customs', 'محرك engine', 'مخزون stock', 'مبيعات sales', 'عميل customer', 'توقع predict', 'عام']:
...
class TestNeuralEngineWave5:
    ...
    def test_maintenance_no_product(self, engine):
        with patch('extensions.db') as mock_db, patch('models.Product'), patch('models.Sale'), patch('models.SaleLine'):
...
class TestAzadResponsesWave5:
    ...
    def test_handler_methods_direct(self, responses):
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si, \
             ...
             patch('models.Product') as Product:
            si.get_customer_balance.return_value = {'success': True, 'customer': {'name': 'Ali', 'balance_aed': 100}}
            si.get_system_summary.return_value = {
                ...
                'summary': {
                    ...
                    'sales': {
                        'total': 10,
                        'today': 2,
                        'recent': [{'id': 1, 'customer': 'Ali', 'amount': 100.0, 'date': '2025-01-01'}],
                    },
            ...
            si.get_product_stock.return_value = {
                'success': True,
                'product': {
                    'name': 'P', 'id': 1, 'sku': 'SK', 'category': 'cat',
                    'current_stock': 5, 'alert_limit': 10, 'unit_price': 25.0,
                },
            }
    ...
    def test_smart_response_branches(self, responses):
        with patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': None, 'confidence': 0}), \
             ...
             patch('ai_knowledge.personality.azad_responses.get_tax_info', return_value='tax'), \
             patch('ai_knowledge.personality.azad_responses.get_customs_info', return_value='customs'), \
             ...
             patch('ai_knowledge.personality.azad_responses.get_part_info', return_value='part'), \
            ...
            for msg in ['مرحبا', 'شكرا', 'ضريبة الإمارات', 'جمارك الإمارات', 'سوق market']:
...
class TestIntelligentAssistantWave5:
    def test_entity_extraction_and_collect(self, knowledge_path):
        ...
        sale = MagicMock(
            id=1, total_amount=Decimal('200'), sale_date=datetime.now(),
        ...
        try:
            with patch('models.Sale') as Sale, patch('models.Customer') as Customer, patch('models.Product') as Product, \
                 ...
                 patch.object(assistant.data_analyzer, 'analyze_customer_debt', return_value={
                     'success': True, 'debt_analysis': {'total_debt': 100, 'overdue_count': 0},
                 }):
    ...
    def test_process_error_paths(self, knowledge_path):
        ...
        with patch('ai_knowledge.learning.quick_learner.quick_learner.get_answer', return_value=None), \
             patch.object(assistant, '_understand_message', return_value={'intent': 'unknown_intent', 'confidence': 0.5}), \
...
class TestSecondarySweepWave5:
    ...
    def test_parts_knowledge_search(self):
        from ai_knowledge.knowledge.parts_knowledge import search_parts, get_compatible_parts
