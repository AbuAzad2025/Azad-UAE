...
def _sale_mock():
    ...
    line = MagicMock(
        quantity=2, unit_price=Decimal('50'), line_total=Decimal('100'),
        product=MagicMock(name='Oil Filter'), sale_id=42, product_id=1,
    )
    line.product.name = 'Oil Filter'
    sale = MagicMock(
        id=42, amount_aed=Decimal('105'),
        created_at=datetime(2025, 6, 1, 10, 30),
        sale_date=datetime(2025, 6, 1),
        customer=customer, status='confirmed',
        paid_amount=Decimal('100'), balance_due=Decimal('0'),
        sale_lines=[line], subtotal=Decimal('100'),
...
class TestAzadResponsesWave4:
    ...
    def test_sensitive_non_owner_and_all_users(self, responses):
        ...
        with patch('services.ai_service.AIService.is_sensitive_request', return_value=(True, False, {'message': 'مرفوض'})), \
        ...
        with patch('services.ai_service.AIService.is_sensitive_request', return_value=(True, True, {})), \
             patch('services.ai_service.AIService.get_user_info_for_owner', return_value={
                 ...
                 'users': [{'username': 'u1', 'email': 'e1', 'role': 'admin', 'password_hash': 'hash12345678901234567890'}],
             }), \
             patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': None, 'confidence': 0}), \
             patch('ai_knowledge.personality.azad_responses.azad_personality') as ap, \
             patch('ai_knowledge.personality.azad_responses.learning_system'):
    ...
    @pytest.mark.parametrize('msg,check', [
        ('شكرا جزيلا', 'شكرا'),
        ('نكتة مضحكة', 'نكتة'),
    ])
    def test_greeting_thanks_joke(self, responses, msg, check):
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('ai_knowledge.personality.azad_responses.get_welcome_message', return_value='welcome'):
    ...
    def test_dialect_greetings(self, responses):
        with patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': None, 'confidence': 0}), \
             ...
             patch('ai_knowledge.personality.azad_responses.azad_personality') as ap, \
             patch('ai_knowledge.personality.azad_responses.learning_system'), \
             patch('ai_knowledge.personality.azad_responses.get_dialectal_greeting', return_value='أهلين'), \
             patch('ai_knowledge.personality.azad_responses.get_welcome_message', return_value='welcome'):
            ap.is_inappropriate_message.return_value = 'normal'
    ...
    def test_tax_customs_parts_branches(self, responses):
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('ai_knowledge.personality.azad_responses.get_tax_info', return_value='tax info'), \
             patch('ai_knowledge.personality.azad_responses.get_customs_info', return_value='customs info'), \
             patch('ai_knowledge.personality.azad_responses.get_tax_advice', return_value='advice'), \
             patch('ai_knowledge.personality.azad_responses.search_parts', return_value=[{'category': 'engine', 'excerpt': 'part'}]), \
             patch('ai_knowledge.personality.azad_responses.get_part_info', return_value='part info'):
            assert 'customs' in responses.smart_response('جمارك الإمارات uae').lower() or 'customs' in responses.smart_response('جمارك الإمارات uae')
            assert 'tax' in responses.smart_response('ضريبة الإمارات uae').lower() or 'tax' in responses.smart_response('ضريبة الإمارات uae')
            assert 'tax' in responses.smart_response('ضريبة السعودية saudi').lower() or 'tax' in responses.smart_response('ضريبة السعودية saudi')
            assert 'tax' in responses.smart_response('ضريبة فلسطين palestine').lower() or 'tax' in responses.smart_response('ضريبة فلسطين palestine')
    ...
    def test_customer_service_and_sales_exception(self, responses):
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('ai_knowledge.personality.azad_responses.get_customer_service_tip', return_value='tip'), \
             patch('ai_knowledge.personality.azad_responses.CUSTOMER_SERVICE', {'principles': ['p1']}, create=True):
            with patch('ai_knowledge.specialized.customer_service.CUSTOMER_SERVICE', {'principles': ['p1', 'p2']}):
        ...
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('ai_knowledge.personality.azad_responses.AzadResponses._smart_sales_analysis', side_effect=RuntimeError('fail')):
    ...
    def test_forecast_branch_trends(self, responses):
        ...
        try:
            with self._safe_mocks(), \
                 ...
                 patch('ai_knowledge.personality.azad_responses.SalesAnalytics.predict_next_month_sales', return_value={
                     'prediction': 5000, 'trend': 'up', 'trend_value': 500, 'confidence': 0.9, 'method': 'linear',
                 }):
    ...
    def test_inventory_profit_failures(self, responses):
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('services.ai_service.AIService.analyze_inventory_health', return_value={'success': False, 'message': 'no products'}):
            assert 'no products' in responses.smart_response('مخزون stock inventory')
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('services.ai_service.AIService.analyze_profit_margins', return_value={'success': False, 'message': 'no sales'}):
            assert 'no sales' in responses.smart_response('ربح profit هامش margin')

    def test_help_guide_market_branches(self, responses):
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('ai_knowledge.personality.azad_responses.get_help_for_task', return_value='task help'), \
             patch('ai_knowledge.personality.azad_responses.search_knowledge', return_value=[]), \
             patch('ai_knowledge.personality.azad_responses.get_system_guide', return_value='system guide'), \
             patch('ai_knowledge.personality.azad_responses.get_market_insights', return_value='market data'):
            assert 'task help' in responses.smart_response('كيف how أستخدم النظام')
            with patch('ai_knowledge.personality.azad_responses.get_help_for_task', return_value='لم أجد'):
    ...
    def test_smart_response_suggestions_else(self, responses):
        with self._safe_mocks(), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('ai_knowledge.personality.azad_responses.get_help_for_task', return_value='لم أجد'), \
             patch('ai_knowledge.personality.azad_responses.search_knowledge', return_value=[]), \
             patch('ai_knowledge.personality.azad_responses.get_system_guide', return_value='guide'):
    ...
    def test_customer_balance_full_paths(self, responses):
        customer = {
            'id': 1, 'name': 'أحمد', 'customer_type': 'regular', 'phone': '050',
            'email': 'a@t.com', 'balance_aed': 6000, 'total_sales': 5, 'last_sale_date': '2025-06-01',
        }
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si, \
            ...
            si.get_customer_balance.return_value = {'success': False, 'error': 'missing'}
            assert 'missing' in responses._handle_customer_balance_query('رصيد عميل x')

    def test_customer_info_paths(self, responses):
        customer = {
            'id': 1, 'name': 'سامي', 'customer_type': 'vip', 'phone': '050',
            'email': 's@t.com', 'balance_aed': 200, 'total_sales': 3, 'last_sale_date': '2025-01-01',
        }
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si:
            si.get_customer_balance.return_value = {'success': True, 'customer': customer}
            si.get_customer_sales_summary.return_value = {
                'success': True,
                'summary': {
                    'total_sales': 3, 'total_amount': 3000, 'paid_amount': 2800, 'balance_due': 200,
    ...
    def test_product_stock_statuses(self, responses):
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si:
            for stock, alert, action in [(0, 5, 'عاجل'), (3, 10, 'قريب'), (50, 10, 'جيد')]:
                si.get_product_stock.return_value = {
                    'success': True,
                    'product': {
                        'name': 'Part', 'id': 1, 'sku': 'P1', 'category': 'X',
                        'unit_price': 25.0, 'current_stock': stock, 'alert_limit': alert,
                    },
                }
    ...
    def test_system_summary_failures(self, responses):
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si:
            ...
            si.get_financial_summary.return_value = {'success': False, 'error': 'fin fail'}
    ...
    def test_search_and_knowledge(self, responses):
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si, \
            ...
            km.get_all_sources_summary.return_value = {
                'total_categories': 2, 'total_sources': 5,
    ...
    def test_smart_sales_analysis_trends(self, responses):
        ...
        sale30 = MagicMock(amount_aed=100, status='confirmed', sale_date=datetime.now(timezone.utc) - timedelta(days=10))
        with patch('models.Sale') as MockSale:
            MockSale.sale_date = _Col()
            MockSale.status = _Col()
            MockSale.query.filter.return_value.all.side_effect = [[sale7] * 7, [sale30] * 30]
    ...
    def test_handle_detected_intent_all(self, responses, intent):
        with patch('ai_knowledge.personality.azad_responses.system_integrator') as si, \
             patch('ai_knowledge.personality.azad_responses.data_analyzer') as da, \
             patch('extensions.db') as mock_db, patch('models.Sale') as MockSale, \
             patch('services.ai_service.AIService.analyze_inventory_health', return_value={
                 'success': True, 'summary': {'total': 1, 'good': 1, 'low': 0, 'out': 0},
                 'rating': 'ok', 'health_score': 100,
             }), \
             patch('models.Supplier') as MockSup:
            si.get_customer_balance.return_value = {
                'success': True,
                'customer': {'id': 1, 'name': 'A', 'customer_type': 'r', 'phone': '', 'email': '',
                             'balance_aed': 0, 'total_sales': 0, 'last_sale_date': None},
            }
...
class TestNeuralEngineWave4:
    ...
    def test_app_context_wrappers(self, engine):
        ...
        cases = [
            ('train_maintenance_prediction', '_train_maintenance_internal', (), {'success': True}),
            ('train_accounting_assistant', '_train_accounting_internal', (), {'success': True}),
            ('train_financial_planning', '_train_financial_internal', (), {'success': True}),
            ('train_price_optimizer', '_train_price_internal', (), {'success': True}),
            ('train_sales_forecaster', '_train_sales_internal', (), {'success': True}),
            ('forecast_sales', '_forecast_sales_internal', (7,), {'forecast': [], 'total_expected': 0}),
            ('train_fraud_detector', '_train_fraud_internal', (), {'success': True}),
            ('train_inventory_optimizer', '_train_inventory_internal', (), {'success': True}),
            ('optimize_stock_level', '_optimize_stock_internal', (1,), {'urgency': 'low'}),
            ('train_demand_predictor', '_train_demand_internal', (), {'success': True}),
            ('predict_product_demand', '_predict_demand_internal', (1, 5), {'forecast': []}),
            ('train_profit_optimizer', '_train_profit_internal', (), {'success': True}),
            ('train_churn_predictor', '_train_churn_internal', (), {'success': True}),
            ('predict_cash_flow', '_predict_cash_flow_internal', (3,), {'predictions': [], 'trend': 'stable'}),
            ('predict_maintenance_needs', '_predict_maintenance_internal', (1,), {'needs_maintenance': False}),
        ]
    ...
    def test_predict_maintenance_no_product_and_else(self, engine):
        ...
        with patch.object(engine, '_predict_maintenance_internal', return_value={
            'needs_maintenance': False, 'confidence': 0.8,
            'recommended_action': 'ok', 'estimated_days': 30, 'model': 'neural_network',
        }) as inner:
    ...
    def test_price_margin_recommendations(self, engine):
        with patch.object(engine, '_load_model', return_value=True), \
             ...
             patch.object(engine.scalers['price_optimizer'], 'transform', return_value=np.array([[1.0] * 8])), \
             patch.object(engine.models['price_optimizer'], 'predict', side_effect=[np.array([105.0]), np.array([200.0])]), \
             patch.object(engine.encoders['price_optimizer'], 'transform', return_value=np.array([0])):
    ...
    def test_cash_flow_trends(self, engine):
        ...
        try:
            with patch.object(engine, '_load_model', return_value=True), \
                 ...
                 patch.object(engine.models['financial_planner'], 'predict', side_effect=[
                     np.array([5000.0]), np.array([6000.0]), np.array([7000.0]),
                 ]):
                ...
                with patch.object(engine.models['financial_planner'], 'predict', side_effect=[
                    np.array([7000.0]), np.array([5000.0]), np.array([3000.0]),
                ]):
                    dec = engine._predict_cash_flow_internal(3)
                    assert dec['trend'] == 'decreasing'
                with patch.object(engine.models['financial_planner'], 'predict', side_effect=[
                    np.array([5000.0]), np.array([5100.0]), np.array([5050.0]),
                ]):
    ...
    def test_forecast_sales_full(self, engine):
        ...
        try:
            ...
            with patch.object(engine, '_load_model', return_value=True), \
                 ...
                 patch.object(engine.models['sales_forecaster'], 'predict', side_effect=[
                     np.array([1200.0 + i * 50]) for i in range(7)
                 ]):
                chain = _db_chain(mock_db)
    ...
    def test_train_exceptions(self, engine):
        for method in ('train_accounting_assistant', 'train_sales_forecaster', 'train_inventory_optimizer',
                       'train_profit_optimizer', 'train_churn_predictor'):
            internal = method.replace('train_', '_train_').replace('_assistant', '_internal').replace('_forecaster', '_internal').replace('_optimizer', '_internal').replace('_predictor', '_internal')
...
class TestAgentsCoreWave4:
    def test_intelligent_response_branches(self, mock_ai_user):
        ...
        with patch('ai_knowledge.trainer.trainer', side_effect=RuntimeError('seed fail')), \
             patch('ai_knowledge.action_dispatcher.action_dispatcher.parse_chat_action', return_value=('create_customer', {'name': 'Ali'})), \
        ...
        with patch('ai_knowledge.action_dispatcher.action_dispatcher.parse_chat_action', return_value=('greeting', {'name': 'Ali'})), \
        ...
        with patch('ai_knowledge.action_dispatcher.action_dispatcher.parse_chat_action', return_value=None), \
        ...
        with patch('ai_knowledge.action_dispatcher.action_dispatcher.parse_chat_action', side_effect=RuntimeError('boom')), \
             patch('ai_knowledge.action_dispatcher._log_ai_error', side_effect=RuntimeError('log fail')):
    ...
    def test_get_llm_failures(self):
        from ai_knowledge import agents_core as ac
        with patch.dict('os.environ', {'GROQ_API_KEY': 'k', 'GEMINI_API_KEY': 'g'}), \
        ...
        with patch.dict('os.environ', {'GROQ_API_KEY': '', 'GEMINI_API_KEY': 'g'}), \
    ...
    def test_ask_azad_enhanced_faq_and_knowledge(self):
        ...
        knowledge = [
            {'type': 'model', 'name': 'Sale', 'info': {'table': 'sales', 'fields': {'id': 'int'}}},
            {'type': 'permission', 'code': 'manage_sales', 'info': {'name_ar': 'مبيعات', 'name': 'Sales'}},
            {'type': 'feature', 'name': 'AI', 'info': {'name_ar': 'ذكاء', 'description': 'مساعد'}},
        ]
        with patch('ai_knowledge.system_knowledge.FAQ', faq), \
    ...
    def test_ask_azad_enhanced_llm_and_brain_errors(self):
        from ai_knowledge import agents_core as ac
        with patch('ai_knowledge.system_knowledge.FAQ', {}), \
             ...
             patch('ai_knowledge.agents_core._get_llm_response', side_effect=RuntimeError('llm fail')), \
             patch('ai_knowledge.agents.master_brain.get_master_brain') as gmb, \
             patch('ai_knowledge.trainer.trainer') as tr:
        ...
        with patch('ai_knowledge.system_knowledge.search_knowledge', side_effect=RuntimeError('kb fail')), \
             patch('ai_knowledge.agents_core._check_llm_availability', return_value=False), \
             patch('ai_knowledge.agents.master_brain.get_master_brain', side_effect=RuntimeError('brain fail')), \
    ...
    def test_build_prompt_with_knowledge(self):
        ...
        knowledge = [
            {'type': 'model', 'name': 'Customer', 'info': {'table': 'customers', 'fields': {'id': 'int'}}},
            {'type': 'permission', 'code': 'view_reports', 'info': {'name_ar': 'تقارير', 'name': 'Reports'}},
            {'type': 'feature', 'name': 'backup', 'info': {'name_ar': 'نسخ', 'description': 'backup desc'}},
        ]
...
class TestActionDispatcherWave4:
    ...
    def test_helper_exceptions(self):
        ...
        with patch('ai_knowledge.action_dispatcher.current_user', SimpleNamespace(is_authenticated=True, has_permission=MagicMock(side_effect=RuntimeError()))):
    ...
    def test_dispatch_error_paths(self, permitted, mock_ai_user):
        ...
        with permitted[0], permitted[1], permitted[2], permitted[3]:
            ...
            assert action_dispatcher.dispatch('create_purchase', {'supplier_name': '', 'product_name': ''}).success is False
    ...
    def test_sales_payment_purchase_errors(self, permitted, mock_ai_user):
        ...
        with permitted[0], permitted[1], permitted[2], permitted[3], \
            ...
            assert action_dispatcher.dispatch('create_sale', {
                'customer_name': 'Ali', 'product_name': 'Bolt', 'quantity': 1,
            }).success is False
            Ex.return_value.receive_payment.side_effect = RuntimeError('pay fail')
            assert action_dispatcher.dispatch('receive_payment', {
                'customer_name': 'Ali', 'amount': 100,
            }).success is False
            ...
            assert action_dispatcher.dispatch('create_purchase', {
                'supplier_name': 'Sup', 'product_name': 'Bolt', 'quantity': 1,
            }).success is False
    ...
    def test_report_and_user_paths(self, permitted, mock_ai_user):
        ...
        with permitted[0], permitted[1], permitted[2], permitted[3], \
             patch('ai_knowledge.action_dispatcher.db.session') as session, \
             patch('models.Sale') as Sale, patch('models.SaleLine') as SaleLine, \
...
class TestSecondaryWave4:
    def test_reasoning_engine_branches(self):
        ...
        ctx = {'cost_price': 100, 'customer_type': 'merchant', 'quantity': 20, 'margin': 1.2, 'discount': 5}
        ...
        for sub in ('سعر التكلفة', 'نوع العميل', 'حجم الطلب', 'السعر النهائي', 'unknown step'):
    ...
    def test_document_generator_branches(self):
        ...
        with patch('models.Sale') as MockSale, patch('models.Customer') as MockC:
            ...
            with patch.object(DocumentGenerator, 'generate_receipt', return_value=('receipt text', 'ok')):
                content, msg = DocumentGenerator.generate_receipt(42)
                assert content == 'receipt text'
            with patch.object(DocumentGenerator, 'generate_invoice', return_value=('invoice text', 'ok')):
                content, msg = DocumentGenerator.generate_invoice(42)
                assert content == 'invoice text'
            MockC.query.get.return_value = MagicMock(
                id=1, name='Ali', customer_type='regular', phone='', email='',
    ...
    def test_knowledge_base_ecu_and_search(self):
        from ai_knowledge.knowledge_base import (
            search_parts, get_compatible_parts, get_automotive_ecu_knowledge,
        )
        hits = search_parts('محرك engine')
        assert hits or isinstance(hits, list)
        assert 'compatible' in get_compatible_parts('filter', 'Toyota').lower() or 'filter' in get_compatible_parts('filter', 'Toyota')
    ...
    def test_continuous_learner_remaining(self, knowledge_path):
        ...
        with patch.object(learner, 'learn_from_wikipedia', return_value={'success': False, 'error': 'wiki fail'}), \
             patch.object(learner, 'learn_arxiv_papers', return_value={'success': True, 'papers': 1}):
    ...
    def test_intelligent_assistant_analysis_branches(self, knowledge_path):
        ...
        sale = MagicMock(id=1, total_amount=Decimal('500'), sale_date=datetime.now(), customer=MagicMock(name='Ali'))
        mock_q = MagicMock()
        ...
        with patch('ai_knowledge.learning.quick_learner.quick_learner.get_answer', return_value=None), \
             patch('ai_knowledge.neural.semantic_matcher.understand_message', return_value={'intent': 'sales_analysis', 'confidence': 0.9}), \
             patch('models.Sale', mock_q), patch('models.Customer', mock_q), patch('models.Product', mock_q), \
             patch('utils.tenanting.get_active_tenant_id', return_value=1), \
        ...
        debt_payload = {
            ...
            'debt_analysis': {'total_debt': 6000, 'overdue_count': 2, 'unpaid_sales_count': 3},
        }
        with patch('ai_knowledge.learning.quick_learner.quick_learner.get_answer', return_value=None), \
             patch('ai_knowledge.neural.semantic_matcher.understand_message', return_value={'intent': 'customer_balance', 'confidence': 0.9}), \
             patch.object(assistant, '_collect_real_data', return_value={'customer_data': debt_payload}), \
             patch.object(assistant, '_learn_from_interaction'):
...
class TestWave4Extended:
    ...
    def test_azad_smart_response_routes(self, responses):
        customer = {
            'id': 1, 'name': 'أحمد', 'customer_type': 'regular', 'phone': '050',
            'email': 'a@t.com', 'balance_aed': 500, 'total_sales': 2, 'last_sale_date': '2025-01-01',
        }
        with patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': None, 'confidence': 0}), \
             patch('services.ai_service.AIService.is_sensitive_request', return_value=(False, False, {})), \
             patch('ai_knowledge.personality.azad_responses.azad_personality') as ap, \
            ...
            si.get_financial_summary.return_value = {
                'success': True,
                'financial': {
                    'total_sales': 1000.0, 'total_payments': 800.0, 'total_receivables': 200.0,
                    'today_sales': 50.0, 'today_payments': 30.0,
                },
            }
            si.get_product_stock.return_value = {
                'success': True,
                'product': {'name': 'F', 'id': 1, 'sku': 'S', 'category': 'C', 'unit_price': 10.0, 'current_stock': 5, 'alert_limit': 10},
            }
            si.search_data.return_value = {'success': True, 'results': {'customers': [], 'products': [], 'sales': []}}
            dg.generate_invoice.return_value = ('inv', 'ok')
            ke.search_knowledge.return_value = {'success': True, 'total_found': 1, 'results': [{'title': 'T', 'type': 'd', 'category': 'c', 'snippet': 's'}]}
            routes = [
                'رصيد balance عميل customer أحمد', 'بيانات info عميل customer سامي',
                'مخزون stock منتج product فلتر', 'ملخص summary نظام system كلي',
                'أضف add عميل customer جديد', 'ابحث search عن علي', 'أضف add موقع website مصدر source',
                'روابط links نظام system', 'مصادر sources كل all', 'أين where أجد find معلومات tax',
                'ابحث search في المعرفة knowledge tax', 'فاتورة invoice جديد new create',
                'سند receipt جديد new create', 'ولد generate فاتورة invoice 55',
                'صدر export excel بيانات data مبيعات sales', 'تقرير report مبيعات sales',
                'ولد generate تقرير report مبيعات sales', 'قانون law ضريبة tax فلسطين palestine',
                'شحن shipping قانون law إجراءات procedures', 'جودة quality معايير standards',
                'نكتة joke ضحك laugh', 'xyzunknown سؤال عام',
            ]
            for msg in routes:
        ...
        try:
            for trend, rec in [('down', 'راجع'), ('stable', 'مستقر'), ('up', 'استمر')]:
                with patch('ai_knowledge.personality.azad_responses.understand_message', return_value={'intent': None, 'confidence': 0}), \
                     ...
                     patch('extensions.db') as mock_db, \
                     patch('ai_knowledge.personality.azad_responses.SalesAnalytics.predict_next_month_sales', return_value={
                         'prediction': 3000, 'trend': trend, 'trend_value': 100, 'confidence': 0.8, 'method': 'avg',
                     }):
                    ap.is_inappropriate_message.return_value = 'normal'
                    mock_db.session.query.return_value.filter.return_value.group_by.return_value.all.return_value = [MagicMock(total=500)]
    ...
    def test_intelligent_assistant_full_analysis(self, knowledge_path):
        from ai_knowledge.agents.intelligent_assistant import IntelligentAssistant
        assistant = IntelligentAssistant()
        sale = MagicMock(id=1, total_amount=Decimal('100'), sale_date=datetime.now(), customer=MagicMock(name='Ali'))
        ...
        try:
            with patch('ai_knowledge.learning.quick_learner.quick_learner.get_answer', return_value=None), \
                 patch('utils.tenanting.get_active_tenant_id', side_effect=RuntimeError('tid')), \
                 patch('flask.has_request_context', return_value=True), \
                 patch('models.Sale', mock_q), patch('models.Customer', mock_q), patch('models.Product', mock_q), \
                 patch.object(assistant, '_learn_from_interaction'):
                um = patch('ai_knowledge.neural.semantic_matcher.understand_message')
                with um as understand:
                    understand.return_value = {'intent': 'sales_analysis', 'confidence': 0.9}
                    mock_q.all.return_value = []
                    mock_q.count.return_value = 0
                    assert assistant.process('حلل المبيعات', user_id=1)['success'] is True
                    understand.return_value = {'intent': 'sales_analysis', 'confidence': 0.9}
                    ...
                    with patch.object(assistant.neural_engine, 'predict_next_week_sales', return_value={'success': True, 'predicted_amount': 9000}, create=True):
                        assert assistant.process('حلل المبيعات', user_id=1)['success'] is True
                    understand.return_value = {'intent': 'inventory_check', 'confidence': 0.9}
                    ...
                    understand.return_value = {'intent': 'customer_balance', 'confidence': 0.9}
                    with patch.object(assistant, '_extract_entities', return_value={'names': ['Ali'], 'products': ['Bolt']}):
                        with patch.object(assistant.data_analyzer, 'analyze_customer_debt', return_value={
                            'success': True, 'debt_analysis': {'total_debt': 0, 'overdue_count': 0},
                        }):
                            assert assistant.process('رصيد Ali', user_id=1)['success'] is True
                        with patch.object(assistant.data_analyzer, 'analyze_customer_debt', return_value={
                            'success': True, 'debt_analysis': {'total_debt': 8000, 'overdue_count': 2},
                        }):
                            assert assistant.process('رصيد Ali', user_id=1)['success'] is True
        finally:
            _stop_patches(cols)
    ...
    def test_intelligent_assistant_generate_and_learn(self, knowledge_path):
        from ai_knowledge.agents.intelligent_assistant import IntelligentAssistant
        assistant = IntelligentAssistant()
        analysis = {'insights': ['i'], 'warnings': ['w'], 'recommendations': ['r'], 'predictions': ['p']}
        data = {'low_stock_products': [], 'customer_data': {'success': True, 'debt_analysis': {'total_debt': 500, 'unpaid_sales_count': 1, 'overdue_count': 0}}}
    ...
    def test_agents_core_train_failure(self):
        ...
        with patch('ai_knowledge.system_knowledge.FAQ', {}), \
             patch('ai_knowledge.system_knowledge.search_knowledge', return_value=[]), \
             patch('ai_knowledge.agents_core._check_llm_availability', return_value=False), \
             patch('ai_knowledge.agents.master_brain.get_master_brain', side_effect=RuntimeError('brain')), \
        ...
        with patch('ai_knowledge.trainer.trainer') as tr:
            ...
            with patch('ai_knowledge.action_dispatcher.action_dispatcher.parse_chat_action', return_value=('create_customer', {'name': 'Ali'})), \
                 patch('ai_knowledge.action_dispatcher.action_dispatcher.dispatch', return_value=MagicMock(success=True, message='ok', needs_permission='')):
                from ai_knowledge.agents_core import intelligent_response
    ...
    def test_document_generator_exports(self):
        ...
        sale = _sale_mock()
        customer = MagicMock(
            id=1, name='Ali', customer_type='regular', phone='050', email='a@t.com',
            get_balance_aed=lambda: Decimal('100'), created_at=datetime.now(),
        )
        product = MagicMock(
            id=1, name='Bolt', sku='B1', current_stock=10, unit_price=Decimal('5'),
            min_stock_alert=2, category=MagicMock(name='Parts'),
        )
        with patch('models.Sale') as MockSale, patch('models.Customer') as MockC, patch('models.Product') as MockP:
    ...
    def test_system_integration_paths(self):
        ...
        sale = MagicMock(id=10, total_amount=Decimal('500'), paid_amount=Decimal('200'), created_at=datetime.now())
    ...
    def test_knowledge_expansion_and_global(self, knowledge_path):
        ...
        from ai_knowledge.expansion.global_knowledge import GlobalKnowledgeConnector, GlobalExpertiseUpdater
    ...
    def test_action_dispatcher_remaining(self, mock_ai_user):
        from ai_knowledge.action_dispatcher import action_dispatcher, _get_active_tenant_id, _log_ai_error
        with patch('flask.g', create=True) as g:
            g.active_tenant_id = None
            with patch('ai_knowledge.action_dispatcher.current_user', SimpleNamespace(is_authenticated=True, tenant_id=5)):
    ...
    def test_reasoning_engine_extended(self):
        from ai_knowledge.core.reasoning_engine import ReasoningEngine
        engine = ReasoningEngine()
        fin = engine.financial_reasoning('تحليل', {'sales': 10000, 'costs': 6000, 'expenses': 1000, 'assets': 50000, 'liabilities': 20000})
    ...
    def test_analytics_and_learning_gaps(self):
        ...
        assert InventoryAnalytics.calculate_reorder_point({'avg_daily_sales': 0, 'lead_time_days': 7, 'current_stock': 0})['status'] == 'order_now'
