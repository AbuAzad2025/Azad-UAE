...
class TestTenantIsolationHardening:
    ...
    def test_developer_with_null_tenant_and_assigned_branch_does_not_raise_or_create_invalid_tenant_session(
        self, db_session, sample_tenant, sample_branch
    ):
...
class TestLoginRouteLevel:
    ...
    def test_cross_tenant_branch_mismatch_no_session(
        ...
        u = str(uuid.uuid4())[:8]
        other_tenant = Tenant(
            name=f"Other Tenant {u}",
            name_ar="Ø´Ø±ÙƒØ© Ø£Ø®Ø±Ù‰",
            slug=f"other-t-{u}",
            email=f"other_{u}@test.local",
            phone_1="0509999999",
            country="AE",
            subscription_plan="basic",
        )
        ...
        other_branch = Branch(
            ...
            name=f"Other Branch {u}",
            code=f"OTH{u[:3].upper()}",
            is_active=True,
            is_main=True,
        )
    ...
    def test_suspended_tenant_switch_never_500(
        ...
        unique = str(uuid.uuid4())[:8]
        suspended = Tenant(
            name=f"Suspended {unique}",
            name_ar="Ù…Ø¹Ù„Ù‚Ø©",
            slug=f"suspended-{unique}",
            email=f"sus_{unique}@test.local",
            phone_1="0501111111",
            country="AE",
            subscription_plan="basic",
            is_active=True,
