...
class TestEInvoiceGeneration:
    ...
    def test_tenant_vat_country_fallback(self, mocker):
        ...
        mocker.patch('models.Tenant.query', new_callable=mocker.PropertyMock, return_value=mock_tenant_q)

        strategy = MagicMock()
        strategy.generate_einvoice.return_value = {'xml_payload': '<KSA/>', 'format': 'zatca'}
    ...
    def test_missing_tenant_defaults_ae(self, mocker):
        ...
        strategy = MagicMock()
        strategy.generate_einvoice.return_value = {'xml_payload': '', 'format': 'fta_ubl_xml'}
        mock_get = mocker.patch('utils.localization.get_strategy', return_value=strategy)
