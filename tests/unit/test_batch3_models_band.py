...
class TestBankReconciliationModel:
    def test_repr_and_status_ar(self):
        from models.bank_reconciliation import BankReconciliation, BankReconciliationItem, BankStatementLine
...
class TestLoginHistoryModel:
    def test_to_dict(self):
        ...
        row = LoginHistory(username='admin', login_time=datetime(2025, 1, 1, tzinfo=timezone.utc), success=True)
        data = row.to_dict()
...
class TestPartnerDistributionAndTransaction:
    ...
    def test_transaction_labels_and_credit_debit(self):
        ...
        tx = PartnerTransaction(transaction_type='withdrawal', amount=Decimal('-50'), balance_after=Decimal('100'))
