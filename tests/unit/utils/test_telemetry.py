...
class TestMachineSignature:
    def test_builds_signature_from_platform(self):
        with patch('utils.telemetry.socket.gethostname', return_value='host'), patch(
            'utils.telemetry.platform.machine', return_value='x86'
        ), patch('utils.telemetry.platform.processor', return_value='intel'), patch(
            'utils.telemetry.platform.node', return_value='node1'
        ):
...
class TestTokenFileHelpers:
    ...
    def test_has_reported_before_handles_read_errors(self, monkeypatch):
        ...
        with patch('utils.telemetry.os.path.exists', return_value=True), patch(
            'builtins.open', side_effect=OSError('read fail')
        ):
...
class TestCollectSystemInfo:
    def test_collects_info_with_primary_ip_service(self, monkeypatch):
        ...
        with patch('utils.telemetry.socket.gethostname', return_value='srv'), patch(
            'utils.telemetry.platform.system', return_value='Windows'
        ), patch('utils.telemetry.platform.release', return_value='11'), patch(
            'utils.telemetry.platform.machine', return_value='AMD64'
        ), patch('utils.telemetry.platform.processor', return_value='cpu'), patch(
            'utils.telemetry.platform.python_version', return_value='3.12.0'
        ):
    ...
    def test_falls_back_to_secondary_ip_service(self, monkeypatch):
        ...
        with patch('utils.telemetry.socket.gethostname', return_value='srv'), patch(
            'utils.telemetry.platform.system', return_value='Linux'
        ), patch('utils.telemetry.platform.release', return_value='6'), patch(
            'utils.telemetry.platform.machine', return_value='x86_64'
        ), patch('utils.telemetry.platform.processor', return_value='cpu'), patch(
            'utils.telemetry.platform.python_version', return_value='3.12.0'
        ):
            info = collect_system_info()
...
class TestLocalLogAndSubmit:
    ...
    def test_send_formsubmit_success_and_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(telemetry_module.requests, 'post', MagicMock(side_effect=RuntimeError('net')))
...
class TestHeartbeatAndStart:
    def test_send_heartbeat_skips_when_already_reported(self, monkeypatch):
        ...
        with patch('utils.telemetry.collect_system_info') as collect, patch(
            'utils.telemetry.send_formsubmit'
        ) as submit:
            send_heartbeat()
    ...
    def test_send_heartbeat_first_run_marks_success(self, monkeypatch):
        ...
        with patch('utils.telemetry.save_local_log') as save_log, patch(
            'utils.telemetry.send_formsubmit', return_value=True
        ) as submit, patch('utils.telemetry.mark_as_reported') as mark:
            send_heartbeat()
