...
class TestSetupEnhancedLogging:
    def test_configures_handlers_and_teardown(self, tmp_path, monkeypatch):
        ...
        with patch('utils.enhanced_logging.RotatingFileHandler', side_effect=lambda *a, **k: make_handler()):
...
class TestSecurityLogger:
    def test_security_events_emit_logs(self):
        with patch('utils.enhanced_logging.logging.warning') as warning, patch(
            'utils.enhanced_logging.logging.info'
        ) as info:
...
class TestPerformanceLogger:
    def test_slow_query_cache_logs(self):
        with patch('utils.enhanced_logging.logging.warning') as warning, patch(
            'utils.enhanced_logging.logging.debug'
        ) as debug:
