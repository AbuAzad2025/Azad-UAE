...
class TestAdaptiveRateLimit:
    ...
    def test_super_admin_and_manager_limits(self, flask_app):
        ...
        with flask_app.test_request_context("/api/adaptive"):
            with (
                patch("utils.rate_limiter_enhanced.current_user", super_admin),
                patch("utils.rate_limiter_enhanced.cache.get", return_value=50),
    ...
    def test_authenticated_user_blocks_at_base_limit(self, flask_app):
        with flask_app.test_request_context("/api/adaptive"):
            with (
                patch("utils.rate_limiter_enhanced.current_user", _regular_user()),
                patch("utils.rate_limiter_enhanced.cache.get", return_value=10),
            ):
                body, status = flask_app.view_functions["adaptive_endpoint"]()
        assert status == 429
