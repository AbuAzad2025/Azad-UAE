...
class TestRedisCacheBasics:
    def test_get_returns_value(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.cache.get', return_value='hit'):
            assert RedisCache.get('k') == 'hit'

    def test_get_logs_and_returns_none_on_error(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.cache.get', side_effect=RuntimeError('down')):
    ...
    def test_set_custom_timeout_and_failure(self, flask_app):
        ...
        with flask_app.app_context(), patch('utils.redis_cache.cache.set', side_effect=OSError('fail')):
            assert RedisCache.set('k', 1) is False

    def test_delete_success_and_failure(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.cache.delete') as delete_fn:
            assert RedisCache.delete('k') is True
        delete_fn.assert_called_once_with('k')

        with flask_app.app_context(), patch('utils.redis_cache.cache.delete', side_effect=OSError('fail')):
...
class TestCachedDecorator:
    def test_cache_hit_skips_function(self, flask_app):
        ...
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.get', return_value='cached'):
    ...
    def test_cache_miss_stores_result_with_args(self, flask_app):
        ...
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.get', return_value=None) as get_fn, patch(
            'utils.redis_cache.RedisCache.set', return_value=True
        ) as set_fn:
...
class TestModelAndQueryHelpers:
    def test_model_cache_roundtrip_and_invalidation(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.set') as set_fn, patch(
            'utils.redis_cache.RedisCache.get', return_value={'id': 1}
        ):
        ...
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.delete') as delete_fn:
            invalidate_model_cache('Customer', 1)
        delete_fn.assert_called_once_with('model:Customer:1')

        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.delete_pattern') as pattern_fn:
    ...
    def test_query_cache_roundtrip(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.set') as set_fn, patch(
            'utils.redis_cache.RedisCache.get', return_value=[1, 2]
        ):
...
class TestDomainCacheHelpers:
    def test_customer_product_dashboard_caches(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.set') as set_fn, patch(
            'utils.redis_cache.RedisCache.get', side_effect=[100.5, 12, {'sales': 1}]
        ):
    ...
    def test_invalidation_helpers(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.delete') as delete_fn, patch(
            'utils.redis_cache.RedisCache.delete_pattern'
        ) as pattern_fn:
...
class TestRateLimitCheck:
    def test_allows_requests_and_sets_expiry_on_first_hit(self, flask_app, redis_client):
        ...
        mock_cache = MagicMock()
        mock_cache.cache = cache_backend
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.increment', return_value=1), patch(
            'utils.redis_cache.cache', mock_cache
        ):
    ...
    def test_blocks_when_limit_exceeded(self, flask_app, redis_client):
        ...
        mock_cache = MagicMock()
        mock_cache.cache = cache_backend
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.increment', return_value=11), patch(
            'utils.redis_cache.cache', mock_cache
        ):
    ...
    def test_fails_open_on_error(self, flask_app):
        with flask_app.app_context(), patch('utils.redis_cache.RedisCache.increment', side_effect=RuntimeError('redis')):
