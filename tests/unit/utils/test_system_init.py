...
class TestEnsureSystemIntegrity:
    def test_ensure_system_integrity_wraps_inner_call(self, flask_app):
        with patch('utils.system_init._ensure_system_integrity_inner') as inner, patch(
            'utils.tenanting.without_tenant_scope', return_value=_tenant_scope()
        ):
...
class TestRoleAndPermissionBootstrap:
    def test_ensure_permissions_creates_missing_codes(self, flask_app):
        ...
        with flask_app.app_context(), patch.object(
            system_init_module.Permission, 'query', query
        ), patch('utils.system_init.db.session') as session, patch(
            'utils.constants.PERMISSION_CODES', ['manage_sales']
        ), patch(
            'utils.constants.PERMISSIONS', {'manage_sales': {'en': 'Sales', 'ar': 'مبيعات'}}
        ):
    ...
    def test_ensure_owner_role_creates_and_assigns_permissions(self, flask_app):
        ...
        with flask_app.app_context(), patch.object(
            system_init_module.Role, 'query', role_query
        ), patch.object(system_init_module.Permission, 'query', perm_query), patch(
...
class TestOwnerUserBootstrap:
    ...
    def test_updates_owner_role_linkage(self, flask_app):
        ...
        owner = MagicMock(username='owner', is_owner=True, role=other_role, email='owner@system.local', id=1)
        user_query = MagicMock()
        user_query.filter_by.return_value.first.return_value = owner
        with flask_app.app_context(), patch.object(
            system_init_module.User, 'query', user_query
...
class TestTenantGlTrees:
    def test_ensure_tenant_gl_trees_provisions_accounts(self, flask_app):
        ...
        with flask_app.app_context(), patch.object(Tenant, 'query', tenant_query), patch(
            'services.gl_service.GLService.ensure_core_accounts'
        ) as ensure:
...
class TestRecordServerActivation:
    def test_records_first_activation_alert(self, flask_app):
        ...
        with flask_app.app_context(), patch(
            'models.SystemSettings.get_current', return_value=settings
        ), patch('utils.telemetry.get_machine_signature', return_value='sig-1'), patch(
            'socket.gethostname', return_value='host'
        ), patch('platform.system', return_value='Windows'), patch(
            'platform.release', return_value='11'
        ), patch('platform.machine', return_value='AMD64'), patch(
            'platform.processor', return_value='cpu'
        ), patch('utils.system_init.db.session') as session, patch.dict(
            'os.environ', {'DISABLE_TELEMETRY': 'false'}, clear=False
    ...
    def test_records_server_changed_alert(self, flask_app):
        ...
        with flask_app.app_context(), patch(
            'models.SystemSettings.get_current', return_value=settings
        ), patch('utils.telemetry.get_machine_signature', return_value='new-sig'), patch(
            'socket.gethostname', return_value='host'
        ), patch('platform.system', return_value='Linux'), patch(
            'platform.release', return_value='6'
        ), patch('platform.machine', return_value='x86'), patch(
            'platform.processor', return_value='cpu'
        ), patch('utils.system_init.db.session'):
    ...
    def test_skips_when_signature_unchanged(self, flask_app):
        ...
        with flask_app.app_context(), patch(
            'models.SystemSettings.get_current', return_value=settings
        ), patch('utils.telemetry.get_machine_signature', return_value='same-sig'), patch(
            'utils.system_init.db.session'
        ) as session:
    ...
    def test_sends_activation_email_when_mail_configured(self, flask_app, monkeypatch):
        ...
        with flask_app.app_context(), patch(
            'models.SystemSettings.get_current', return_value=settings
        ), patch('utils.telemetry.get_machine_signature', return_value='sig-mail'), patch(
            'socket.gethostname', return_value='host'
        ), patch('platform.system', return_value='Windows'), patch(
            'platform.release', return_value='11'
        ), patch('platform.machine', return_value='AMD64'), patch(
            'platform.processor', return_value='cpu'
        ), patch('utils.system_init.db.session'), patch('flask_mail.Message'), patch(
    ...
    def test_skips_mail_when_telemetry_disabled(self, flask_app, monkeypatch):
        ...
        with flask_app.app_context(), patch(
            'models.SystemSettings.get_current', return_value=settings
        ), patch('utils.telemetry.get_machine_signature', return_value='sig-skip'), patch(
            'socket.gethostname', return_value='host'
        ), patch('platform.system', return_value='Windows'), patch(
            'platform.release', return_value='11'
        ), patch('platform.machine', return_value='AMD64'), patch(
            'platform.processor', return_value='cpu'
        ), patch('utils.system_init.db.session'), patch('extensions.mail.send') as send_mail:
            system_init_module._record_server_activation(owner, True)
    ...
    def test_returns_before_mail_when_smtp_not_configured(self, flask_app, monkeypatch):
        ...
        with flask_app.app_context(), patch(
            'models.SystemSettings.get_current', return_value=settings
        ), patch('utils.telemetry.get_machine_signature', return_value='sig-no-mail'), patch(
            'socket.gethostname', return_value='host'
        ), patch('platform.system', return_value='Windows'), patch(
            'platform.release', return_value='11'
        ), patch('platform.machine', return_value='AMD64'), patch(
