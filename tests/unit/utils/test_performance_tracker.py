...
from utils.performance_tracker import PerformanceContext, log_slow_queries, track_performance
...
class TestTrackPerformance:
    def test_slow_operation_logs_warning(self):
        ...
        with patch('utils.performance_tracker.time.time', side_effect=[0.0, 0.25, 0.25]), patch(
            'utils.performance_tracker.logger.warning'
        ) as warning:
            assert work() == 'x'
    ...
    def test_fast_operation_logs_debug(self):
        ...
        with patch('utils.performance_tracker.time.time', side_effect=[1.0, 1.01, 1.01]), patch(
            'utils.performance_tracker.logger.debug'
        ) as debug:
            assert work() == 1
...
class TestPerformanceContext:
    def test_context_manager_logs_elapsed(self):
        with patch('utils.performance_tracker.time.time', side_effect=[0.0, 0.05, 0.05]), patch(
            'utils.performance_tracker.logger.info'
        ) as info:
...
class TestLogSlowQueries:
    def test_registers_and_warns_on_slow_query(self):
        ...
        with patch('utils.performance_tracker.time.time', side_effect=[0.0, 0.2, 0.2]), patch(
            'utils.performance_tracker.logger.warning'
        ) as warning:
