...
class TestPerformanceMonitor:
    ...
    def test_monitor_endpoint_success_and_error(self, flask_app):
        ...
        with flask_app.app_context(), patch('utils.monitoring.time.time', side_effect=[0.0, 0.1]):
            assert ok() == 'done'
        ...
        with flask_app.app_context(), patch('utils.monitoring.time.time', side_effect=[0.0, 0.2]):
...
class TestErrorLogger:
    def test_log_error_with_context_and_audit_failure(self, flask_app):
        ...
        with flask_app.test_request_context('/broken', method='POST'):
            g.request_id = 'rid-9'
            g.user = 'alice'
            with patch('utils.monitoring.db.session', session), patch(
                'models.audit.AuditLog', side_effect=RuntimeError('audit down')
            ):
    ...
    def test_log_error_persists_audit_entry(self, flask_app):
        ...
        with flask_app.test_request_context('/save', method='PUT'):
            with patch('utils.monitoring.db.session', session), patch(
                'models.audit.AuditLog', return_value=audit_entry
            ):
...
class TestHealthCheck:
    ...
    def test_get_health_status_overall(self):
        with patch.object(HealthCheck, 'check_database', return_value={'status': 'healthy'}), patch.object(
        ...
        with patch.object(HealthCheck, 'check_database', return_value={'status': 'unhealthy'}), patch.object(
            HealthCheck, 'check_redis', return_value={'status': 'healthy'}
        ), patch.object(HealthCheck, 'check_disk_space', return_value={'status': 'healthy'}):
...
class TestSetupAdvancedLogging:
    ...
    def test_metrics_route_requires_owner(self, flask_app, tmp_path):
        ...
        owner.is_owner = True
        with patch.object(
            HealthCheck, 'get_health_status', return_value={'status': 'unhealthy', 'checks': {}}
