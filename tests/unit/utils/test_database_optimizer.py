...
class TestVacuumPostgres:
    def test_vacuums_postgresql_database(self):
        ...
        with (
            patch("utils.database_optimizer.db", mock_db),
            patch("utils.database_optimizer.logger") as logger,
        ):
    ...
    def test_vacuum_failure_returns_error(self):
        ...
        with (
            patch("utils.database_optimizer.db", mock_db),
            patch("utils.database_optimizer.logger"),
        ):
            result = DatabaseOptimizer.vacuum_postgres()
    ...
    def test_runs_all_optimization_steps(self):
        with (
            ...
            patch.object(
                DatabaseOptimizer,
                "get_table_sizes",
                return_value={"success": True, "tables": []},
            ),
        ):
