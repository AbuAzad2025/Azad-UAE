...
class TestMeasureTime:
    ...
    def test_logs_warning_for_slow_function(self):
        ...
        with patch('utils.performance.time.time', side_effect=self._time_sequence(0.0, 1.5)), patch(
            'utils.performance.logger.warning'
        ) as warning:
            assert slow() == 'done'
    ...
    def test_logs_info_for_medium_duration(self):
        ...
        with patch('utils.performance.time.time', side_effect=self._time_sequence(0.0, 0.6)), patch(
            'utils.performance.logger.info'
        ) as info:
            assert medium() == 1
    ...
    def test_fast_function_returns_without_warning(self):
        ...
        with patch('utils.performance.time.time', side_effect=self._time_sequence(1.0, 1.1)), patch(
            'utils.performance.logger.warning'
        ) as warning, patch('utils.performance.logger.info') as info:
            assert fast() == 'ok'
...
class TestCacheResult:
    def test_cache_hit_returns_cached_value(self):
        ...
        with patch('extensions.cache.get', return_value=10), patch('utils.performance.logger.debug') as debug:
            assert compute(5) == 10
    ...
    def test_cache_miss_sets_value(self):
        ...
        with patch('extensions.cache.get', return_value=None), patch('extensions.cache.set') as set_fn, patch(
            'utils.performance.logger.debug'
        ) as debug:
            assert compute(4) == 5
...
class TestPerformanceMonitor:
    ...
    def test_end_request_warns_on_slow_request(self, flask_app):
        with flask_app.app_context():
            ...
            response = MagicMock()
            response.headers = {}
            with patch('utils.performance.time.time', return_value=3.0), patch(
                'utils.performance.logger.warning'
            ) as warning:
    ...
    def test_start_request_sets_timer(self, flask_app):
        with flask_app.test_request_context(), patch('utils.performance.time.time', return_value=12.5):
