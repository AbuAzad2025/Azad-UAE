...
class TestSanitizeHtml:
    ...
    def test_allow_tags_uses_bleach_when_available(self):
        ...
        with patch.object(sanitizer_module, '_BLEACH_AVAILABLE', True), patch.object(
            sanitizer_module, 'bleach', bleach_mock
        ):
...
class TestSanitizeFormData:
    def test_applies_per_field_rules(self):
        with patch.object(InputSanitizer, 'sanitize_email', return_value='a@b.co') as email_fn, patch.object(
            InputSanitizer, 'sanitize_phone', return_value='+971501234567'
        ), patch.object(InputSanitizer, 'sanitize_number', return_value=10), patch.object(
            InputSanitizer, 'sanitize_html', return_value=Markup('<b>x</b>')
        ) as html_fn, patch.object(InputSanitizer, 'sanitize_text', return_value='plain') as text_fn:
            form_data = {
                'email': 'a@b.co',
                'phone': '+971',
                'qty': '10',
