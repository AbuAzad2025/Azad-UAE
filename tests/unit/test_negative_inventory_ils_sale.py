...
from models import (
    ...
    Sale, SaleLine, GLAccount, GLJournalEntry, GLJournalLine,
...
class TestNegativeInventoryILSSale:
    ...
    def test_sale_with_zero_stock_negative_inventory_allowed(
        self, app, db_session,
        ils_tenant, ils_branch, negative_warehouse,
        ils_customer, ils_seller, ils_product_zero_stock,
        ils_currency_setup, ils_coa,
    ):
        """
        ...
        with app.app_context():
            ...
            gl_accounts = GLAccount.query.filter_by(
                tenant_id=ils_tenant.id
            ).all()
            ...
            assert cogs_dr[0].debit > 0, \
                f"COGS should be > 0: got {cogs_dr[0].debit}"
