...
class TestPrintService:
    ...
    def test_render_print_basic(self, mock_render):
        from services.print_service import PrintService
        with patch.object(PrintService, '_get_tenant_context', return_value={'tenant': None, 'settings': None, 'company': None, 'print_branding': {}, 'print_tenant_id': 1}):
            with patch.object(PrintService, '_user_context', return_value={'print_user_name': 'test', 'print_user_id': 1}):
    ...
    @patch('services.print_service.PrintService.render_print', side_effect=['<html>page-1</html>', '<html>page-2</html>'])
...
class TestUserService:
    ...
    def test_get_users_list_context(self):
        from services.user_service import UserService
        with patch('services.user_service.Role') as mock_role, \
...
class TestRoleService:
    ...
    def test_get_roles_permissions_context_empty(self):
        from services.role_service import RoleService
        with patch('services.role_service.Role') as mock_role, \
    ...
    def test_create_payment_for_sale_needs_cheque_number(self):
        ...
        with pytest.raises(ValueError, match='رقم الشيك مطلوب'):
            SaleService.create_payment_for_sale(
                sale, amount=100, payment_method='cheque',
                cheque_number=None, cheque_date='2025-06-01', bank_name='Test Bank',
            )
