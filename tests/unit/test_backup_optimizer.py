...
class TestCompressBackup:
    def test_compresses_backup_and_returns_ratio(self, tmp_path):
        ...
        with patch('utils.backup_optimizer.os.path.getsize', side_effect=[100, 40]), patch(
            'utils.backup_optimizer.os.remove'
        ) as remove, patch('utils.backup_optimizer.logger'):
...
class TestCleanupOldBackups:
    def test_deletes_backups_beyond_keep_count(self, tmp_path):
        ...
        with patch('utils.backup_optimizer.os.path.getmtime', side_effect=[3, 2, 1]), patch(
            'utils.backup_optimizer.os.remove'
        ) as remove, patch('utils.backup_optimizer.logger'):
    ...
    def test_cleanup_failure_returns_error(self):
        with patch('utils.backup_optimizer.Path.glob', side_effect=RuntimeError('glob fail')), patch(
            'utils.backup_optimizer.logger'
        ):
...
class TestVerifyBackup:
    ...
    def test_verification_failure_returns_false(self):
        with patch('builtins.open', side_effect=OSError('missing')), patch(
            'utils.backup_optimizer.logger'
        ):
...
class TestGetBackupInfo:
    def test_returns_sorted_backup_metadata(self, tmp_path):
        ...
        with patch('utils.backup_optimizer.Path.glob', return_value=[first, second]), patch(
