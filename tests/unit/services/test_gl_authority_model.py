...
class TestProvisioningScope:
    ...
    def test_provision_creates_no_liquidity_record_nonposting_mappings(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"PrvScope1-{uuid.uuid4().hex[:6]}",
                name_ar="بروف سكوب",
                name_en="PrvScope1",
                slug=f"prv-scope-{uuid.uuid4().hex[:6]}",
    ...
    def test_no_mapping_targets_header_inactive_foreign(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"PrvScope2-{uuid.uuid4().hex[:6]}",
                name_ar="بروف سكوب 2",
                name_en="PrvScope2",
                slug=f"prv-scope-2-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
...
class TestRecordMode:
    ...
    def test_depreciation_posts_to_exact_accounts(self, app):
        ...
        with app.app_context():
            ...
            tenant = Tenant(
                ...
                name_ar="ريك دبر",
                name_en="RecDepr",
                slug=f"rec-depr-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_disposal_posts_to_exact_asset_account(self, app):
        ...
        with app.app_context():
            ...
            tenant = Tenant(
                ...
                name_ar="ريك دس",
                name_en="RecDis",
                slug=f"rec-dis-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_record_requires_explicit_account_allowed(self, app):
        ...
        with app.app_context():
            ...
            tenant = Tenant(
                name=f"RecReq-{uuid.uuid4().hex[:6]}",
                name_ar="ريك ريك",
                name_en="RecReq",
                slug=f"rec-req-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
...
class TestMappingMode:
    ...
    def test_gain_loss_mappings_exist_and_are_valid(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"MapGL-{uuid.uuid4().hex[:6]}",
                name_ar="ماب جل",
                name_en="MapGL",
                slug=f"map-gl-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_gain_loss_resolves_through_resolver(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"MapGL2-{uuid.uuid4().hex[:6]}",
                name_ar="ماب جل 2",
                name_en="MapGL2",
                slug=f"map-gl-2-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
...
class TestStaleMappings:
    ...
    def test_stale_mappings_are_warnings_not_errors(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"Stale1-{uuid.uuid4().hex[:6]}",
                name_ar="ستال 1",
                name_en="Stale1",
                slug=f"stale-1-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_provisioner_rejects_header_target_mapping(self, app, monkeypatch):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"HdrTgt-{uuid.uuid4().hex[:6]}",
                name_ar="هدر هدف",
                name_en="HdrTgt",
                slug=f"hdr-tgt-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
...
class TestTenantBranchIsolation:
    ...
    def test_entry_branch_from_another_tenant_is_rejected(self, app):
        ...
        with app.app_context():
            # Create two tenants
            tenant1 = Tenant(
                name=f"Tenant1-{uuid.uuid4().hex[:6]}",
                name_ar=" tenant1",
                name_en="Tenant1",
                slug=f"tenant1-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            tenant2 = Tenant(
                name=f"Tenant2-{uuid.uuid4().hex[:6]}",
                name_ar=" tenant2",
                name_en="Tenant2",
...
class TestAuthorityModelEdgeCases:
    ...
    def test_ar_concept_code_case_insensitive(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"ArCase-{uuid.uuid4().hex[:6]}",
                name_ar="ar case",
                name_en="ArCase",
                slug=f"arcase-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_unknown_concept_raises_glmappingerror(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"UnkConcept-{uuid.uuid4().hex[:6]}",
                name_ar="unk",
                name_en="UnkConcept",
                slug=f"unkconcept-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_branch_a_cash_rejected_when_resolved_with_branch_b(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"LiqBranch-{uuid.uuid4().hex[:6]}",
                name_ar="liq",
                name_en="LiqBranch",
                slug=f"liqbranch-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_branch_a_bank_rejected_when_resolved_with_branch_b(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"LiqBranchB-{uuid.uuid4().hex[:6]}",
                name_ar="liq",
                name_en="LiqBranchB",
                slug=f"liqbranchb-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_landed_cost_raises_with_dynamic_mapping_disabled(self, app, monkeypatch):
        ...
        with app.app_context():
            ...
            tenant = Tenant(
                name=f"LcDisabled-{uuid.uuid4().hex[:6]}",
                name_ar="lc",
                name_en="LcDisabled",
                slug=f"lcdisabled-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
    ...
    def test_cash_bank_readiness_after_build(self, app):
        ...
        with app.app_context():
            tenant = Tenant(
                name=f"ReadyAfter-{uuid.uuid4().hex[:6]}",
                name_ar="ready",
                name_en="ReadyAfter",
                slug=f"readyafter-{uuid.uuid4().hex[:6]}",
                default_currency="AED",
            )
            db.session.add(tenant)
            db.session.flush()
