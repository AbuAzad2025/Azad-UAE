...
def _balanced_lines(amount=Decimal('100')):
    return [
        {'account': '1111', 'debit': amount, 'credit': Decimal('0'), 'description': 'cash'},
        {'account': '4101', 'debit': Decimal('0'), 'credit': amount, 'description': 'revenue'},
    ]
...
class TestResolveJournalLineAccount:
    ...
    def test_missing_ok_returns_none(self, sample_tenant):
        result = GLService._resolve_journal_line_account(
            {'account': '9999999'}, sample_tenant.id, ensure_core=False, missing_ok=True,
        )
...
class TestCreateJournalEntry:
    ...
    def test_wrong_branch_tenant_raises(self, db_session, sample_tenant, sample_branch, sample_gl_accounts, mocker):
        ...
        other = Tenant(
            name='Other', name_ar='Other', slug='other-gl-test',
            email='other@test.com', country='AE', is_active=True,
        )
...
class TestReconciliationCheck:
    def test_reconciliation_returns_structure(self, db_session, sample_tenant, sample_gl_accounts, sample_customer, sample_supplier):
...
class TestLiquidityEdgeCases:
    def test_multiple_cash_accounts_requires_branch(self, db_session, sample_tenant, sample_gl_accounts):
        cash_accounts = GLAccount.query.filter_by(
            tenant_id=sample_tenant.id, liquidity_kind='cash', is_active=True, is_header=False,
        ).all()
    ...
    def test_payment_credit_bank_path(self, db_session, sample_tenant, sample_branch, sample_gl_accounts, mocker):
        mocker.patch('services.gl_service.is_dynamic_gl_mapping_enabled', return_value=False)
        code = GLService.get_payment_credit_account('bank_transfer', tenant_id=sample_tenant.id, branch_id=sample_branch.id)
        assert code is not None
...
class TestEnsureCoreLogging:
    def test_ensure_core_accounts_logs_changes(self, db_session, sample_tenant, mocker):
        mocker.patch('services.gl_service.GLTreeBuilder.build', return_value={
            'created': [{'code': '9999'}], 'updated': [], 'converted': [], 'deactivated': [],
        })
