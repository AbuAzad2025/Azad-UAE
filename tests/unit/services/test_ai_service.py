...
class TestSensitiveRequest:
    def test_non_sensitive_message(self):
        ...
        assert AIService.is_sensitive_request("ما سعر المنتج؟", user) == (
            False,
            False,
            None,
        )
...
class TestChatResponse:
    ...
    def test_groq_success(self, mocker, monkeypatch):
        ...
        mocker.patch("ai_knowledge.agents_core.ask_azad_enhanced", return_value=None)
        mocker.patch(
            "services.ai_service.AIService._gather_relevant_knowledge",
            return_value="stats",
        )
        resp = MagicMock()
...
class TestExecuteAiAction:
    ...
    def test_all_action_types(self, mocker):
        ...
        actions = [
            ...
            (
                "create_sale",
                {"products": [{"name": "p", "quantity": 1}], "اسم_العميل": "c"},
            ),
...
class TestMiscHelpers:
    ...
    def test_predict_cash_flow(self, mocker):
        mocker.patch(
            "services.ai_service.AIService.predict_cash_flow_neural",
...
@pytest.mark.parametrize(
    "method,path,cls_name",
    [
        (
            "get_learning_system",
            "ai_knowledge.core.learning_system",
            "AzadLearningSystem",
        ),
        ("get_context_engine", "ai_knowledge.core.context_engine", "ContextEngine"),
        (
            "get_personality",
            "ai_knowledge.personality.azad_personality",
            "AzadPersonality",
        ),
        ("get_dialect_manager", "ai_knowledge.personality.dialects", "DialectManager"),
        (
            "get_security_rules",
            "ai_knowledge.specialized.security_rules",
            "SecurityRules",
        ),
        ("get_neural_engine", "ai_knowledge.neural.neural_engine", "get_neural_engine"),
        (
            "get_reasoning_engine",
            "ai_knowledge.core.reasoning_engine",
            "get_reasoning_engine",
        ),
        ("get_memory_system", "ai_knowledge.core.memory_system", "get_memory_system"),
        (
            "get_code_generator",
            "ai_knowledge.generation.code_generator",
            "get_code_generator",
        ),
        (
            "get_agent_coordinator",
            "ai_knowledge.agents.multi_agent_system",
            "get_agent_coordinator",
        ),
        (
            "get_reflection_engine",
            "ai_knowledge.improvement.self_reflection",
            "get_reflection_engine",
        ),
        (
            "get_conversation_manager",
            "ai_knowledge.core.conversation_manager",
            "get_conversation_manager",
        ),
        (
            "get_vision_processor",
            "ai_knowledge.neural.vision_processor",
            "get_vision_processor",
        ),
        ("get_master_brain", "ai_knowledge.agents.master_brain", "get_master_brain"),
        (
            "get_transformers_brain",
            "ai_knowledge.neural.transformers_brain",
            "get_transformers_brain",
        ),
    ],
)
...
class TestNeuralAndAdvanced:
    ...
    def test_predict_price_fallback(self, mocker):
        mocker.patch(
            "services.ai_service.AIService.get_neural_engine",
            side_effect=RuntimeError(),
        )
        mocker.patch(
            "services.ai_service.AIService.recommend_price",
            return_value={"recommended_price": 1},
        )
    ...
    def test_advanced_errors(self, mocker):
        mocker.patch(
            "services.ai_service.AIService.get_reasoning_engine",
            side_effect=RuntimeError(),
        )
        mocker.patch(
            "services.ai_service.AIService.get_memory_system",
            side_effect=RuntimeError(),
        )
    ...
    def test_transformers_errors(self, mocker):
        mocker.patch(
            "services.ai_service.AIService.get_transformers_brain",
            side_effect=RuntimeError(),
        )
    ...
    def test_ecu_and_external(self, mocker):
        ...
        mocker.patch(
            "ai_knowledge.learning.external_learning.get_external_learning",
    ...
    def test_expand_and_integrate_mocked(self, mocker):
        ...
        mocker.patch(
            "ai_knowledge.core.system_integration.SystemIntegrator",
            return_value=integrator,
        )
        connector = MagicMock(fetch_knowledge=MagicMock(return_value={"data": 1}))
        mocker.patch(
            "ai_knowledge.expansion.global_knowledge.GlobalKnowledgeConnector",
            return_value=connector,
        )
        beginners = MagicMock(get_help=MagicMock(return_value="help"))
        mocker.patch(
            "ai_knowledge.personality.beginners_mode.BeginnersGuide",
            return_value=beginners,
        )
        doc_gen = MagicMock(generate=MagicMock(return_value="doc"))
        mocker.patch(
            "ai_knowledge.generation.document_generator.DocumentGenerator",
            return_value=doc_gen,
        )
...
class TestCoverageGaps:
    ...
    def test_wrapper_error_paths(self, mocker):
        ...
        mocker.patch(
            "services.ai_service.AdvancedLaws.get_customs_info",
            side_effect=RuntimeError(),
        )
        assert AIService.get_advanced_law_info("customs") == {}
    ...
    def test_advanced_memory_chat_errors(self, mocker):
        ...
        mocker.patch(
            "services.ai_service.AIService.get_conversation_manager",
            side_effect=RuntimeError(),
        )
        mocker.patch(
            "services.ai_service.AIService.get_reflection_engine",
            side_effect=RuntimeError(),
        )
        mocker.patch(
            "services.ai_service.AIService.get_vision_processor",
            side_effect=RuntimeError(),
        )
        mocker.patch(
            "services.ai_service.AIService.get_code_generator",
            side_effect=RuntimeError(),
        )
    ...
    def test_security_compliance_error(self, mocker):
        mocker.patch(
            "services.ai_service.AIService.get_security_rules",
            side_effect=RuntimeError(),
        )
    ...
    def test_ecu_errors(self, mocker):
        mocker.patch(
            "services.ai_service.get_automotive_ecu_knowledge",
            side_effect=RuntimeError("x"),
        )
    ...
    def test_remaining_wrapper_errors(self, mocker):
        mocker.patch(
            "services.ai_service.get_customer_service_tip",
            side_effect=RuntimeError("x"),
        )
    ...
    def test_delegate_and_codegen_errors(self, mocker):
        mocker.patch(
            "services.ai_service.AIService.get_agent_coordinator",
            side_effect=RuntimeError(),
        )
