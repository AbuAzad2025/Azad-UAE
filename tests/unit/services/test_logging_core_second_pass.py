...
class TestFormattersSecondPass:
    ...
    def test_color_formatter_encoding_fallback(self, mocker):
        ...
        mocker.patch('services.logging_core.sys.stdout', MagicMock(encoding='invalid-charset-xyz'))
        text = _ColorFormatter().format(record)
...
class TestTraceAndErrorSecondPass:
    ...
    def test_persist_error_config_exception(self, app, mocker):
        ...
        mocker.patch('services.logging_core.current_app.config.get', side_effect=RuntimeError('no app'))
        conn = MagicMock()
    ...
    def test_persist_error_frontend_url_endpoint(self, mocker):
        ...
        mocker.patch('extensions.db.engine.connect', return_value=ctx)
        row = LoggingCore._persist_error(
            'fe', category='FRONTEND', level='ERROR', source='browser',
            url='https://example.com/page?q=1', extra={'fingerprint_key': 'fp-key'},
        )
        assert row == 16
...
class TestErrorQueriesSecondPass:
    def test_get_error_logs_resolved_filter(self, mocker):
        ...
        E.query = q
        mocker.patch('extensions.db.session.query').return_value.distinct.return_value.order_by.return_value.all.return_value = []
...
class TestHealthSecondPass:
    ...
    def test_get_disk_usage_success_and_error(self, mocker):
        ...
        mocker.patch.dict('sys.modules', {'psutil': MagicMock(disk_usage=MagicMock(side_effect=RuntimeError()))})
    ...
    def test_get_memory_usage_success_and_error(self, mocker):
        ...
        mocker.patch.dict('sys.modules', {'psutil': MagicMock(virtual_memory=MagicMock(side_effect=RuntimeError()))})
    ...
    def test_get_cpu_usage_success_and_error(self, mocker):
        ...
        mocker.patch.dict('sys.modules', {'psutil': MagicMock(cpu_percent=MagicMock(side_effect=RuntimeError()))})
