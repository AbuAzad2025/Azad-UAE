...
class TestCheckNowpayments:
    ...
    def test_fully_configured(self, mocker):
        ...
        mocker.patch('services.health_service.PaymentVault.get_platform_vault', return_value=vault)
        result = HealthCheckService.check_nowpayments()
        assert result['status'] == 'healthy'
...
class TestCheckSystemResources:
    def test_healthy_resources(self, mocker):
        ...
        mocker.patch('services.health_service.psutil.virtual_memory', return_value=MagicMock(percent=20.0))
        mocker.patch('services.health_service.psutil.disk_usage', return_value=MagicMock(percent=30.0))
    ...
    def test_high_memory_warning(self, mocker):
        mocker.patch('services.health_service.psutil.cpu_percent', return_value=10.0)
        mocker.patch('services.health_service.psutil.virtual_memory', return_value=MagicMock(percent=95.0))
        mocker.patch('services.health_service.psutil.disk_usage', return_value=MagicMock(percent=30.0))
        result = HealthCheckService.check_system_resources()
        assert result['status'] == 'warning'
    ...
    def test_low_disk_warning(self, mocker):
        mocker.patch('services.health_service.psutil.cpu_percent', return_value=10.0)
        mocker.patch('services.health_service.psutil.virtual_memory', return_value=MagicMock(percent=20.0))
        mocker.patch('services.health_service.psutil.disk_usage', return_value=MagicMock(percent=95.0))
        result = HealthCheckService.check_system_resources()
        assert result['status'] == 'warning'
...
class TestRunFullHealthCheck:
    def test_overall_healthy(self, mocker):
        ...
        mocker.patch.object(HealthCheckService, 'check_system_resources', return_value={'status': 'healthy'})
...
class TestGetHealthData:
    def test_health_data_structure(self, mocker, app):
        ...
        mocker.patch('services.health_service.psutil.virtual_memory', return_value=MagicMock(total=8e9, used=4e9))
        mocker.patch('services.health_service.psutil.disk_usage', return_value=MagicMock(total=100e9, used=50e9, free=50e9))
