...
def _partner(**kwargs):
    p = MagicMock()
    defaults = dict(
        id=1, tenant_id=1, is_active=True, scope_type='company', scope_id=None,
        share_percentage=Decimal('50'), expense_share_percentage=Decimal('10'),
        loss_share_percentage=Decimal('0'), fixed_monthly_amount=Decimal('0'),
        min_profit_threshold=Decimal('0'), current_balance=Decimal('0'),
        total_profit_received=Decimal('0'), total_loss_borne=Decimal('0'),
        total_withdrawals=Decimal('0'), total_additional_investment=Decimal('0'),
    )
...
class TestScopeAggregation:
    def test_get_scope_revenue_company(self, mocker):
        mocker.patch('services.partner_service.db.session.query', return_value=_query_chain(Decimal('1000')))
    ...
    def test_get_scope_expenses_branch(self, mocker):
        mocker.patch('services.partner_service.db.session.query', return_value=_query_chain(Decimal('150')))
...
class TestCreateDistributions:
    ...
    def test_skips_existing_period(self, mocker):
        ...
        mocker.patch.object(PartnerService, 'calculate_scope_profit', return_value={'revenue': 0, 'cogs': 0, 'expenses': 0, 'gross_profit': 0, 'net_profit': 0})
        assert PartnerService.create_distributions(1, date(2026, 1, 1), date(2026, 1, 31)) == []
    ...
    def test_profit_distribution_commits(self, mocker):
        ...
        D.return_value = dist
        mocker.patch.object(PartnerService, 'calculate_scope_profit', return_value={
            'revenue': 10000.0, 'cogs': 2000.0, 'expenses': 1000.0,
            'gross_profit': 8000.0, 'net_profit': 7000.0,
        })
...
class TestDistributionLifecycle:
    ...
    def test_approve_distribution_positive_net(self, mocker):
        dist = MagicMock(status='draft', tenant_id=1, net_due=100.0, partner_id=1, period_start=date(2026, 1, 1), period_end=date(2026, 1, 31))
    ...
    def test_pay_distribution_success(self, mocker):
        ...
        mocker.patch('services.gl_service.GLService.get_account_code_for_concept', return_value='2150')
    ...
    def test_pay_distribution_gl_fallback(self, mocker):
        ...
        mock_db.session.get.return_value = dist
        mocker.patch('utils.tax_settings._resolve_main_branch', return_value=1)
        mocker.patch('services.gl_service.GLService.get_account_code_for_concept', side_effect=RuntimeError())
        mocker.patch('services.gl_service.GLService.get_default_liquidity_account', side_effect=RuntimeError())
