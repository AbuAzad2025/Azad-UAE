...
class TestClearCheque:
    ...
    def test_clear_confirms_receipt(self, mocker, db_session, sample_tenant, incoming_cheque, sample_customer, sample_user):
        incoming_cheque.status = 'deposited'
        receipt = Receipt(
            tenant_id=sample_tenant.id,
...
class TestChequeEdgePaths:
    ...
    def test_clear_confirms_receipt_with_tenant(self, db_session, sample_tenant, incoming_cheque, sample_customer, sample_user):
        incoming_cheque.status = 'deposited'
        receipt = Receipt(
            tenant_id=sample_tenant.id,
            receipt_number=f'RCP-{uuid.uuid4().hex[:6]}',
    ...
    def test_cancel_rejects_receipt_loop(self, db_session, sample_tenant, incoming_cheque, sample_customer, sample_user):
        receipt = Receipt(
            tenant_id=sample_tenant.id,
            receipt_number=f'RCP-{uuid.uuid4().hex[:6]}',
            customer_id=sample_customer.id,
    ...
    def test_status_listener_error_logged(self, mocker):
        ...
        register_cheque_event_listeners()
        err = mocker.patch('services.cheque_service.logger.error')
        mocker.patch('services.cheque_service.logger.info', side_effect=RuntimeError('status log'))
