...
class TestIndexSale:
    ...
    def test_not_configured_returns_error(self, mocker):
        mocker.patch('services.elasticsearch_service.ElasticsearchService.is_enabled', return_value=False)
        from services.elasticsearch_service import ElasticsearchService
    ...
    def test_successful_index(self, mocker):
        mocker.patch('services.elasticsearch_service.ElasticsearchService.is_enabled', return_value=True)
...
class TestSearchSales:
    ...
    def test_offline_uses_sql_fallback(self, mocker):
        ...
        mocker.patch(
            ...
            return_value={'success': True, 'results': [{'id': 1}], 'total': 1, 'fallback': True},
        )
        from services.elasticsearch_service import ElasticsearchService
    ...
    def test_es_search_with_filters(self, mocker):
        ...
        mock_es.search.return_value = {
            'hits': {'hits': [{'_source': {'sale_number': 'S-9'}}], 'total': {'value': 1}},
        }
        import sys
        es_mod = MagicMock()
        es_mod.Elasticsearch = MagicMock(return_value=mock_es)
