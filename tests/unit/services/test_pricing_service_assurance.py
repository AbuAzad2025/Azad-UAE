...
class TestTieredPricing:
    ...
    def test_partner_discount_when_no_tier(self, mocker):
        product = MagicMock(
            id=2, regular_price=Decimal('200'),
            partner_price=Decimal('10'), merchant_price=None,
        )
    ...
    def test_merchant_discount_applied(self, mocker):
        product = MagicMock(
            id=3, regular_price=Decimal('100'),
            partner_price=None, merchant_price=Decimal('25'),
        )
        _mock_tier_query(mocker, None)
...
class TestFormatPrice:
    ...
    def test_decimal_string_conversion_stable(self, mocker):
        import utils.currency_utils as cu
        mocker.patch.object(cu, 'format_currency_value', side_effect=lambda p, c: f'{c}:{p}', create=True)
