...
def _account(aid=10, code='1130', name='AR', name_ar='', acc_type='asset', active=True, header=False):
    acc = SimpleNamespace(
        id=aid, code=code, name=name, name_ar=name_ar, type=acc_type,
        is_active=active, is_header=header, level=1, children=[],
    )
    return acc
...
class TestExecute:
    ...
    def test_execute_dry_run_rolls_back(self, mocker):
        ...
        mock_q.filter_by.return_value.first.return_value = tenant
        mocker.patch('services.gl_accounting_setup.Tenant.query', mock_q)
        mocker.patch.object(GLAccountingSetupService, '_build_plan', side_effect=[
            [SetupPlanAction('create_account', 1, 'CASH')],
            [SetupPlanAction('map_concept', 1, 'CASH', gl_account_id=7, gl_account_code='1110-B1')],
        ])
        mocker.patch.object(GLAccountingSetupService, '_create_account', return_value=_account(7, '1110-B1'))
    ...
    def test_execute_create_account_error(self, mocker):
        ...
        mocker.patch.object(GLAccountingSetupService, '_create_account', side_effect=RuntimeError('dup code'))
...
class TestFindBestCandidate:
    ...
    def test_parent_child_scan_prefers_main(self, mocker):
        ...
        rule = ConceptSetupRule(
            parent_code_hint='1110', expected_types=('asset',),
