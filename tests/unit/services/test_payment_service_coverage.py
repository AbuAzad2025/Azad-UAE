...
class TestPaymentServiceHelpers:
    def test_resolve_transaction_rate(self, app):
        ...
        with app.app_context(), \
             patch('services.payment_service.ExchangeRateService.resolve_exchange_rate_for_transaction', return_value={'rate': 3.67, 'rate_mode': 'auto'}):
    ...
    def test_resolve_transaction_rate_needs_input(self, app):
        ...
        with app.app_context(), \
             patch('services.payment_service.ExchangeRateService.resolve_exchange_rate_for_transaction', return_value={'rate_mode': 'needs_input'}):
    ...
    def test_get_customer_balance_and_unpaid_sales(self):
        ...
        sale = MagicMock(
            id=2, sale_number='S-1', sale_date=MagicMock(strftime=lambda fmt: '2025-01-01'),
            total_amount=Decimal('100'), balance_due=Decimal('50'), currency='AED',
        )
        with patch.object(PaymentService, 'get_customer_balance_aed', return_value=Decimal('50')), \
...
class TestPaymentServiceCreatePayment:
    ...
    def test_create_payment_success_cash(self, app):
        ...
        payment = MagicMock(id=10, payment_number='PAY-1', branch_id=1, amount=Decimal('100'), amount_aed=Decimal('100'), payment_confirmed=True)
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, id=1, tenant_id=1)), \
...
class TestPaymentServiceCreateReceipt:
    ...
    def test_allocate_receipt_to_oldest_sales(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1)
        receipt = MagicMock(amount_aed=Decimal('100'), receipt_number='RCV-1', currency='AED', exchange_rate=Decimal('1'), payment_method='cash', payment_confirmed=True, branch_id=1)
        sale = MagicMock(id=2, balance_due=Decimal('100'), branch_id=1, tenant_id=1, exchange_rate=Decimal('1'))
        with app.app_context(), \
             patch.object(PaymentService, 'get_unpaid_sales', return_value=[sale]), \
...
class TestPaymentServiceResolveBranch:
    ...
    def test_resolve_branch_from_current_user(self):
        ...
        with patch('services.payment_service.branch_scope_id_for', return_value=None), \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, branch_id=4)):
...
class TestPaymentServiceCreateReceiptSuccess:
    def test_create_receipt_cash_success(self, app):
        ...
        receipt = MagicMock(id=10, receipt_number='RCV-1', amount=Decimal('100'), amount_aed=Decimal('100'),
        ...
        with app.app_context(), \
            ...
            gl.ensure_core_accounts.return_value = None
            result = PaymentService.create_receipt({'customer_id': 1, 'amount': 100, 'currency': 'AED', 'payment_method': 'cash'})
    ...
    def test_create_receipt_cheque_success(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1, name='Cust')
        receipt = MagicMock(id=11, receipt_number='RCV-2', amount=Decimal('200'), amount_aed=Decimal('200'),
                            payment_method='cheque', payment_confirmed=False, branch_id=1, currency='AED',
                            exchange_rate=Decimal('1'), receipt_date=datetime.now())
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, id=1, tenant_id=1)), \
             ...
             patch.object(PaymentService, '_resolve_transaction_rate', return_value=Decimal('1')), \
             patch.object(PaymentService, '_resolve_branch_id', return_value=1), \
             patch('services.payment_service.process_cheque_receive', return_value=MagicMock()), \
             patch('models.Cheque'), \
             patch('services.payment_service.Receipt', return_value=receipt):
            mock_db.session.get.return_value = customer
            result = PaymentService.create_receipt({
                'customer_id': 1, 'amount': 200, 'currency': 'AED', 'payment_method': 'cheque',
                'cheque_number': 'CHQ1', 'cheque_date': '2026-06-30', 'bank_name': 'ENBD',
            })
    ...
    def test_create_receipt_with_allocation(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1, name='Cust')
        sale = MagicMock(id=2, customer_id=1, branch_id=1, tenant_id=1, balance_due=Decimal('100'), exchange_rate=Decimal('1'))
        receipt = MagicMock(id=12, receipt_number='RCV-3', amount_aed=Decimal('100'), amount=Decimal('100'),
                            payment_method='cash', payment_confirmed=True, branch_id=1, currency='AED', exchange_rate=Decimal('1'))
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, id=1, tenant_id=1)), \
            ...
            gl.ensure_core_accounts.return_value = None
            PaymentService.create_receipt({
                'customer_id': 1, 'amount': 100, 'currency': 'AED', 'payment_method': 'cash',
...
class TestPaymentServiceCreatePaymentExtended:
    def test_create_payment_cheque(self, app):
        ...
        supplier = MagicMock(id=1, name='Sup', tenant_id=1)
        payment = MagicMock(id=10, payment_number='PAY-1', branch_id=1, amount=Decimal('100'), amount_aed=Decimal('100'), payment_confirmed=False)
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, id=1, tenant_id=1)), \
             patch('services.payment_service.generate_number', return_value='PAY-001'), \
...
class TestPaymentServiceFailurePaths:
    def test_create_payment_gl_failure(self, app):
        ...
        supplier = MagicMock(id=1, name='Sup', tenant_id=1)
        payment = MagicMock(id=10, payment_number='PAY-GL', branch_id=1, amount=Decimal('100'),
                            amount_aed=Decimal('100'), payment_confirmed=True, currency='AED', exchange_rate=Decimal('1'))
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, id=1, tenant_id=1)), \
            ...
            with pytest.raises(ValueError, match='فشل الترحيل المحاسبي'):
                PaymentService.create_payment({
                    'supplier_id': 1, 'amount': Decimal('100'), 'currency': 'AED', 'payment_method': 'cash',
                })
    ...
    def test_create_receipt_fx_gain_on_allocation(self, app):
        ...
        source_sale = MagicMock(
            id=2, customer_id=1, branch_id=1, tenant_id=1, currency='USD',
            exchange_rate=Decimal('3.67'), balance_due=Decimal('100'),
        )
        source_sale.recalculate_payment_status = MagicMock()
        receipt = MagicMock(
            id=12, receipt_number='RCV-FX', amount=Decimal('100'), amount_aed=Decimal('380'),
            payment_method='cash', payment_confirmed=True, branch_id=1, currency='USD',
        ...
        with app.app_context(), \
             ...
             patch.object(PaymentService, '_resolve_transaction_rate', return_value=Decimal('3.80')), \
             patch.object(PaymentService, '_resolve_branch_id', return_value=1), \
             patch('services.payment_service.GLService') as gl, \
            ...
            gl.ensure_core_accounts.return_value = None
            PaymentService.create_receipt({
                'customer_id': 1, 'amount': 100, 'currency': 'USD', 'payment_method': 'cash',
                'allocate_to_sales': {2: 100},
            })
...
class TestPaymentServiceReceiptCommitFailure:
    def test_create_receipt_commit_failure(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1, name='Cust')
        receipt = MagicMock(
            id=14, receipt_number='RCV-COMMIT', amount=Decimal('50'), amount_aed=Decimal('50'),
            payment_method='cash', payment_confirmed=True, branch_id=1, currency='AED', exchange_rate=Decimal('1'),
        )
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
    ...
    def test_create_receipt_outer_exception(self, app):
        ...
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, id=1, tenant_id=1)), \
             patch('services.payment_service.generate_number', side_effect=RuntimeError('gen fail')), \
             patch.object(PaymentService, '_resolve_transaction_rate', return_value=Decimal('1')), \
             patch.object(PaymentService, '_resolve_branch_id', return_value=1), \
             patch('services.payment_service.current_app') as capp:
            mock_db.session.get.return_value = customer
...
class TestPaymentServicePaymentCommitFailure:
    def test_create_payment_commit_failure(self, app):
        ...
        supplier = MagicMock(id=1, name='Sup', tenant_id=1)
        payment = MagicMock(id=10, payment_number='PAY-C', branch_id=1, amount=Decimal('100'),
                            amount_aed=Decimal('100'), payment_confirmed=True, currency='AED', exchange_rate=Decimal('1'))
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             patch('services.payment_service.current_user', MagicMock(is_authenticated=True, id=1, tenant_id=1)), \
...
class TestPaymentServiceChequeAndFxLoss:
    def test_create_receipt_cheque_gl_failure(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1, name='Cust')
        receipt = MagicMock(id=15, receipt_number='RCV-CHQ', amount=Decimal('200'), amount_aed=Decimal('200'),
                            payment_method='cheque', payment_confirmed=False, branch_id=1, currency='AED',
                            exchange_rate=Decimal('1'), receipt_date=datetime.now())
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
    ...
    def test_create_receipt_fx_loss_on_allocation(self, app):
        ...
        source_sale = MagicMock(
            id=2, customer_id=1, branch_id=1, tenant_id=1, currency='USD',
            exchange_rate=Decimal('3.80'), balance_due=Decimal('100'),
        )
        source_sale.recalculate_payment_status = MagicMock()
        receipt = MagicMock(
            id=16, receipt_number='RCV-LOSS', amount=Decimal('100'), amount_aed=Decimal('350'),
            payment_method='cash', payment_confirmed=True, branch_id=1, currency='USD',
        ...
        with app.app_context(), \
             ...
             patch.object(PaymentService, '_resolve_transaction_rate', return_value=Decimal('3.50')), \
             patch.object(PaymentService, '_resolve_branch_id', return_value=1), \
             patch('services.payment_service.GLService') as gl, \
             patch('services.payment_service.post_or_fail') as post, \
             patch('services.payment_service.Receipt', return_value=receipt), \
    ...
    def test_allocate_receipt_commit_failure(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1)
        receipt = MagicMock(amount_aed=Decimal('50'), receipt_number='RCV-ALLOC', currency='AED',
                            exchange_rate=Decimal('1'), payment_method='cash', payment_confirmed=True, branch_id=1)
        sale = MagicMock(id=2, balance_due=Decimal('50'), branch_id=1, tenant_id=1, exchange_rate=Decimal('1'))
        with app.app_context(), \
             patch.object(PaymentService, 'get_unpaid_sales', return_value=[sale]), \
    ...
    def test_create_receipt_gl_posting_failure(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1, name='Cust')
        receipt = MagicMock(
            id=17, receipt_number='RCV-GL', amount=Decimal('50'), amount_aed=Decimal('50'),
            payment_method='cash', payment_confirmed=True, branch_id=1, currency='AED', exchange_rate=Decimal('1'),
        )
        with app.app_context(), \
             patch('services.payment_service.db') as mock_db, \
             ...
             patch.object(PaymentService, '_resolve_branch_id', return_value=1), \
             patch('services.payment_service.GLService') as gl, \
             patch('services.payment_service.post_or_fail', side_effect=RuntimeError('gl fail')), \
             patch('services.payment_service.Receipt', return_value=receipt), \
             patch('services.payment_service.current_app') as capp:
            mock_db.session.get.return_value = customer
            gl.get_payment_debit_account.return_value = '1100'
    ...
    def test_create_receipt_fx_post_skip_on_error(self, app):
        ...
        source_sale.recalculate_payment_status = MagicMock()
        receipt = MagicMock(
            id=18, receipt_number='RCV-FXSKIP', amount=Decimal('100'), amount_aed=Decimal('380'),
            payment_method='cash', payment_confirmed=True, branch_id=1, currency='USD',
            exchange_rate=Decimal('3.50'),
        )
        with app.app_context(), \
             ...
             patch.object(PaymentService, '_resolve_branch_id', return_value=1), \
             patch('services.payment_service.GLService') as gl, \
             patch('services.payment_service.post_or_fail', side_effect=[None, RuntimeError('fx skip')]), \
             patch('services.payment_service.Receipt', return_value=receipt), \
             patch('models.Payment', return_value=MagicMock()), \
             patch('services.payment_service.current_app') as capp:
            mock_db.session.get.side_effect = lambda model, pk: customer if pk == 1 else source_sale
    ...
    def test_allocate_receipt_zero_balance_skipped(self, app):
        ...
        customer = MagicMock(id=1, tenant_id=1)
        receipt = MagicMock(amount_aed=Decimal('50'), receipt_number='RCV-ZERO', currency='AED',
                            exchange_rate=Decimal('1'), payment_method='cash', payment_confirmed=True, branch_id=1)
        sale = MagicMock(id=2, balance_due=Decimal('0'), branch_id=1, tenant_id=1, exchange_rate=Decimal('1'))
        with app.app_context(), \
             patch.object(PaymentService, 'get_unpaid_sales', return_value=[sale]), \
