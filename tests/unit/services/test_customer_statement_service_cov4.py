...
def test_payments_receipts_returns_and_opening(
    ...
    pay = Payment(
        tenant_id=sample_tenant.id,
        customer_id=sample_customer.id,
        payment_number="PAY-COV4",
        payment_type="sale_payment",
        reference_number="PAY-COV4",
        direction="incoming",
        amount=Decimal("40"),
    ...
    ctx2 = CustomerStatementService.build_statement_context(
        **_base_kwargs(sample_customer, sample_tenant, date_from=(date.today()).isoformat())
    )
    assert ctx2["transactions"][0]["type"] == "opening"
    # date_to filter arc
    ctx3 = CustomerStatementService.build_statement_context(
        **_base_kwargs(sample_customer, sample_tenant, date_from="2000-01-01", date_to="2000-12-31")
    )
...
def test_cheque_payment_pending_counts_via_pending_cheque_arc(
    ...
    chq = Payment(
        tenant_id=sample_tenant.id,
        customer_id=sample_customer.id,
        payment_number="PAY-CHQ",
        payment_type="sale_payment",
        direction="incoming",
        amount=Decimal("25"),
        amount_aed=Decimal("25"),
    ...
    db_session.flush()
    ctx = CustomerStatementService.build_statement_context(**_base_kwargs(sample_customer, sample_tenant))
    chq_t = next(t for t in ctx["transactions"] if t.get("payment", {}).get("payment_number") == "PAY-CHQ")
...
def test_receipt_and_return_arcs(
    ...
    pay = Payment(
        tenant_id=sample_tenant.id,
        customer_id=sample_customer.id,
        payment_number="PAY-AL",
        payment_type="sale_payment",
        reference_number="RCP-COV4",
        direction="incoming",
        amount=Decimal("15"),
        amount_aed=Decimal("15"),
        payment_method="cash",
