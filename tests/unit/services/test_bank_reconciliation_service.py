...
class TestCreateReconciliation:
    def test_creates_with_auto_items(
        self,
        mocker,
        db_session,
        sample_tenant,
        ...
        mocker.patch(
            "services.bank_reconciliation_service.generate_number",
            return_value="BR-NEW-001",
        )
...
class TestSuspenseAndApply:
    ...
    def test_apply_matches_skips_missing_rows(self, db_session, draft_reconciliation):
        rec = BankReconciliationService.apply_matches(
            ...
            [
                {
                    "statement_line_id": 99999,
                    "journal_line_id": 88888,
                    "match_type": "exact",
                }
            ],
        )
...
class TestCommitRollbackPaths:
    ...
    def test_apply_matches_commit_failure(
        self, mocker, db_session, draft_reconciliation, sample_tenant, bank_gl_account
    ):
        stmt = BankStatementLine(
            tenant_id=sample_tenant.id,
            bank_account_id=bank_gl_account.id,
        ...
        mocker.patch(
            "services.bank_reconciliation_service.db.session.flush",
            side_effect=RuntimeError("apply"),
        )
