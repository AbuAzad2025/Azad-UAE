...
class TestSubscribe:
    ...
    def test_new_subscriber_with_customer(self, tenant_user, db_session, sample_tenant, sample_customer):
        ...
        sub = EmailMarketingService.subscribe(
            lst.id, 'cust@example.com', name='Cust', customer_id=sample_customer.id, user=tenant_user,
        )
...
class TestSendCampaign:
    ...
    def test_no_list(self, tenant_user, db_session, sample_tenant):
        lst = _email_list(db_session, sample_tenant.id)
        tpl = _template(db_session, sample_tenant.id)
        camp = EmailCampaign(tenant_id=sample_tenant.id, name='No list', template_id=tpl.id, status='draft')
        db_session.add(camp)
        db_session.flush()
