...
from services.backup_scope_config import SCOPE_STORE, SCOPE_TENANT
...
class TestImportScopedTablesExtended:
    ...
    def test_fatal_table_missing_marks_not_ok(self, mocker):
        ...
        mocker.patch(
            "services.backup_scoped_restore.normalize_row_to_target",
            side_effect=lambda c, t, r: r,
        )
        tables = {
            "tenants": [{"id": 1}],
            "roles": [{"id": 1}],
...
class TestRestoreScopedBackupFlow:
    ...
    def _write_minimal_bundle(tmp_path, include_uploads=False):
        ...
        (data_dir / "tenants.jsonl").write_text(
            json.dumps({"id": 7, "slug": "t7"}),
            encoding="utf-8",
        )
        (data_dir / "products.jsonl").write_text(
            json.dumps({"id": 10, "tenant_id": 7, "name": "P"}),
            encoding="utf-8",
        )
    ...
    def test_restore_verify_failure_extends_errors(self, mocker, tmp_path):
        ...
        mocker.patch(
            ...
            return_value={
                "ok": False,
                "errors": ["products: expected>=1 got 0"],
                "warnings": [],
            },
        )
        result = restore_scoped_backup(
