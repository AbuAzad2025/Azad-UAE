...
class TestShopCustomerAuthGaps:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "authenticate",
                lambda svc, tid: svc.authenticate(tid, "c2@shop.test", "pass1234"),
            ),
            (
                "request_password_reset",
                lambda svc, tid: svc.request_password_reset(tid, "c3@shop.test"),
            ),
            (
                "reset_password",
                lambda svc, tid: svc.reset_password(tid, "tok", "newpass1"),
            ),
        ],
    )
    def test_commit_failure_rolls_back(
        ...
        mocker.patch(
            "services.shop_customer_auth_service.db.session.flush",
...
class TestReturnServiceRemainingGaps:
    ...
    def test_serial_not_linked_to_line(self, app, mocker):
        ...
        helper._patch_common(mocker, sale, line, product)
        mocker.patch(
            "services.return_service.ReturnService._serials_from_line_data",
            return_value=["SN-MISS"],
        )
    ...
    def test_vat_inclusive_commission_revenue_path(self, app):
        ...
        with _create_ctx(customer, seller, product) as (svc, stock, db_sess, _):
            ...
            with (
                ...
                patch("utils.tax_settings.get_prices_include_vat", return_value=True),
                patch(
                    "services.sale_service.normalize_tax_rate",
