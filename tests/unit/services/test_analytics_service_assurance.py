...
class TestDonationAnalytics:
    def test_revenue_by_period(self, mocker):
        ...
        mocker.patch('services.analytics_service._db_session').return_value.query.return_value.all.return_value = [donation]
        mocker.patch('services.analytics_service.get_active_tenant_id', return_value=1, create=True)
    ...
    def test_predict_revenue(self, mocker):
        mocker.patch(
            ...
            return_value={'total_revenue': 600, 'labels': [], 'purchases': [], 'donations': []},
        )
        from services.analytics_service import AnalyticsService
