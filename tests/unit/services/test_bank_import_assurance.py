...
class TestStatementImport:
    ...
    def test_imports_new_line(self, app, mocker):
        ...
        mocker.patch.object(
            BankStatementLine, 'query', new_callable=mocker.PropertyMock, return_value=mock_q
        )
        ...
        with app.app_context():
            lines = BankImportService.import_bank_statement(
                tenant_id=1, bank_account_id=10, user_id=3,
                filename='stmt.ofx', file_content=b'OFXDATA', format='ofx',
            )
        assert len(lines) == 1
    ...
    def test_duplicate_reference_skipped(self, app, mocker):
        ...
        with app.app_context():
            lines = BankImportService.import_bank_statement(
                1, 10, 3, 'stmt.csv', b'data', format='csv',
            )
        assert lines == []
    ...
    def test_unrecognized_format_still_parses_placeholder(self, app, mocker):
        ...
        with app.app_context():
            lines = BankImportService.import_bank_statement(
                1, 10, 3, 'unknown.xlsx', b'excel-bytes', format='xlsx',
            )
        assert len(lines) == 1
