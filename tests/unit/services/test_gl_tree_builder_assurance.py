...
from models import GLAccount, Branch, Tenant
...
class TestProcessAccount:
    def test_process_existing_updates_fields(self, app, db_session, sample_tenant):
        with app.app_context():
            ...
            result = GLTreeBuilder._process_account(
                sample_tenant.id,
                tmpl.code, tmpl.name_ar, tmpl.name, tmpl.type,
                tmpl.parent_code, tmpl.is_header, tmpl.level,
                existing, processed,
            )
    ...
    def test_process_existing_updates_all_fields(self, app, db_session, sample_tenant):
        with app.app_context():
            ...
            result = GLTreeBuilder._process_account(
                sample_tenant.id,
                child_tmpl.code, child_tmpl.name_ar, child_tmpl.name, child_tmpl.type,
                child_tmpl.parent_code, child_tmpl.is_header, child_tmpl.level,
                existing, processed,
            )
    ...
    def test_process_create_with_parent_from_existing(self, app, db_session):
        ...
        with app.app_context():
            ...
            GLTreeBuilder._process_account(
                tenant.id,
                parent_tmpl.code, parent_tmpl.name_ar, parent_tmpl.name, parent_tmpl.type,
                parent_tmpl.parent_code, parent_tmpl.is_header, parent_tmpl.level,
                existing, processed,
            )
            result = GLTreeBuilder._process_account(
                tenant.id,
        ...
        with app.app_context():
            ...
            if acc is None:
                GLTreeBuilder._ensure_branch_liquidity_accounts(
                    ...
                    processed=processed,
                    audit_report={'created': [], 'updated': [], 'converted': [], 'deactivated': [], 'errors': []},
                )
                acc = existing.get(code)
            acc.name = 'Stale'
            acc.name_ar = 'قديم'
            ...
            report = {'created': [], 'updated': [], 'converted': [], 'deactivated': [], 'errors': []}
