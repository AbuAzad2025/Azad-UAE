...
def test_receive_idempotent_and_issue_expense_skip(
    db_session, incoming_cheque, outgoing_cheque, sample_tenant, sample_user
):
    ...
    cat = ExpenseCategory(tenant_id=sample_tenant.id, name="Test Expense", gl_account_code="6010")
    db_session.add(cat)
    db_session.flush()
    exp = Expense(
        tenant_id=sample_tenant.id,
