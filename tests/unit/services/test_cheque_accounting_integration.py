...
class TestClearCheque:
    ...
    def test_exchange_rate_branch(self, mocker, db_session, incoming_cheque):
        ...
        ChequeAccountingIntegration.clear_cheque(
            incoming_cheque.id, exchange_gain_loss=Decimal('3'),
        )
...
class TestAccountingSummary:
    def test_summary_assembly(
        self, incoming_cheque, db_session, sample_tenant, sample_gl_accounts,
    ):
        ...
        if not account:
            account = GLAccount(
                tenant_id=sample_tenant.id, code='1110', name='Cash', account_type='asset',
            )
