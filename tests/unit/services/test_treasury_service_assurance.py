...
class TestLiquidityPosition:
    ...
    def test_cashbox_balances_summed_by_kind(self, mocker):
        box = MagicMock(
            id=1, code='CASH-01', name_ar='صندوق', name_en='Cash',
            box_type='cash', current_balance=Decimal('5000'),
            currency='AED', branch_id=2,
        )
        mock_q = MagicMock()
    ...
    def test_gl_fallback_when_no_cashboxes(self, mocker):
        ...
        acc = MagicMock(
            id=10, code='1100', liquidity_kind='bank',
            name_ar='بنك', name_en='Bank', currency='AED',
        )
    ...
    def test_normalizes_bank_account_box_type(self, mocker):
        box = MagicMock(
            id=2, code='BNK', name_ar=None, name_en='Main', box_type='bank_account',
            current_balance=100, currency='AED', branch_id=None,
        )
        mock_q = MagicMock()
        mock_q.filter_by.return_value = mock_q
        mock_q.order_by.return_value = mock_q
...
class TestChequeMaturity:
    ...
    def test_buckets_incoming_cheques(self, mocker):
        today = date.today()
        overdue = MagicMock(
            id=1, cheque_type='incoming', amount_aed=Decimal('1000'),
            due_date=today - timedelta(days=5), cheque_number='C1',
            cheque_bank_number=None, bank_name='B', drawer_name='D',
            payee_name='P', status='pending', cheque_type_ar='وارد',
        )
        soon = MagicMock(
            id=2, cheque_type='incoming', amount_aed=Decimal('500'),
            due_date=today + timedelta(days=3), cheque_number='C2',
            cheque_bank_number=None, bank_name='B', drawer_name='D',
            payee_name='P', status='pending', cheque_type_ar='وارد',
        )
        mock_q = MagicMock()
...
class TestDashboard:
    ...
    def test_reconciliation_list_shape(self, mocker):
        rec = MagicMock(
            id=1, reconciliation_number='REC-1',
            period_start=date(2025, 1, 1), period_end=date(2025, 1, 31),
            closing_balance_per_books=100, closing_balance_per_bank=99,
            difference=1, status='open', status_ar='مفتوح',
            created_at=None,
        )
