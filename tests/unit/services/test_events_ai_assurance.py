...
class TestErrorPaths:
    ...
    def test_customer_analysis_learning_unavailable(self, mocker):
        ...
        target = SimpleNamespace(
            id=1, balance=Decimal('0'), total_purchases=Decimal('0'),
            customer_classification='regular', credit_limit=Decimal('100'),
        )
        from services.events_ai_service import _customer_analysis
    ...
    def test_customer_credit_limit_exceeded(self, mocker):
        _mock_learning(mocker)
        target = SimpleNamespace(
            id=6, balance=Decimal('500'), total_purchases=Decimal('0'),
            customer_classification='regular', credit_limit=Decimal('100'),
        )
        from services.events_ai_service import _customer_analysis
    ...
    def test_comprehensive_sale_low_margin(self, mocker):
        ...
        target = SimpleNamespace(
            id=1, sale_number='S-L', amount_aed=Decimal('100'),
            discount_amount=Decimal('0'), subtotal=Decimal('100'), lines=[line],
        )
        from services.events_ai_service import _comprehensive_sale_analysis
    ...
    def test_learn_accounting_entries_unbalanced(self, mocker):
        ...
        target = SimpleNamespace(
            ...
            reference_type='sale', reference_id=1, lines=[1, 2],
        )
    ...
    def test_intelligent_sale_analysis_large_invoice(self, mocker):
        ...
        target = SimpleNamespace(
            status='confirmed', is_active=True, amount_aed=Decimal('15000'), lines=[line] * 12,
            sale_number='S-BIG', customer=customer, payment_status='paid',
        )
        from services.events_ai_service import _intelligent_sale_analysis
    ...
    def test_learn_product_terminology_with_category(self, mocker):
        ...
        target = SimpleNamespace(
            id=2, name='Laptop', name_ar='حاسوب', commercial_name='Pro',
            part_number='L1', category=cat,
        )
        from services.events_ai_service import _learn_product_terminology
...
class TestHandlerExceptions:
    def test_outer_exception_handlers(self, mocker):
        targets = {
            '_learn_sale_patterns': MagicMock(
                sale_date=None, lines=None, subtotal=0, discount_amount=0,
                amount_aed=1, sale_number='S',
            ),
            '_customer_analysis': SimpleNamespace(
                id=1, balance=0, total_purchases=0,
                customer_classification='r', credit_limit=0,
            ),
            '_learn_product_performance': SimpleNamespace(
                id=1, name='X', current_stock=1, min_stock_alert=0,
                cost_price=1, regular_price=2,
            ),
            '_detect_stock_anomaly': SimpleNamespace(
                id=1, name='X', current_stock=1, min_stock_alert=0,
            ),
            '_comprehensive_sale_analysis': SimpleNamespace(
                id=1, sale_number='S', amount_aed=1, discount_amount=0,
                subtotal=1, lines=[],
            ),
            '_learn_product_terminology': SimpleNamespace(
                id=1, name='X', name_ar=None, commercial_name=None,
                part_number=None, category=None,
            ),
            '_learn_customer_names': SimpleNamespace(
                name='A', name_ar=None, customer_type='r', phone=None, email=None,
            ),
            '_learn_sales_practices': SimpleNamespace(
                sale_number='S', amount_aed=1, paid_amount_aed=1,
                payment_status='paid', discount_amount=0, subtotal=1,
                shipping_cost=0, tax_amount=0,
            ),
            '_learn_procurement_strategy': SimpleNamespace(
                supplier_id=1, amount_aed=1, payment_method='cash',
                status='open', purchase_number='P',
            ),
            '_learn_expense_patterns': SimpleNamespace(
                category_id=1, amount_aed=1, payment_method='cash', is_recurring=False,
            ),
            '_learn_accounting_entries': SimpleNamespace(
                entry_number='J', total_debit=1, total_credit=1,
                reference_type='x', reference_id=1, lines=[],
            ),
            '_learn_revenue_recognition': SimpleNamespace(
                sale_number='S', amount_aed=1, status='confirmed',
                paid_amount_aed=1, balance_due=0,
            ),
            '_learn_expense_recognition': SimpleNamespace(
                purchase_number='P', amount_aed=1, status='confirmed', paid_amount_aed=0,
            ),
            '_predict_future_sales': MagicMock(),
            '_predict_stockout': SimpleNamespace(
                id=1, name='X', current_stock=1, min_stock_alert=10,
            ),
            ...
            '_neural_inventory_learning': SimpleNamespace(
                id=1, current_stock=1, min_stock_alert=0,
            ),
            '_intelligent_sale_analysis': SimpleNamespace(
                status='draft', is_active=True, amount_aed=1, lines=[],
                sale_number='S', customer=None, payment_status='paid',
            ),
            '_intelligent_customer_monitoring': SimpleNamespace(id=1),
            '_intelligent_inventory_alert': SimpleNamespace(
                id=1, name='X', current_stock=0, min_stock_alert=1,
            ),
        }
        mocker.patch(
            'ai_knowledge.core.learning_system.AzadLearningSystem',
    ...
    def test_register_handlers_are_invoked(self, app, mocker):
        ...
        sale = SimpleNamespace(
            id=1, sale_number='S', customer_id=1, amount_aed=Decimal('10'),
            subtotal=Decimal('10'), discount_amount=Decimal('0'), payment_status='paid',
            sale_date=datetime.now(timezone.utc), lines=[], status='confirmed', is_active=True,
            paid_amount_aed=Decimal('10'), shipping_cost=0, tax_amount=0, balance_due=0,
        )
    ...
    def test_accounting_entries_inner_learning_failure(self, mocker):
        target = SimpleNamespace(
            entry_number='JE-3', total_debit=Decimal('1'), total_credit=Decimal('1'),
            reference_type='sale', reference_id=1, lines=[],
        )
        target.is_balanced = lambda: True
        ins = MagicMock()
    ...
    def test_intelligent_sale_analysis_exception(self, mocker):
        ...
        target = SimpleNamespace(
            status='confirmed', is_active=True, amount_aed=Decimal('100'), lines=[line],
            sale_number='S-ERR', customer=None, payment_status='paid',
        )
        from services.events_ai_service import _intelligent_sale_analysis
    ...
    def test_neural_handlers_outer_failure(self, mocker):
        ...
        from services.events_ai_service import _neural_auto_retrain, _neural_customer_data
    ...
    def test_predict_stockout_logs_days_remaining(self, mocker):
        ...
        target = SimpleNamespace(
            id=2, name='Widget', current_stock=Decimal('8'), min_stock_alert=Decimal('10'),
        )
        from services.events_ai_service import _predict_stockout
    ...
    def test_intelligent_sale_low_margin_insight_line(self, mocker):
        ...
        target = SimpleNamespace(
            status='confirmed', is_active=True, amount_aed=Decimal('100'), lines=[line],
            sale_number='S-LOW', customer=None, payment_status='paid',
        )
        from services.events_ai_service import _intelligent_sale_analysis
    ...
    def test_intelligent_sale_skips_non_confirmed(self, mocker):
        ...
        mocker.patch('flask.current_app', mock_app, create=True)
        target = SimpleNamespace(
            status='draft', is_active=True, amount_aed=Decimal('100'), lines=[],
            sale_number='S', customer=None, payment_status='paid',
        )
        from services.events_ai_service import _intelligent_sale_analysis
    ...
    def test_intelligent_sale_excellent_margin(self, mocker):
        ...
        target = SimpleNamespace(
            status='confirmed', is_active=True, amount_aed=Decimal('100'), lines=[line],
            sale_number='S-HIGH', customer=None, payment_status='paid',
        )
        from services.events_ai_service import _intelligent_sale_analysis
