...
class TestSumHelpers:
    ...
    def test_sum_sales_applies_branch_and_dates(self, mocker):
        ...
        result = FinancialService.sum_sales(
            1, branch_id=3, date_from=date(2025, 1, 1), date_to=date(2025, 6, 30),
        )
...
class TestFinancialOverview:
    ...
    def test_month_period_renders_template(self, app, mocker):
        mocker.patch('services.financial_service.FinancialService.sum_sales', return_value=Decimal('3000'))
        mocker.patch('services.financial_service.FinancialService.sum_purchases', return_value=Decimal('1000'))
        mocker.patch('services.financial_service.FinancialService.sum_receipts', return_value=Decimal('500'))
