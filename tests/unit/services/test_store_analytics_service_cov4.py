...
def test_order_stats_empty_and_with_orders(db_session, sample_tenant, sample_customer, sample_user, sample_warehouse):
...
def test_top_products_with_and_without_lines(
    db_session, sample_tenant, sample_product, sample_customer, sample_user, sample_warehouse
):
    ...
    sale = Sale(
        tenant_id=sample_tenant.id,
        sale_number="STORE-COV4-2",
        customer_id=sample_customer.id,
        seller_id=sample_user.id,
        warehouse_id=sample_warehouse.id,
        sale_date=dt.now(UTC),
        source="online_store",
        status="confirmed",
        subtotal=Decimal("30"),
...
def test_low_stock_default_and_custom_threshold(db_session, sample_tenant):
    with (
        patch("services.store_analytics_service.StoreService.get_tenant_store", return_value=None),
        patch(
    ...
    with (
        patch(
            ...
            return_value=SimpleNamespace(low_stock_threshold=None),
        ),
        patch(
            "services.store_analytics_service.StoreService.get_catalog_products",
    ...
    with (
        patch(
            "services.store_analytics_service.StoreService.get_tenant_store",
            return_value=SimpleNamespace(low_stock_threshold=10),
        ),
        patch(
            "services.store_analytics_service.StoreService.get_catalog_products",
