...
class TestMWACHelper:
    ...
    def test_mwac_calc_delegates(self):
        ...
        result = StockService._mwac_calc(
            Decimal('5'), Decimal('50'), Decimal('5'), Decimal('10'),
        )
...
class TestAddRemoveStock:
    ...
    def test_remove_stock_negative_quantity(self, mocker):
        mock_create = mocker.patch('services.stock_service.StockService.create_movement', return_value=MagicMock())
        from services.stock_service import StockService
...
class TestAdjustStock:
    ...
    def test_rollback_on_create_failure(self, mocker, app):
        session = mocker.patch('services.stock_service.db.session')
        mocker.patch('services.stock_service.StockService.create_movement', side_effect=RuntimeError('fail'))
...
class TestPostAdjustmentGl:
    ...
    def test_loss_lines_for_negative_qty(self, mocker):
        ...
        StockService._post_adjustment_gl(MagicMock(
            product_id=1, quantity=Decimal('-2'), warehouse_id=5, id=9, tenant_id=1,
        ))
    ...
    def test_gain_lines_for_positive_qty(self, mocker):
        ...
        StockService._post_adjustment_gl(MagicMock(
            product_id=1, quantity=Decimal('2'), warehouse_id=None, id=9,
        ))
        assert mock_post.call_args.kwargs['lines'][0]['debit'] > 0
...
class TestAddOpeningStock:
    ...
    def test_without_cost_skips_gl(self, mocker, app):
        ...
        session = mocker.patch('services.stock_service.db.session')
        session.get.return_value = product
        mocker.patch('services.stock_service.StockService.create_movement', return_value=MagicMock())
        mock_post = mocker.patch('services.gl_posting.post_or_fail')
        from services.stock_service import StockService
...
class TestCreateMovement:
    ...
    def test_auto_creates_warehouse_when_missing(self, mocker, app):
        ...
        mov_cls = mocker.patch('services.stock_service.StockMovement', return_value=MagicMock())
        ...
        pws_cls.query = pws_q
        mocker.patch('services.stock_service.current_user', MagicMock(is_authenticated=True, id=10))
        from services.stock_service import StockService
...
class TestTransferStock:
    ...
    def test_insufficient_stock(self, mocker):
        ...
        wh_q = _patch_warehouse_query(mocker)
        wh_q.filter_by.return_value.first.side_effect = [from_wh, to_wh]
        mocker.patch('services.stock_service.StockService.get_product_stock', return_value=Decimal('0'))
        from services.stock_service import StockService
    ...
    def test_happy_path(self, mocker):
        ...
        wh_q = _patch_warehouse_query(mocker)
        wh_q.filter_by.return_value.first.side_effect = [from_wh, to_wh]
        mocker.patch('services.stock_service.StockService.get_product_stock', return_value=Decimal('20'))
...
class TestCalculateSaleCogs:
    def test_mwac_positive_stock(self, mocker, app):
        pwc = _pwc(total_quantity=Decimal('100'), total_value=Decimal('5000'), average_cost=Decimal('50'))
        pwc_q = MagicMock()
    ...
    def test_negative_inventory_existing_pwc(self, mocker, app):
        pwc = _pwc(total_quantity=Decimal('-1'), total_value=Decimal('-50'), average_cost=Decimal('50'))
        pwc_q = MagicMock()
        pwc_q.filter_by.return_value.with_for_update.return_value.first.return_value = pwc
        mocker.patch('services.stock_service.ProductWarehouseCost.query', pwc_q)
        mocker.patch('services.stock_service.ProductCostHistory')
...
class TestUpdateWacOnReceipt:
    def test_first_receipt_creates_pwc(self, mocker):
        ...
        StockService._update_wac_on_receipt(
            ...
            received_qty=Decimal('10'), unit_cost_aed=Decimal('5'),
            reference_type=GLRef.PURCHASE, reference_id=1,
        )
        session.add.assert_called()

    def test_existing_pwc_updated(self, mocker):
        pwc = _pwc(total_quantity=Decimal('10'), total_value=Decimal('100'), average_cost=Decimal('10'))
        pwc_q = MagicMock()
        pwc_q.filter_by.return_value.with_for_update.return_value.first.return_value = pwc
        mocker.patch('services.stock_service.ProductWarehouseCost.query', pwc_q)
        mocker.patch('services.stock_service.ProductCostHistory')
        ...
        StockService._update_wac_on_receipt(
            tenant_id=1, product_id=1, warehouse_id=5,
            received_qty=Decimal('10'), unit_cost_aed=Decimal('12'),
            reference_type=GLRef.PURCHASE, reference_id=2,
        )
        assert pwc.total_quantity == Decimal('20')

    def test_negative_stock_retrospective(self, mocker):
        pwc = _pwc(total_quantity=Decimal('-2'), total_value=Decimal('-20'), average_cost=Decimal('10'))
        pwc_q = MagicMock()
        pwc_q.filter_by.return_value.with_for_update.return_value.first.return_value = pwc
        mocker.patch('services.stock_service.ProductWarehouseCost.query', pwc_q)
        mocker.patch('services.stock_service.ProductCostHistory')
        ...
        StockService._update_wac_on_receipt(
            tenant_id=1, product_id=1, warehouse_id=5,
            received_qty=Decimal('5'), unit_cost_aed=Decimal('12'),
            reference_type=GLRef.PURCHASE, reference_id=7,
        )
...
class TestPostRetrospectiveCostAdjustment:
    def test_skips_non_negative_qty(self, mocker):
        ...
        assert StockService._post_retrospective_cost_adjustment(
            1, 1, 5, Decimal('5'), Decimal('10'), Decimal('12'), Decimal('5'),
            GLRef.PURCHASE, 1,
        ) == Decimal('0')
    ...
    def test_skips_invalid_old_avg(self, mocker):
        ...
        assert StockService._post_retrospective_cost_adjustment(
            1, 1, 5, Decimal('-2'), None, Decimal('12'), Decimal('5'),
            GLRef.PURCHASE, 1,
        ) == Decimal('0')
    ...
    def test_skips_invalid_unit_cost(self, mocker):
        ...
        assert StockService._post_retrospective_cost_adjustment(
            1, 1, 5, Decimal('-2'), Decimal('10'), Decimal('0'), Decimal('5'),
            GLRef.PURCHASE, 1,
        ) == Decimal('0')
    ...
    def test_skips_near_zero_variance(self, mocker):
        ...
        assert StockService._post_retrospective_cost_adjustment(
            1, 1, 5, Decimal('-2'), Decimal('10'), Decimal('10'), Decimal('5'),
            GLRef.PURCHASE, 1,
        ) == Decimal('0')
    ...
    def test_loss_variance(self, mocker):
        ...
        result = StockService._post_retrospective_cost_adjustment(
            1, 1, 5, Decimal('-5'), Decimal('10'), Decimal('15'), Decimal('10'),
            GLRef.PURCHASE, 3,
        )
    ...
    def test_gain_variance(self, mocker):
        ...
        result = StockService._post_retrospective_cost_adjustment(
            1, 1, 5, Decimal('-5'), Decimal('20'), Decimal('10'), Decimal('10'),
            GLRef.PURCHASE, 4,
        )
        assert result < Decimal('0')
    ...
    def test_post_failure_logged(self, mocker, app):
        ...
        with app.app_context():
            result = StockService._post_retrospective_cost_adjustment(
                1, 1, 5, Decimal('-2'), Decimal('10'), Decimal('15'), Decimal('5'),
                GLRef.PURCHASE, 5,
            )
        assert result > Decimal('0')
...
class TestReverseSale:
    ...
    def test_mwac_with_history(self, mocker, app):
        mocker.patch('services.stock_service.StockService.add_stock')
        pwc = _pwc(total_quantity=Decimal('8'), total_value=Decimal('400'), average_cost=Decimal('50'))
        pwc_q = MagicMock()
        pwc_q.filter_by.return_value.with_for_update.return_value.first.return_value = pwc
        mocker.patch('services.stock_service.ProductWarehouseCost.query', pwc_q)
        history = MagicMock()
    ...
    def test_mwac_without_history(self, mocker, app):
        mocker.patch('services.stock_service.StockService.add_stock')
        pwc = _pwc(total_quantity=Decimal('5'), total_value=Decimal('250'), average_cost=Decimal('50'))
        pwc_q = MagicMock()
        pwc_q.filter_by.return_value.with_for_update.return_value.first.return_value = pwc
        mocker.patch('services.stock_service.ProductWarehouseCost.query', pwc_q)
        pch_q = MagicMock()
...
class TestReversePurchase:
    ...
    def test_mwac_clamps_negative_qty(self, mocker, app):
        mocker.patch('services.stock_service.StockService.remove_stock')
        pwc = _pwc(total_quantity=Decimal('1'), total_value=Decimal('10'), average_cost=Decimal('10'))
        pwc_q = MagicMock()
        pwc_q.filter_by.return_value.with_for_update.return_value.first.return_value = pwc
        mocker.patch('services.stock_service.ProductWarehouseCost.query', pwc_q)
        history = MagicMock()
    ...
    def test_without_cost_history_uses_pwc_avg(self, mocker, app):
        mocker.patch('services.stock_service.StockService.remove_stock')
        pwc = _pwc(total_quantity=Decimal('10'), total_value=Decimal('100'), average_cost=Decimal('12'))
        pwc_q = MagicMock()
        pwc_q.filter_by.return_value.with_for_update.return_value.first.return_value = pwc
        mocker.patch('services.stock_service.ProductWarehouseCost.query', pwc_q)
        pch_q = MagicMock()
...
class TestAvailability:
    ...
    def test_check_in_warehouse_insufficient(self, mocker):
        ...
        mocker.patch('services.stock_service.StockService.get_product_stock', return_value=Decimal('1'))
        from services.stock_service import StockService
    ...
    def test_check_in_warehouse_success(self, mocker):
        mocker.patch('services.stock_service.db.session').get.return_value = _product()
        _patch_warehouse_query(mocker, warehouse=_warehouse())
        mocker.patch('services.stock_service.StockService.get_product_stock', return_value=Decimal('10'))
        from services.stock_service import StockService
...
class TestInventoryQueries:
    def test_get_product_stock_by_warehouse(self, mocker):
        mocker.patch('services.stock_service.get_branch_stock_map', return_value={1: Decimal('42')})
        from services.stock_service import StockService
    ...
    def test_get_product_stock_with_user(self, mocker):
        ...
        mocker.patch('services.stock_service.get_branch_stock_map', return_value={1: Decimal('3')})
        from services.stock_service import StockService
    ...
    def test_get_low_stock_with_warehouses(self, mocker):
        ...
        mocker.patch('services.stock_service.StockService.get_visible_products_query', return_value=query)
        mocker.patch('services.stock_service.get_accessible_warehouse_ids', return_value=[1])
    ...
    def test_get_low_stock_no_access(self, mocker):
        mocker.patch('services.stock_service.StockService.get_visible_products_query', return_value=MagicMock())
...
class TestReconcileStock:
    def _setup_reconcile_queries(self, mocker, *, existing_rows, movement_rows, pws_sum_rows, products):
        session = mocker.patch('services.stock_service.db.session')
