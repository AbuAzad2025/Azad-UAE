...
def test_store_order_webhook_branches(db_session, sample_tenant, sample_customer, sample_user, sample_warehouse):
    ...
    with (
        ...
        _patch("services.store_order_service.StoreOrderService.confirm_order", return_value=sale),
        _patch("services.store_order_service.StoreOrderService.cancel_order", return_value=sale),
    ):
        out = WebhookService._process_store_order_webhook(
            {"payment_id": "gw-1", "payment_status": "finished", "order_id": f"STORE_{sale.id}_{sample_tenant.id}"}
        )
        assert out["success"] is True
        out = WebhookService._process_store_order_webhook(
