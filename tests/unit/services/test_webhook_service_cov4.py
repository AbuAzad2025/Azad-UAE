...
def test_purchase_webhook_finished_failed_idempotent(db_session, sample_tenant):
    ...
    out = WebhookService._process_purchase_webhook(
        {"payment_id": "cov4-pay-1", "payment_status": "finished"})
    ...
    out = WebhookService._process_purchase_webhook(
        {"payment_id": "cov4-pay-1", "payment_status": "expired"})
...
def test_donation_webhook_finished_failed_idempotent(db_session):
    ...
    d = Donation(amount_usd=Decimal("10"), payment_method="crypto", status="pending",
                 donor_name="D", transaction_hash="cov4-tx-1")
    db_session.add(d)
    db_session.flush()
    out = WebhookService._process_donation_webhook(
        {"payment_id": "cov4-tx-1", "payment_status": "finished"})
    assert out["success"] is True
    ...
    db_session.flush()
    out = WebhookService._process_donation_webhook(
        {"payment_id": "cov4-tx-1", "payment_status": "failed"})
    assert d.status == "failed"
    # gateway_transaction_id lookup branch
    d2 = Donation(amount_usd=Decimal("5"), payment_method="card", status="pending",
                  gateway_transaction_id="cov4-gw-2")
    db_session.add(d2)
    db_session.flush()
    out = WebhookService._process_donation_webhook(
        {"payment_id": "cov4-gw-2", "payment_status": "finished"})
    assert d2.status == "completed"


def test_store_order_webhook_branches(db_session, sample_tenant, sample_customer,
    ...
    sale = Sale(tenant_id=sample_tenant.id, sale_number="STORE-COV4-WH",
                customer_id=sample_customer.id, seller_id=sample_user.id,
                warehouse_id=sample_warehouse.id, sale_date=dt.now(),
                source="online_store", status="pending",
                subtotal=Decimal("10"), total_amount=Decimal("10"), amount=Decimal("10"),
                amount_aed=Decimal("10"))
    db_session.add(sale)
    db_session.flush()
    with _patch("services.store_online_payment_service.StoreOnlinePaymentService.parse_store_order_id",
                return_value=(sale.id, sample_tenant.id)), \
         _patch("services.store_order_service.StoreOrderService.confirm_order",
                return_value=sale), \
         _patch("services.store_order_service.StoreOrderService.cancel_order",
                return_value=sale):
        out = WebhookService._process_store_order_webhook(
            {"payment_id": "gw-1", "payment_status": "finished",
             "order_id": f"STORE_{sale.id}_{sample_tenant.id}"})
        assert out["success"] is True
        out = WebhookService._process_store_order_webhook(
            {"payment_id": "gw-1", "payment_status": "refunded",
             "order_id": f"STORE_{sale.id}_{sample_tenant.id}"})
        assert out["success"] is True
...
def test_stripe_signatures_and_dispatch():
    ...
    out = WebhookService.process_stripe_webhook(
        {"type": "payment_intent.succeeded",
         "data": {"object": {"amount": 2500, "receipt_email": "a@b.c"}}})
    assert out == {"success": True, "message": "Payment processed"}
    out = WebhookService.process_stripe_webhook(
        {"type": "payment_intent.payment_failed",
         "data": {"object": {"receipt_email": "a@b.c",
