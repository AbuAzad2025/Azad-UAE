...
class TestRepairAccountingData:
    def _base_patches(self, mocker, tenant_id=1):
        ...
        mocker.patch('services.gl_helpers.get_account', side_effect=lambda code, tid: inv_acct if code == '1140' else eq_acct)
        mocker.patch('models.gl.GLJournalLine').query.filter.return_value.all.return_value = []
        product = MagicMock(current_stock=10, cost_price=Decimal('5'), merchant_customer_id=None, tenant_id=tenant_id)
        Product = MagicMock()
    ...
    def test_creates_merchant_when_missing(self, mocker):
        ...
        mocker.patch('services.gl_helpers.get_account', side_effect=lambda c, t: MagicMock(id=1))
        mocker.patch('models.gl.GLJournalLine').query.filter.return_value.all.return_value = []
    ...
    def test_backfills_legacy_cheque_entries(self, app, mocker):
        ...
        entry = MagicMock(description='استلام شيك وارد رقم CH-001', reference_type=None, reference_id=None)
        mocker.patch('models.gl.GLJournalEntry').query.filter.return_value.all.return_value = [entry]
        cheque = MagicMock(id=42)
        mocker.patch('models.Cheque').query.filter.return_value.first.return_value = cheque
        result = repair_accounting_data()
    ...
    def test_skips_entries_without_cheque_match(self, app, mocker):
        ...
        mocker.patch('models.gl.GLJournalEntry').query.filter.return_value.all.return_value = [entry]
        mocker.patch('models.Cheque').query.filter.return_value.first.return_value = None
        result = repair_accounting_data()
    ...
    def test_posts_positive_inventory_adjustment(self, app, mocker):
        self._base_patches(mocker)
        tenant_q = MagicMock()
        tenant_q.all.return_value = [MagicMock(current_stock=100, cost_price=Decimal('2'))]
        mocker.patch('utils.tenant_orm.tenant_query', return_value=tenant_q)
    ...
    def test_posts_negative_inventory_adjustment(self, app, mocker):
        ...
        mocker.patch('models.gl.GLJournalLine').query.filter.return_value.all.return_value = [line]
        tenant_q = MagicMock()
        tenant_q.all.return_value = [MagicMock(current_stock=10, cost_price=Decimal('10'))]
        mocker.patch('utils.tenant_orm.tenant_query', return_value=tenant_q)
        post = mocker.patch('services.gl_service.GLService.post_entry')
        repair_accounting_data()
        post.assert_called_once()
    ...
    def test_skips_migration_when_existing(self, app, mocker):
        self._base_patches(mocker)
        mocker.patch('models.gl.GLJournalEntry').query.filter_by.return_value.first.return_value = MagicMock()
        post = mocker.patch('services.gl_service.GLService.post_entry')
        result = repair_accounting_data()
    ...
    def test_raises_when_no_tenant_for_inventory(self, app, mocker):
        ...
        Tenant.query.filter_by.return_value.order_by.return_value.first.return_value = None
        mocker.patch('services.gl_service.GLService.ensure_core_accounts')
        mocker.patch('models.Customer').query.filter_by.return_value.order_by.return_value.first.return_value = MagicMock(id=1)
    ...
    def test_resolves_tenant_from_default(self, mocker):
        ...
        Tenant = mocker.patch('models.Tenant')
        Tenant.query.filter_by.return_value.order_by.return_value.first.return_value = tenant
