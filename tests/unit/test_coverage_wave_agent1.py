...
class TestCreateUserValidation:
    ...
    def test_create_user_missing_username_password(self):
        ...
        user = MagicMock(is_authenticated=True, is_owner=True, tenant_id=1,
                         full_name='Owner', has_permission=MagicMock(return_value=True))
...
class TestAutoRetrainingThreshold:
    def test_should_retrain_100_increment(self, app, tmp_path):
        with app.app_context():
            ...
            with patch('ai_knowledge.learning.auto_retraining.os.path.exists', return_value=True), \
