...
def test_landing_contains_banner():
    ...
    mock_tenant.name_en = 'Demo Store'
    mock_tenant.id = 1
    with patch('services.user_service.UserService.active_tenants', return_value=[mock_tenant]), patch('services.store_service.StoreService.get_store_by_host', return_value=None):
...
def test_shop_visitor_browsing_no_login():
    ...
    with patch('services.store_service.StoreService.get_store_by_slug', return_value=mock_store), patch('services.store_service.StoreService.get_store_by_host', return_value=None), patch('services.store_service.StoreService.stores_globally_enabled', return_value=True), patch('services.store_service.StoreService.is_platform_locked', return_value=False), patch('services.store_service.StoreService.get_public_catalog', return_value={"items": [], "categories": [], "page": 1, "pages": 1, "total": 0}), patch('services.store_service.StoreService.get_catalog_products', return_value=([], {})), patch('services.store_service.StoreService.active_categories', return_value=[]):
...
def test_demo_users_all_roles():
    ...
    role_names = ['Owner','Super Admin','Manager','Seller','Cashier','General Accountant','Branch Manager','Kitchen Staff']
