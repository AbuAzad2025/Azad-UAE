...
class TestApiUpdateTenantSettings:
    ...
    def test_missing_json_returns_400(self, company_admin_client):
        resp = company_admin_client.post(
            "/owner/api/update-tenant-settings", data={},
    ...
    def test_tenant_not_found_returns_404(
        ...
        Tenant.query.get.return_value = None
        resp = company_admin_client.post(
            "/owner/api/update-tenant-settings",
    ...
    def test_success_default_tax_rate(
        ...
        Tenant = model_patch("routes.owner.Tenant")
        inst = MagicMock()
        Tenant.query.get.return_value = inst
    ...
    def test_exception_path_returns_500(
        self, company_admin_client, mocker, model_patch,
    ):
        Tenant = model_patch("routes.owner.Tenant")
        Tenant.query.get.side_effect = RuntimeError("DB exploded")
        mocker.patch("routes.owner._invalidate_owner_changes")
        mocker.patch("routes.owner._audit_owner_db_action")
...
class TestApiTenantToggleStatus:
    ...
    def test_missing_json_returns_400(self, owner_client):
        resp = owner_client.post(
            "/owner/api/tenant/2/toggle-status", data={},
            content_type="application/x-www-form-urlencoded",
        )
        assert resp.status_code == 400
        assert resp.json["error"] == "JSON required"
    ...
    def test_exception_path_returns_500(
        self, owner_client, mocker, model_patch,
    ):
        Tenant = model_patch("routes.owner.Tenant")
        Tenant.query.get.side_effect = RuntimeError("DB exploded")
        mocker.patch("routes.owner._invalidate_owner_changes")
...
class TestApiTenantUpdatePackage:
    ...
    allowed = [
        ...
        "max_branches", "max_warehouses", "max_invoices_per_month",
        "max_sales_per_month",
    ]

    def test_missing_json_returns_400(self, owner_client):
        resp = owner_client.post(
            "/owner/api/tenant/2/update-package", data={},
            content_type="application/x-www-form-urlencoded",
        )
        assert resp.status_code == 400
        assert resp.json["error"] == "JSON required"
