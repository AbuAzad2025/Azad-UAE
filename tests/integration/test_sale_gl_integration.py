...
class TestSaleGlIntegration:
    def test_sale_creates_balanced_gl_entries(self, app, db_session):
        ...
        tenant = Tenant(
            name=f'Test {tid}', name_ar=f'Test {tid}', slug=f'test-{tid}',
        ...
        wh = Warehouse(
            tenant_id=tenant.id, name=f'WH {tid}',
            branch_id=branch.id, allow_negative_inventory=True,
        )
        db_session.add(wh)
        db_session.flush()
        ...
        seller = User(
            tenant_id=tenant.id, username=f'seller_{tid}',
            email=f'seller_{tid}@test.com', is_active=True,
            password_hash='fakehash', branch_id=branch.id,
            role_id=role.id,
        )
        ...
        product = Product(
            tenant_id=tenant.id, name=f'Prod {tid}', name_ar=f'Prod {tid}',
            sku=f'SKU-{tid}', cost_price=Decimal('50'), regular_price=Decimal('100'),
        ...
        lines = [{
            ...
            'discount_percent': 0, 'unit_price': 100.00,
            'serials': [],
        }]
        ...
        gl_entry = GLJournalEntry.query.filter_by(
            reference_type=GLRef.SALE, reference_id=sale.id,
            tenant_id=tenant.id,
        ).first()
        ...
        cogs_entry = GLJournalEntry.query.filter_by(
            reference_type=GLRef.SALE_COGS, reference_id=sale.id,
            tenant_id=tenant.id,
        ).first()
        ...
        movements = StockMovement.query.filter_by(
            product_id=product.id, movement_type='sale',
            reference_type=GLRef.SALE, reference_id=sale.id,
        ).all()
