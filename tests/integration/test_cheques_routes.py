...
class TestChequesCreate:
    def test_create_incoming_cheque_gl_balanced(self, app, db_session, client):
        ...
        user = User(username=f'chqu-{tid}', email=f'chqu-{tid}@t.com',
                    full_name='CHQ User', role_id=role.id,
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-{tid}',
        ...
        with client:
            resp = client.post('/auth/login', data={'username': user.username, 'password': 'x'},
        ...
        lines = GLJournalLine.query.join(
    ...
    def test_create_outgoing_cheque_gl_balanced(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqu2-{tid}', email=f'chqu2-{tid}@t.com',
                    full_name='CHQ User 2', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()

        supplier = Supplier(tenant_id=tenant.id, name=f'S {tid}',
                           total_purchases_aed=Decimal('0'), total_paid_aed=Decimal('0'))
        ...
        purchase = Purchase(tenant_id=tenant.id, branch_id=branch.id, supplier_id=supplier.id,
                           ...
                           user_id=user.id, purchase_number=f'P-{tid}',
...
class TestChequesLifecycle:
    def test_incoming_cheque_full_lifecycle_gl_balanced(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqu3-{tid}', email=f'chqu3-{tid}@t.com',
                    full_name='CHQ User 3', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S3-{tid}',
                    total_amount=Decimal('8000'), amount=Decimal('8000'),
        ...
        with client:
            resp = client.post(f'/cheques/{cheque_id}/deposit', data={
        ...
        with client:
            resp = client.post(f'/cheques/{cheque_id}/clear', data={
    ...
    def test_incoming_cheque_bounce_reverses_gl(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqu4-{tid}', email=f'chqu4-{tid}@t.com',
                    full_name='CHQ User 4', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S4-{tid}',
        ...
        with client:
            resp = client.post(f'/cheques/{cheque.id}/deposit', data={
                'deposit_date': today_str,
            }, follow_redirects=False)
        assert resp.status_code in (200, 302)

        with client:
            resp = client.post(f'/cheques/{cheque.id}/bounce', data={
        ...
        accounts = [(l.account.code, l.debit or 0, l.credit or 0) for l in lines]
        has_ar_debit = any(
            '1130' in str(acc) and d > 0
            for acc, d, c in accounts
        )
        has_cuc_credit = any(
            '1150' in str(acc) and c > 0
            for acc, d, c in accounts
        )
...
class TestChequesViewEdit:
    def test_cheque_list_page_renders(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqv-{tid}', email=f'chqv-{tid}@t.com',
                    full_name='CHQ View', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
    ...
    def test_cheque_create_page_renders(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqc-{tid}', email=f'chqc-{tid}@t.com',
                    full_name='CHQ Create', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.commit()
    ...
    def test_cheque_view_page_renders(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqvw-{tid}', email=f'chqvw-{tid}@t.com',
                    full_name='CHQ View2', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        cheque = Cheque(
            tenant_id=tenant.id, branch_id=branch.id, user_id=user.id,
            cheque_number=f'CHQ-{tid}', cheque_bank_number=f'BNK-V-{tid}',
            cheque_type='incoming', bank_name='Test Bank',
            amount=Decimal('1000.00'), currency='AED', exchange_rate=Decimal('1'),
            amount_aed=Decimal('1000.00'),
            issue_date=date.today(), due_date=date.today() + timedelta(days=30),
            drawer_name='John', status='pending',
        )
        db_session.add(cheque); db_session.commit()
    ...
    def test_edit_cheque_updates_cheque(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqe-{tid}', email=f'chqe-{tid}@t.com',
                    full_name='CHQ Edit', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        cheque = Cheque(
            tenant_id=tenant.id, branch_id=branch.id, user_id=user.id,
            cheque_number=f'CHQ-E-{tid}', cheque_bank_number=f'BNK-E-{tid}',
            cheque_type='incoming', bank_name='Old Bank',
            amount=Decimal('1000.00'), currency='AED', exchange_rate=Decimal('1'),
            amount_aed=Decimal('1000.00'),
            issue_date=date.today(), due_date=date.today() + timedelta(days=30),
            drawer_name='Old', status='pending',
        )
        db_session.add(cheque); db_session.commit()
        ...
        with client:
            client.post('/auth/login', data={'username': user.username, 'password': 'x'},
                        follow_redirects=True)
            resp = client.post(f'/cheques/{cheque.id}/edit', data={
...
class TestChequesDelete:
    def test_archive_cheque_with_links_soft_deletes(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqdel-{tid}', email=f'chqdel-{tid}@t.com',
                    full_name='CHQ Del', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        with client:
            resp = client.post(f'/cheques/{cheque.id}/delete', data={
...
class TestChequesApi:
    def test_api_stats_returns_json(self, app, db_session, client):
        ...
        role = Role(name=f'R {tid}', slug=f'r-{tid}', is_active=True)
        db_session.add(role); db_session.flush()
        user = User(username=f'chqapi-{tid}', email=f'chqapi-{tid}@t.com',
                    full_name='CHQ API', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.commit()
