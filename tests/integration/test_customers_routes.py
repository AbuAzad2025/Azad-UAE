...
class TestCustomerBranchIsolation:
    def test_customer_branch_isolation(self, app, db_session, client):
        ...
        u1 = User(username=f'u1-{tid}', email=f'u1-{tid}@t.com', full_name='User 1',
                  role_id=role.id, tenant_id=tenant.id, branch_id=b1.id,
                  is_active=True, is_owner=True)
        u1.set_password('x')
        db_session.add(u1); db_session.flush()

        u2 = User(username=f'u2-{tid}', email=f'u2-{tid}@t.com', full_name='User 2',
        ...
        db_session.add(cust_b1); db_session.flush()
        db_session.add(cust_b2); db_session.flush()

        GLService.ensure_core_accounts(tenant_id=tenant.id)
        db_session.commit()

        sale_b1 = Sale(tenant_id=tenant.id, branch_id=b1.id, customer_id=cust_b1.id,
                       seller_id=u1.id, sale_number=f'S-N-{tid}',
                       total_amount=Decimal('100'), amount=Decimal('100'),
                       amount_aed=Decimal('100'))
        db_session.add(sale_b1)

        sale_b2 = Sale(tenant_id=tenant.id, branch_id=b2.id, customer_id=cust_b2.id,
                       seller_id=u2.id, sale_number=f'S-S-{tid}',
                       total_amount=Decimal('200'), amount=Decimal('200'),
    ...
    def test_customer_branch_isolation_reverse(self, app, db_session, client):
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=b1.id, customer_id=cust.id,
                    seller_id=u1.id, sale_number=f'S-L-{tid}',
                    total_amount=Decimal('150'), amount=Decimal('150'),
...
class TestCustomerStatement:
    def test_statement_shows_sales_receipts(self, app, db_session, client):
        ...
        user = User(username=f'acc-{tid}', email=f'acc-{tid}@t.com', full_name='Acc',
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-STMT-{tid}',
        ...
        receipt = Receipt(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                          receipt_number=f'R-STMT-{tid}',
                          amount=Decimal('300'), amount_aed=Decimal('300'),
                          payment_method='cash', direction='incoming',
                          receipt_date=now, user_id=user.id)
    ...
    def test_statement_shows_payments_with_cheques(self, app, db_session, client):
        ...
        user = User(username=f'acc2-{tid}', email=f'acc2-{tid}@t.com', full_name='Acc2',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S2-STMT-{tid}',
                    total_amount=Decimal('1000'), amount=Decimal('1000'),
                    amount_aed=Decimal('1000'))
        db_session.add(sale)
        db_session.commit()

        cheque = Cheque(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                        cheque_number=f'CH-STMT-{tid}', cheque_bank_number=f'BNK-STMT-{tid}',
                        cheque_type='incoming', bank_name='Test Bank',
                        amount=Decimal('200'), amount_aed=Decimal('200'),
                        currency='AED', status='pending',
                        issue_date=date.today(), due_date=date.today() + timedelta(days=30))
        db_session.add(cheque)
        db_session.commit()

        payment = Payment(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                          user_id=user.id, sale_id=sale.id,
                          payment_number=f'P-STMT-{tid}', cheque_id=cheque.id,
                          amount=Decimal('200'), amount_aed=Decimal('200'),
                          currency='AED', payment_method='cheque', payment_type='sale_payment',
                          direction='incoming', payment_confirmed=True)
...
class TestCustomerDelete:
    def test_delete_customer_with_sales_soft_deletes(self, app, db_session, client):
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-DEL-{tid}',
                    total_amount=Decimal('100'), amount=Decimal('100'),
                    amount_aed=Decimal('100'))
        db_session.add(sale)
        db_session.commit()
...
class TestCustomerView:
    def test_customer_view_shows_balance_and_unpaid_sales(self, app, db_session, client):
        ...
        user = User(username=f'vw-{tid}', email=f'vw-{tid}@t.com', full_name='Vw',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-VW-{tid}',
                    total_amount=Decimal('500'), amount=Decimal('500'),
                    amount_aed=Decimal('500'), status='confirmed')
        db_session.add(sale)
        db_session.commit()
...
class TestCustomerSearch:
    def test_customer_search_by_name(self, app, db_session, client):
        ...
        user = User(username=f'sch-{tid}', email=f'sch-{tid}@t.com', full_name='Sch',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        db_session.add(cust_a); db_session.flush()
        db_session.add(cust_b); db_session.flush()

        GLService.ensure_core_accounts(tenant_id=tenant.id)
        db_session.commit()

        sale_a = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=cust_a.id,
                      seller_id=user.id, sale_number=f'S-SA-{tid}',
                      total_amount=Decimal('10'), amount=Decimal('10'),
                      amount_aed=Decimal('10'))
        sale_b = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=cust_b.id,
                      seller_id=user.id, sale_number=f'S-SB-{tid}',
                      total_amount=Decimal('20'), amount=Decimal('20'),
                      amount_aed=Decimal('20'))
        db_session.add(sale_a); db_session.add(sale_b)
        db_session.commit()
...
class TestReceivablesReport:
    def test_receivables_report_aging_buckets(self, app, db_session, client):
        ...
        user = User(username=f'recv-{tid}', email=f'recv-{tid}@t.com', full_name='Recv',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale_old = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                        seller_id=user.id, sale_number=f'S-OLD-{tid}',
                        total_amount=Decimal('1000'), amount=Decimal('1000'),
                        amount_aed=Decimal('1000'),
                        sale_date=old_date)
    ...
    def test_receivables_report_totals_accurate(self, app, db_session, client):
        ...
        user = User(username=f'rec2-{tid}', email=f'rec2-{tid}@t.com', full_name='Rec2',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-REC2-{tid}',
                    total_amount=Decimal('500'), amount=Decimal('500'),
        ...
        payment = Payment(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                          ...
                          amount=Decimal('200'), amount_aed=Decimal('200'),
                          currency='AED', payment_method='cash', payment_type='sale_payment',
                          direction='incoming', payment_confirmed=True)
        db_session.add(payment)
        db_session.commit()
    ...
    def test_receivables_defaults_to_all_confirmed_sales(self, app, db_session, client):
        ...
        user = User(username=f'rec3-{tid}', email=f'rec3-{tid}@t.com', full_name='Rec3',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-REC3-{tid}',
                    total_amount=Decimal('300'), amount=Decimal('300'),
...
class TestCustomerApi:
    def test_api_search_returns_json(self, app, db_session, client):
        ...
        user = User(username=f'api-{tid}', email=f'api-{tid}@t.com', full_name='API',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-API-{tid}',
                    total_amount=Decimal('50'), amount=Decimal('50'),
    ...
    def test_api_balance_returns_json(self, app, db_session, client):
        ...
        user = User(username=f'bal-{tid}', email=f'bal-{tid}@t.com', full_name='Bal',
                    role_id=role.id, tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        db_session.add(user); db_session.flush()
        ...
        sale = Sale(tenant_id=tenant.id, branch_id=branch.id, customer_id=customer.id,
                    seller_id=user.id, sale_number=f'S-BAL-{tid}',
                    total_amount=Decimal('750'), amount=Decimal('750'),
