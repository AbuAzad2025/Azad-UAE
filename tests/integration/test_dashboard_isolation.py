...
def _make_tenant(db_session, name, slug, currency='ILS', piv=False, tax_rate=Decimal('5.00')):
    ...
    t = Tenant(
        ...
        default_currency=currency, base_currency=currency,
        prices_include_vat=piv, default_tax_rate=tax_rate,
    )
    db_session.add(t)
    db_session.flush()
    return t
...
def _make_branch(db_session, tenant_id, name='Main'):
    ...
    suffix = str(uuid.uuid4())[:8]
    b = Branch(
        tenant_id=tenant_id, name=f'{name}_{suffix}',
        code=f'BR{suffix[:4].upper()}', is_active=True,
    )
    db_session.add(b)
    db_session.flush()
    return b
...
def _make_warehouse(db_session, tenant_id, branch_id, allow_neg=False):
    ...
    suffix = str(uuid.uuid4())[:8]
    w = Warehouse(
        tenant_id=tenant_id, name=f'WH_{suffix}',
        branch_id=branch_id, allow_negative_inventory=allow_neg,
    )
    db_session.add(w)
    db_session.flush()
    return w
...
def _make_user(db_session, tenant_id, branch_id=None, role_slug='seller', is_owner=False):
    ...
    if not role:
        ...
        db_session.flush()
    u = User(
        tenant_id=tenant_id, username=f'test_{suffix}',
        email=f'{suffix}@test.com', is_active=True,
        password_hash='fakehash', branch_id=branch_id,
        role_id=role.id, is_owner=is_owner,
    )
    db_session.add(u)
    db_session.flush()
    return u
...
def _make_sale(db_session, tenant_id, branch_id, user_id, amount=Decimal('100.000')):
    ...
    s = Sale(
        tenant_id=tenant_id, branch_id=branch_id, customer_id=c.id,
        sale_number=f'SAL-{uuid.uuid4().hex[:8].upper()}',
        seller_id=user_id, sale_date=datetime.now(timezone.utc),
        subtotal=amount, total_amount=amount,
        amount=amount, amount_aed=amount,
        paid_amount=Decimal('0'), balance_due=amount,
        currency='ILS', status='confirmed',
    )
    db_session.add(s)
    db_session.flush()
    return s
...
def _make_sale_line(db_session, tenant_id, sale_id, product_id, qty=1, price=Decimal('100.000'), cost=Decimal('50.000')):
    from models import SaleLine
    sl = SaleLine(
        tenant_id=tenant_id, sale_id=sale_id, product_id=product_id,
        quantity=qty, unit_price=price, line_total=price * qty, cost_price=cost,
    )
    db_session.add(sl)
    db_session.flush()
    return sl
...
def _make_manager_user(db_session, tenant_id, branch_id):
    ...
    if not role:
        ...
        db_session.flush()
    u = User(
        tenant_id=tenant_id, username=f'mgr_{suffix}', email=f'mgr_{suffix}@test.com',
        is_active=True, password_hash='fakehash', branch_id=branch_id,
...
def _make_product(db_session, tenant_id, name='Test', cost=None):
    ...
    p = Product(
        tenant_id=tenant_id, name=f'{name}_{suffix}', sku=f'SKU-{suffix}',
        cost_price=cost_price, regular_price=Decimal('100.000'),
...
class TestTenantDashboardStats:
    ...
    def test_month_cogs_computed(self, db_session):
        ...
        _make_sale_line(db_session, t.id, s.id, p.id, qty=2, price=Decimal('100.000'), cost=Decimal('50.000'))
    ...
    def test_net_profit_equals_sales_minus_cogs_minus_commissions(self, db_session):
        ...
        _make_sale_line(db_session, t.id, s.id, p.id, qty=3, price=Decimal('50.000'), cost=Decimal('30.000'))
        ctx = build_company_dashboard_context(t.id)
...
class TestDashboardPermissions:
    ...
    def test_cashier_cannot_apply_discount(self, db_session):
        ...
        suffix = str(uuid.uuid4())[:8]
        t = Tenant(name=f'CTenant_{suffix}', name_ar=f'CTenant_{suffix}', slug=f'ctenant_{suffix}')
        db_session.add(t)
        db_session.flush()
        ...
        db_session.flush()
        u = User(
            tenant_id=t.id, username=f'cashier_{suffix}', email=f'cashier_{suffix}@test.com',
            is_active=True, password_hash='fakehash',
            role_id=role.id, is_owner=False,
        )
    ...
    def test_cashier_edit_price_false(self, db_session):
        ...
        db_session.flush()
        u = User(
            tenant_id=t.id, username=f'cedp_{suffix}', email=f'cedp_{suffix}@test.com',
            is_active=True, password_hash='fakehash',
            role_id=role.id, is_owner=False,
        )
        assert not u.can_edit_price()
    ...
    def test_manager_can_edit_price(self, db_session):
        from models import User, Role, Tenant
        suffix = str(uuid.uuid4())[:8]
        t = Tenant(name=f'MTenant_{suffix}', name_ar=f'MTenant_{suffix}', slug=f'mtenant_{suffix}')
        db_session.add(t)
        db_session.flush()
        ...
        db_session.flush()
        u = User(
            tenant_id=t.id, username=f'mgr_{suffix}', email=f'mgr_{suffix}@test.com',
            is_active=True, password_hash='fakehash',
            role_id=role.id, is_owner=False,
        )
        db_session.add(u)
    ...
    def test_seller_cannot_apply_discount(self, db_session):
        from models import User, Role, Tenant
        suffix = str(uuid.uuid4())[:8]
        t = Tenant(name=f'STenant_{suffix}', name_ar=f'STenant_{suffix}', slug=f'stenant_{suffix}')
        db_session.add(t)
        db_session.flush()
        ...
        db_session.flush()
        u = User(
            tenant_id=t.id, username=f'seller_{suffix}', email=f'seller_{suffix}@test.com',
            is_active=True, password_hash='fakehash',
            role_id=role.id, is_owner=False,
        )
        assert not u.can_apply_discount()
...
class TestTenantSettingsAPI:
    ...
    def test_update_tax_rate(self, db_session, app):
        ...
        with app.test_client() as client:
            ...
            resp = client.post('/owner/api/update-tenant-settings', json={
                'field': 'default_tax_rate', 'value': '15.00',
            })
            assert resp.status_code == 200
            data = resp.get_json()
    ...
    def test_update_prices_include_vat(self, db_session, app):
        ...
        with app.test_client() as client:
            client.post('/auth/login', data={'username': u.username, 'password': 'password123'}, follow_redirects=True)
            resp = client.post('/owner/api/update-tenant-settings', json={
                'field': 'prices_include_vat', 'value': True,
            })
            assert resp.status_code == 200
            db_session.refresh(t)
    ...
    def test_update_logo_url(self, db_session, app):
        ...
        with app.test_client() as client:
            client.post('/auth/login', data={'username': u.username, 'password': 'password123'}, follow_redirects=True)
            resp = client.post('/owner/api/update-tenant-settings', json={
                'field': 'logo_url', 'value': 'https://example.com/logo.png',
            })
            assert resp.status_code == 200
            db_session.refresh(t)
...
class TestWarehouseNegativeToggle:
    ...
    def test_toggle_warehouse_negative(self, db_session, app):
        ...
        with app.test_client() as client:
            client.post('/auth/login', data={'username': u.username, 'password': 'password123'}, follow_redirects=True)
            resp = client.post('/owner/api/toggle-warehouse-negative', json={
                'warehouse_id': w.id,
            })
            assert resp.status_code == 200
            data = resp.get_json()
...
class TestSupervisorOverride:
    ...
    def _make_supervisor(self, db_session, tenant_id, branch_id):
        ...
        if not role:
            ...
            db_session.flush()
        u = User(
            tenant_id=tenant_id, username=f'sup_{suffix}', email=f'sup_{suffix}@test.com',
            is_active=True, password_hash='fakehash', branch_id=branch_id,
            role_id=role.id, is_owner=True,
        )
        db_session.add(u)
        db_session.flush()
        db_session.refresh(u)
    ...
    def test_supervisor_override_valid(self, db_session, app):
        ...
        db_session.commit()
        with app.test_client() as client:
            client.post('/auth/login', data={'username': cashier.username, 'password': 'cashier123'}, follow_redirects=True)
            resp = client.post('/owner/api/supervisor-override', json={
                'action': 'apply_discount', 'supervisor_id': supervisor.id, 'password': 'sup456',
            })
            assert resp.status_code == 200
            data = resp.get_json()
            assert data['success'] is True
    ...
    def test_supervisor_override_wrong_password(self, db_session, app):
        ...
        with app.test_client() as client:
            client.post('/auth/login', data={'username': cashier.username, 'password': 'cashier123'}, follow_redirects=True)
            resp = client.post('/owner/api/supervisor-override', json={
                'action': 'edit_price', 'supervisor_id': supervisor.id, 'password': 'wrongpass',
            })
    ...
    def test_supervisor_override_not_supervisor(self, db_session, app):
        ...
        db_session.commit()
        with app.test_client() as client:
            client.post('/auth/login', data={'username': cashier1.username, 'password': 'cash123'}, follow_redirects=True)
            resp = client.post('/owner/api/supervisor-override', json={
                'action': 'apply_discount', 'supervisor_id': cashier2.id, 'password': 'cash456',
            })
            assert resp.status_code == 403
...
class TestSuperAdminEndpoints:
    ...
    def _make_global_owner(self, db_session):
        ...
        if not role:
            ...
            db_session.flush()
        u = User(
            tenant_id=None, username=f'owner_{suffix}', email=f'owner_{suffix}@test.com',
            is_active=True, password_hash='fakehash',
            role_id=role.id, is_owner=True,
        )
    ...
    def test_tenant_toggle_status(self, db_session, app):
        ...
        db_session.commit()
        with app.test_client() as client:
            client.post('/auth/login', data={'username': owner.username, 'password': 'ownerpass'}, follow_redirects=True)
    ...
    def test_update_package_limits(self, db_session, app):
        ...
        with app.test_client() as client:
            client.post('/auth/login', data={'username': owner.username, 'password': 'ownerpass'}, follow_redirects=True)
            resp = client.post(f'/owner/api/tenant/{t.id}/update-package', json={
                'field': 'max_users', 'value': '50',
            })
            assert resp.status_code == 200
            data = resp.get_json()
            assert data['success'] is True
