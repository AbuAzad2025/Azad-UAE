...
class TestPaymentsListPage:
    def test_receipts_list_page_renders(self, app, db_session, client):
        ...
        tenant = Tenant(name=f'PM {tid}', name_ar=f'PM {tid}', slug=f'pm-{tid}', default_currency='AED', base_currency='AED')
        ...
        user = User(username=f'pmuser-{tid}', email=f'pm-{tid}@t.com',
        ...
        receipt = Receipt(tenant_id=tenant.id, receipt_number=f'R-{tid}',
                          direction='incoming', amount=Decimal('100'),
                          currency='AED', exchange_rate=Decimal('1'),
                          amount_aed=Decimal('100'),
                          customer_id=customer.id, payment_method='cash',
                          receipt_date=date.today(), branch_id=branch.id,
                          user_id=user.id)
        db_session.add(receipt)
        payment = Payment(tenant_id=tenant.id, payment_number=f'P-{tid}',
                          payment_type='expense', direction='outgoing',
                          branch_id=branch.id,
                          amount=Decimal('200'), currency='AED',
                          exchange_rate=Decimal('1'), amount_aed=Decimal('200'),
