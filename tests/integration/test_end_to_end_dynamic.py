...
class TestVoucherPaymentIsolation:
    ...
    def test_receipt_print_rejects_cross_tenant(self, app, db_session):
        from models import Tenant, Branch, Warehouse, Product, Customer, Receipt, Permission
    ...
    def test_payment_print_rejects_cross_branch(self, app, db_session):
        ...
        from models import Role, User, Supplier, Permission
        ...
        db_session.flush()
        suffix = str(uuid.uuid4())[:8]
        u = User(tenant_id=t.id, username=f'admin_voucher_{suffix}', email=f'admin_voucher_{suffix}@test.com', password_hash='fakehash', branch_id=b1.id, role_id=manager_role.id, is_active=True, is_owner=False)
        db_session.add(u)
        db_session.commit()
...
class TestPayrollIsolation:
    ...
    def test_payroll_process_isolated_by_tenant_and_branch(self, app, db_session):
        from models import Tenant, Branch, Employee, PayrollTransaction, Permission
    ...
    def test_payroll_accruals_post_balanced_gl(self, app, db_session):
        from models import Tenant, Branch, Employee, GLJournalEntry, GLJournalLine, PayrollTransaction, Permission
...
class TestFxGainLossAutoPosting:
    ...
    def test_fx_gain_loss_auto_posted_for_foreign_currency_receipt(self, app, db_session):
        from models import Tenant, Branch, Warehouse, Product, Customer, Sale, Receipt, GLJournalEntry, Permission
        from utils.gl_reference_types import GLRef
