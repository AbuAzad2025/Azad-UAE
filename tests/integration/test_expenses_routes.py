...
class TestExpensesCreate:
    def test_create_expense_creates_gl_entry(self, app, db_session, client):
        ...
        tenant = Tenant(name=f'EXP {tid}', name_ar=f'EXP {tid}',
                        slug=f'exp-test-{tid}', default_currency='AED', base_currency='AED')
        ...
        user = User(username=f'exp-{tid}', email=f'exp-{tid}@t.com',
                    full_name='Expenser', role_id=role.id,
        ...
        cat = ExpenseCategory(tenant_id=tenant.id, name=f'Office {tid}',
                              gl_account_code='6500', is_active=True)
        db_session.add(cat)
        db_session.commit()
