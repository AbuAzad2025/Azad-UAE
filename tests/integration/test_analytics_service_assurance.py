...
def _add_sale(
    db_session, *, tenant_id, customer_id, seller_id, amount,
    branch_id=None, sale_date=None,
):
...
class TestCustomerInsightsIntegration:
        ...
        with app.app_context():
            for i in range(55):
                customer = _add_customer(
                    ...
                    name=f'C{i}', email_suffix=f'ltv{i}',
                )
                _add_sale(
                    db_session,
            ...
            seeded = db_session.query(Customer).filter_by(
    ...
    def test_branch_filter_scopes_to_branch_sales(
        self, app, db_session, sample_tenant, sample_user, sample_branch,
    ):
        ...
        with app.app_context():
            ...
            main_customer = _add_customer(
                db_session, sample_tenant.id, name='Main Branch Cust', email_suffix='main',
            )
            other_customer = _add_customer(
                db_session, sample_tenant.id, name='Other Branch Cust', email_suffix='other',
            )
            _add_sale(
                db_session,
                tenant_id=sample_tenant.id,
...
class TestProductPerformanceIntegration:
    ...
    def test_branch_filter_limits_performance_rows(
        ...
        with app.app_context():
            ...
            assert AnalyticsService.get_product_performance(
                tenant_id=sample_tenant.id,
