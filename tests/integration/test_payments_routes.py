...
class TestPaymentsVoucher:
    def test_create_receipt_reduces_customer_balance(self, app, db_session, client):
        ...
        tenant = Tenant(name=f'P1 {tid}', name_ar=f'P1 {tid}',
                        slug=f'pay-test-{tid}', default_currency='AED', base_currency='AED')
        ...
        user = User(username=f'cash-{tid}', email=f'cash-{tid}@t.com',
                    full_name='Cashier', role_id=role.id,
        ...
        customer = Customer(tenant_id=tenant.id, name=f'Debtor {tid}', phone=f'053{tid}',
                            balance=Decimal('500'))
        db_session.add(customer)
        db_session.flush()

        sale = Sale(
            tenant_id=tenant.id, customer_id=customer.id, seller_id=user.id,
            sale_number=f'INV-{tid}', branch_id=branch.id,
            subtotal=Decimal('500'), total_amount=Decimal('500'),
            amount=Decimal('500'), amount_aed=Decimal('500'),
            paid_amount=Decimal('0'), balance_due=Decimal('500'),
    ...
    def test_create_payment_to_supplier_reduces_ap(self, app, db_session, client):
        ...
        tid = str(uuid.uuid4())[:8]
        tenant = Tenant(name=f'P2 {tid}', name_ar=f'P2 {tid}',
                        slug=f'pay-test-2-{tid}', default_currency='AED', base_currency='AED')
        db_session.add(tenant)
        db_session.flush()
        ...
        user = User(username=f'acc-{tid}', email=f'acc-{tid}@t.com',
        ...
        purchase = Purchase(
            tenant_id=tenant.id, supplier_id=supplier.id, branch_id=branch.id,
            supplier_name=f'Cred {tid}', user_id=user.id,
            purchase_number=f'PO-{tid}',
            subtotal=Decimal('1000'), total_amount=Decimal('1000'),
            amount=Decimal('1000'), amount_aed=Decimal('1000'),
            status='confirmed', currency='AED',
        )
