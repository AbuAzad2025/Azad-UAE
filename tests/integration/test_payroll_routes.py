...
class TestPayrollProcess:
    def test_process_payroll_creates_transaction_and_gl(self, app, db_session, client):
        ...
        tenant = Tenant(name=f'PR {tid}', name_ar=f'PR {tid}',
                        slug=f'payroll-test-{tid}', default_currency='AED', base_currency='AED')
        ...
        user = User(username=f'hr-{tid}', email=f'hr-{tid}@t.com',
                    full_name='HR Mgr', role_id=role.id,
        ...
        employee = Employee(
            tenant_id=tenant.id, branch_id=branch.id,
            name=f'Staff {tid}', phone=f'055{tid}',
        ...
        with client:
            ...
            try:
                resp = client.post('/payroll/process', data={
        ...
        tx = PayrollTransaction.query.filter_by(
            employee_id=employee.id, tenant_id=tenant.id).first()
