...
class TestWarehouseAddStock:
    def test_add_stock_increases_stock_and_creates_gl(self, app, db_session, client):
        from models import Tenant, Branch, User, Role, Product, Warehouse, ProductWarehouseStock
        from services.gl_service import GLService
        from models.gl import GLJournalEntry

        tid = str(uuid.uuid4())[:8]
        tenant = Tenant(name=f'WH {tid}', name_ar=f'WH {tid}',
                        slug=f'wh-test-{tid}', default_currency='AED', base_currency='AED')
        ...
        user = User(username=f'whuser-{tid}', email=f'whuser-{tid}@t.com',
                    full_name='WH User', role_id=role.id,
        ...
        product = Product(name=f'Item {tid}', sku=f'ITM-{tid}', tenant_id=tenant.id,
                          cost_price=Decimal('50'), regular_price=Decimal('100'),
        ...
        with client:
            ...
            resp = client.post(f'/warehouse/add-stock/{product.id}', data={
                'quantity': '5',
    ...
    def test_add_stock_calculates_gl_from_cost_price(self, app, db_session, client):
        ...
        tid = str(uuid.uuid4())[:8]
        tenant = Tenant(name=f'WH2 {tid}', name_ar=f'WH2 {tid}',
                        slug=f'wh2-test-{tid}', default_currency='AED', base_currency='AED')
        db_session.add(tenant)
        db_session.flush()
        ...
        warehouse = Warehouse(tenant_id=tenant.id, name=f'WH2 {tid}', code=f'WH2{tid[:4]}',
        ...
        user = User(username=f'whuser2-{tid}', email=f'whuser2-{tid}@t.com',
                    full_name='WH User', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        ...
        product = Product(name=f'CostItem {tid}', sku=f'CST-{tid}', tenant_id=tenant.id,
                          cost_price=Decimal('20'), regular_price=Decimal('80'),
                          current_stock=Decimal('0'), is_active=True)
        db_session.add(product)
        db_session.flush()

        stock = ProductWarehouseStock(tenant_id=tenant.id, product_id=product.id,
                                      warehouse_id=warehouse.id, quantity=Decimal('0'))
        db_session.add(stock)
