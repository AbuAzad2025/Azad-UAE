...
class TestLedgerManualEntry:
    def test_ledger_journal_entry_creates_balanced_gl(self, app, db_session, client):
        ...
        tenant = Tenant(
            name=f'Test {tid}', name_ar=f'Test {tid}',
            slug=f'test-ledger-{tid}', default_currency='AED', base_currency='AED',
        )
        ...
        if perm is None:
            perm = Permission(code='manage_ledger', name='Manage Ledger', name_ar='إدارة دفتر الأستاذ', category='finance')
            db_session.add(perm)
            db_session.flush()
        ...
        user = User(
            username=f'acctest-{tid}', email=f'acctest-{tid}@test.com',
            full_name='Accountant', tenant_id=tenant.id,
