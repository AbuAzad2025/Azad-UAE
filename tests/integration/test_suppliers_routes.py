...
class TestPayablesReport:
    def test_purchases_report_shows_supplier_balances(self, app, db_session, client):
        ...
        user = User(
            username=f"rep-{tid}",
            email=f"rep-{tid}@t.com",
            full_name="ReportUser",
            role_id=role.id,
            tenant_id=tenant.id,
            branch_id=branch.id,
            is_active=True,
...
class TestSupplierChequeReport:
    def test_statement_shows_cheque_payments(self, app, db_session, client):
        ...
        cheque = Cheque(
            ...
            branch_id=branch.id,
            supplier_id=supplier.id,
            cheque_number=f"CH-SUP-{tid}",
            cheque_bank_number=f"BNK-SUP-{tid}",
...
class TestSupplierSearch:
    def test_supplier_search_by_name(self, app, db_session, client):
        ...
        purchase = Purchase(
            ...
            branch_id=branch.id,
            supplier_id=supplier.id,
            supplier_name=f"FindMeSupp {tid}",
            user_id=user.id,
...
class TestSupplierCreditLimit:
    def test_supplier_credit_limit_tracks_purchases(self, app, db_session, client):
        ...
        supplier = Supplier(
            tenant_id=tenant.id,
            name=f"LimitSupp {tid}",
            phone="0501231234",
            credit_limit=Decimal("1000"),
            total_purchases_aed=Decimal("800"),
