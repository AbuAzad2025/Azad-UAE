...
class TestSalesCreate:
    def test_create_invoice_creates_gl_and_reduces_stock(self, app, db_session, client):
        from models import Tenant, Branch, User, Role, Customer, Product, Warehouse, ProductWarehouseStock
        from services.gl_service import GLService
        from models.gl import GLJournalEntry

        tid = str(uuid.uuid4())[:8]
        tenant = Tenant(name=f'ST {tid}', name_ar=f'ST {tid}',
                        slug=f'sales-test-{tid}', default_currency='AED', base_currency='AED')
        ...
        user = User(username=f'seller-{tid}', email=f'seller-{tid}@t.com',
        ...
        product = Product(name=f'Widget {tid}', sku=f'WID-{tid}', tenant_id=tenant.id,
    ...
    def test_create_invoice_with_tax_calculates_vat_correctly(self, app, db_session, client):
        ...
        tid = str(uuid.uuid4())[:8]
        tenant = Tenant(name=f'TAX {tid}', name_ar=f'TAX {tid}',
                        slug=f'tax-test-{tid}', default_currency='AED', base_currency='AED')
        db_session.add(tenant)
        db_session.flush()
        ...
        user = User(username=f'seller2-{tid}', email=f'seller2-{tid}@t.com',
                    full_name='Seller', role_id=role.id,
                    tenant_id=tenant.id, branch_id=branch.id,
                    is_active=True, is_owner=True)
        user.set_password('x')
        ...
        product = Product(name=f'TaxWidget {tid}', sku=f'TW-{tid}', tenant_id=tenant.id,
                          cost_price=Decimal('50'), regular_price=Decimal('100'),
