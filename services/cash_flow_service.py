...
class CashFlowService:
    ...
    def generate_cash_flow(period_start, period_end, branch_id=None, tenant_id=None):
        ...
        operating: dict[str, Any] = CashFlowService._get_operating_activities(
            period_start, period_end, branch_id=branch_id, tenant_id=tenant_id
        )

        # 2. الأنشطة الاستثمارية (Investing Activities)
        investing: dict[str, Any] = CashFlowService._get_investing_activities(
            period_start, period_end, branch_id=branch_id, tenant_id=tenant_id
        )

        # 3. الأنشطة التمويلية (Financing Activities)
        financing: dict[str, Any] = CashFlowService._get_financing_activities(
            period_start, period_end, branch_id=branch_id, tenant_id=tenant_id
        )
        ...
        if branch_id:
            cash_accounts_query = cash_accounts_query.filter(
                GLAccount.branch_id == branch_id
            )
        cash_accounts = cash_accounts_query.all()

        cash_beginning = CashFlowService._get_cash_balance(
            cash_accounts,
            period_start,
            is_beginning=True,
            branch_id=branch_id,
    ...
    @staticmethod
    def _get_operating_activities(
        period_start, period_end, branch_id=None, tenant_id=None
    ):
        ...
        # المدفوعات للموردين
        supplier_payments_query = db.session.query(func.sum(Payment.amount_aed)).filter(
            and_(
        ...
        if tenant_id:
            supplier_payments_query = supplier_payments_query.filter(
        ...
        # المدفوعات للمصروفات
        expense_payments_query = db.session.query(func.sum(Expense.amount_aed)).filter(
            and_(
        ...
        if tenant_id:
            expense_payments_query = expense_payments_query.filter(
        ...
        net_cash_from_operating = (
            receipts - supplier_payments - expense_payments - salaries
        )
    ...
    def _get_investing_activities(
        ...
        fixed_assets_query = GLAccount.query.filter(GLAccount.code.like("12%"))
        if tenant_id:
            fixed_assets_query = fixed_assets_query.filter(
    ...
    def _get_financing_activities(
        ...
        if tenant_id:
            owner_draw_account_query = owner_draw_account_query.filter_by(
                tenant_id=tenant_id
            )
        ...
        if owner_draw_account:
            ...
            if tenant_id:
                owner_withdrawals_query = owner_withdrawals_query.filter(
                    GLJournalEntry.tenant_id == tenant_id
                )
            if branch_id:
                owner_withdrawals_query = owner_withdrawals_query.filter(
        ...
        if loans_account:
            ...
            if tenant_id:
                loans_received_query = loans_received_query.filter(
                    GLJournalEntry.tenant_id == tenant_id
                )
            if branch_id:
                loans_received_query = loans_received_query.filter(
            ...
            if tenant_id:
                loan_repayments_query = loan_repayments_query.filter(
                    GLJournalEntry.tenant_id == tenant_id
                )
            if branch_id:
                loan_repayments_query = loan_repayments_query.filter(
    ...
    def _get_cash_balance(
        cash_accounts, target_date, is_beginning=False, branch_id=None, tenant_id=None
    ):
            ...
            debit_query = (
                ...
                .filter(
                    and_(
                        GLJournalLine.account_id == account.id,
                        (
                            func.date(GLJournalEntry.entry_date) < target_date
                            if is_beginning
