...
class ChequeAccountingIntegration:
    ...
    def issue_cheque(cheque_id, issued_by=None):
        ...
        try:
            ...
            entry = GLJournalEntry.query.filter_by(
                reference_type='cheque_issue', reference_id=cheque.id
    ...
    def clear_cheque(cheque_id, cleared_by=None, bank_charges=0, exchange_gain_loss=0):
        ...
        try:
            ...
            entry = GLJournalEntry.query.filter(
                GLJournalEntry.reference_type == 'cheque_clear',
    ...
    def bounce_cheque(cheque_id, bounced_by=None, bounce_reason=None):
        ...
        try:
            ...
            # إرجاع القيد المرتبط
            entry = GLJournalEntry.query.filter_by(
                reference_type='cheque_bounce', reference_id=cheque.id
            ).order_by(GLJournalEntry.id.desc()).first()
            return entry
        except Exception as e:
            db.session.rollback()
    ...
    def get_cheque_accounting_summary(cheque_id):
        ...
        ref_types = [('cheque_receive', 'receive'), ('cheque_issue', 'issue'), ('cheque_clear', 'clear'), ('cheque_bounce', 'bounce')]
        for ref_type, entry_type in ref_types:
            entries = GLJournalEntry.query.filter_by(
                reference_type=ref_type, reference_id=cheque.id
