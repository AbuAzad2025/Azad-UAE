...
class AIService:
    ...
    def get_user_info_for_owner(username=None):
        ...
        if username:
            term = username.strip()
            user = user_q.filter(or_(User.username == term, User.email == term)).first()
            if not user:
                user = (
                    session.query(User)
                    .filter(
    ...
    def recommend_price(product_id, customer_id):
        ...
        avg_sale_price = (
            db.session.query(func.avg(SaleLine.unit_price))
            .join(Sale)
            .filter(
    ...
    def get_exchange_rate_suggestion(currency, target_date=None):
        ...
        default_rates = {"USD": 3.67, "EUR": 4.02, "AED": 1.0}
...
7. الشيكات (تسجيل شيك وارد/صادر وعرض الشيكات)
8. مرتجعات المبيعات (إنشاء مرتجع من فاتورة وعرض المرتجعات)
9. عروض الأسعار (إنشاء/عرض/إرسال/قبول/رفض/تحويل إلى فاتورة)
10. تحديث بيانات العملاء والمنتجات، وتسويات المخزون بسبب موثق
11. مردودات المشتريات (إنشاء مرتجع من فاتورة شراء وعرض التفاصيل)
12. دورة الشيكات (إيداع في البنك/تأكيد تحصيل/معالجة ارتداد)
13. الرواتب (احتساب مسير شهري واعتماد وترحيل مع القيود وملف WPS)
...
لتسجيل شيك:
  {{"action": "create_cheque", "data": {{"cheque_number": "الرقم", "cheque_type": "incoming", "amount": 0, "bank_name": "البنك", "due_date": "YYYY-MM-DD"}}, "message": "جاري تسجيل الشيك..."}}

لإنشاء مرتجع مبيعات:
  {{"action": "create_sale_return", "data": {{"sale_number": "رقم الفاتورة", "product_name": "المنتج", "quantity": 1}}, "message": "جاري إنشاء المرتجع..."}}

لإنشاء عرض سعر:
  {{"action": "create_quotation", "data": {{"customer_name": "اسم العميل", "lines": [{{"product_name": "منتج1", "quantity": 1}}]}}, "message": "جاري إنشاء عرض السعر..."}}

لإنشاء مرتجع مشتريات:
  {{"action": "create_purchase_return", "data": {{"purchase_number": "رقم فاتورة الشراء", "product_name": "المنتج", "quantity": 1}}, "message": "جاري إنشاء مرتجع المشتريات..."}}
        ...
        try:
            ...
            try:
                ...
                LoggingCore.log_error(
                    message=str(e),
                    category="AI",
                    source="services.ai_service._execute_ai_action",
                    level="WARNING",
                    exception=e,
                )
            except Exception:
    ...
    def predict_customer_churn():
        ...
        try:
            ...
            for customer in active_customers:
                ...
                last_sale = last_sale_q.order_by(Sale.sale_date.desc()).first()
    ...
    def classify_customer_neural(customer_id):
        ...
        try:
            ...
            classification = neural.classify_customer_intelligence(
                customer_id, from_app_context=current_app.app_context
            )
        ...
        try:
            ...
            if code_type == "sql":
                ...
                code = generator.generate_python_function(
                    params.get("name", "my_function"), purpose, params.get("params", [])
                )
            else:
                code = "# Unsupported code type"

            return {"code": code, "type": code_type, "purpose": purpose}
