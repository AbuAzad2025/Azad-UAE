...
class AdvancedFinancialAnalytics:
    ...
    def get_financial_ratios(date_from=None, date_to=None, tenant_id=None):
        ...
        expenses = AdvancedFinancialAnalytics._calculate_balance_by_prefix(
            ["5", "6"], date_from, date_to, is_pl=True, tenant_id=tenant_id
        )
    ...
    def _calculate_balance_by_prefix(
        ...
        query = scope_gl_accounts(
            GLAccount.query.filter(GLAccount.is_active, GLAccount.is_header == False),
            tenant_id=tenant_id,
        )
    ...
    def get_expense_breakdown(tenant_id=None):
        ...
        return {"items": breakdown, "total": float(total_expenses)}
    ...
    def get_revenue_breakdown(tenant_id=None):
        ...
        return {"items": breakdown, "total": float(total_revenue)}
