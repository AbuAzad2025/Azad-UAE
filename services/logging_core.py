...
class LoggingCore:
    ...
    # ──────────────────────────────────────────────────────────────
    #  LOG FILE READING (ErrorLogService merge)
    # ──────────────────────────────────────────────────────────────
    ...
    def parse_error_log(
        ...
        """Parse and return entries from the errors.log file.
        ...
        if level_filter:
            ...
            parsed = [e for e in parsed if e["level"] == level_filter]

        total = len(parsed)
        start = (page - 1) * per_page
        paginated = parsed[start:start + per_page]
        total_pages = max(1, (total + per_page - 1) // per_page)

        stats = {}
        if parsed:
            stats["by_level"] = dict(Counter(e["level"] for e in parsed))
            stats["by_module"] = dict(Counter(e["module"] for e in parsed if e["module"]).most_common(10))
            stats["total"] = total
