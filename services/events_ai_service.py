...
def _learn_accounting_entries(mapper, connection, target):
    try:
        ...
        try:
            ...
            logger.info(
                f"AI Accounting: Learned entry {target.entry_number} - {'Balanced' if entry_data['is_balanced'] else 'Unbalanced'}"
            )
        except Exception:
...
def _predict_future_sales(mapper, connection, target):
    try:
        ...
        if len(recent_sales) > 10:
            ...
            logger.info(
                f"AI Prediction: Monthly sales forecast: {predicted_month:.2f} AED (based on {len(recent_sales)} sales)"
            )
    except Exception as e:
            ...
            if movements:
                ...
                if daily_rate > 0:
                    ...
                    if days_until_stockout < 7:
                        logger.warning(
                            f"AI Prediction: Product {target.name} will run out in {days_until_stockout:.0f} days! (Selling {daily_rate:.1f} units/day)"
                        )
                    else:
                        logger.info(
                            f"AI Prediction: Product {target.name} stock will last {days_until_stockout:.0f} days"
                        )
    except Exception as e:
...
def _predict_customer_churn(mapper, connection, target):
    try:
        ...
        last_sale = connection.execute(
            Sale.__table__.select()
            .where(Sale.customer_id == target.id, Sale.status == "confirmed")
        ...
        if last_sale:
            ...
            if days_since_purchase > 90:
                churn_risk = "high"
                logger.warning(
                    f"AI Churn Risk: Customer {target.id} - {days_since_purchase} days inactive - HIGH RISK!"
                )
                ...
                (
                    float(
                        (
                            ...
                            - sum(
                                (line.cost_price or Decimal("0")) * (line.quantity or 0)
