...
with app.app_context():
    ...
    from models import Tenant, Branch, Warehouse, User, Role
    ...
    if not wh:
        with atomic_transaction("create_demo_warehouse"):
            wh = Warehouse(tenant_id=demo.id, branch_id=branch.id, name='المستودع الرئيسي', code='WH-DEMO-01', is_active=True)
            db.session.add(wh)
            db.session.flush()
    ...
    for username, full_ar, role_name, pwd in demo_users:
        ...
        with atomic_transaction(f"create_demo_user_{username}"):
            u = User(username=username, email=f"{username}@demo.test", full_name=full_ar, full_name_ar=full_ar,
                     tenant_id=demo.id, branch_id=branch.id, role=role, is_active=True, email_verified=True)
            u.set_password(pwd)
            db.session.add(u)
            db.session.flush()
    ...
    if not ts:
        with atomic_transaction("create_demo_store"):
            ts = TenantStore(tenant_id=demo.id, warehouse_id=wh.id, store_slug='demo', title='متجر تجريبي',
                             is_enabled=True, display_currency='ILS', phone='+970599000000')
            db.session.add(ts)
            db.session.flush()
