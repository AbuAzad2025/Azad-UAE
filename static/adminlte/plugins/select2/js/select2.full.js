	...
	var S2 = (() => {
			...
			($, Utils, KEYS) => {
				...
				BaseSelection.prototype.bind = function (container, $container) {
					...
					container.on("enable", () => {
						...
						this.$selection.attr("aria-disabled", "false");
					});
		...
		S2.define(
			...
			($, Utils, KEYS) => {
				...
				Search.prototype.bind = function (decorated, container, $container) {
					...
					container.on("open", () => {
						this.$search.attr("aria-controls", resultsId);
					...
					container.on("results:focus", (params) => {
						if (params.data._resultId) {
							this.$search.attr("aria-activedescendant", params.data._resultId);
						} else {
							this.$search.removeAttr("aria-activedescendant");
						}
					});
					...
					this.$selection.on("keydown", ".select2-search--inline", (evt) => {
						...
						if (key === KEYS.BACKSPACE && this.$search.val() === "") {
							var $previousChoice = this.$searchContainer.prev(
			...
			MaximumSelectionLength.prototype._checkIfMaximumSelected = function (
				...
				this.current((currentData) => {
					...
					if (
						...
						count >= this.maximumSelectionLength
					) {
						this.trigger("results:message", {
			...
			($, Utils) => {
				...
				AttachBody.prototype.bind = function (
					...
					container.on("open", () => {
						...
						this._bindContainerResultHandlers(container);
					});
		...
		S2.define(
			...
			(
				...
				Defaults.prototype._resolveLanguage = (language) => {
					if (!language) {
						return [];
					}
				...
				Select2.prototype._isChangeMutation = function (evt, mutations) {
					...
					if (!mutations) {
						...
						$.each(mutations, (evt, mutation) => {
							if (this._isChangeMutation(evt, mutation)) {
		...
		S2.define(
			...
			($, Utils) => {
				...
				InputData.prototype.addOptions = function (_, $options) {
					var options = $.map($options, ($option) =>
						Utils.GetData($option[0], "data"),
					);
