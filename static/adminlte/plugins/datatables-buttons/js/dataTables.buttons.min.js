...
((d) => {
	"function" === typeof define && define.amd
		? define(["jquery", "datatables.net"], (z) => d(z, window, document))
	...
	var u = d.fn.dataTable,
		...
		C = u.ext.buttons,
		x = function (a, b) {
			if (!(this instanceof x)) return (c) => new x(c, a).container();
	...
	d.extend(x.prototype, {
		...
		text: function (a, b) {
			...
			var e = this.s.dt,
				h = d(c.node),
				f = (g) => ("function" === typeof g ? g(e, h, c.conf) : g);
		...
		_buildButton: function (a, b, c, e) {
			var h = this.c.dom.button,
				...
				r = this.s.dt,
				q = (w) => ("function" === typeof w ? w(r, k, a) : w);
			if (a.spacer) {
				var n = d("<span></span>")
			...
			if (a.available && !a.available(r, a) && !Object.hasOwn(a, "html"))
				return !1;
			if (Object.hasOwn(a, "html")) var k = d(a.html);
			else {
				var t = (w, D, F, G) => {
				...
				h = (m) => {
					for (var r = 0; !d.isPlainObject(m) && !Array.isArray(m); ) {
						...
						if ("function" === typeof m) {
							if (((m = m.call(this, c, a)), !m)) return !1;
		...
		_popover: function (a, b, c, e) {
			...
			if (!1 === a) l();
				...
				f.autoClose &&
					setTimeout(() => {
						b.on("buttons-action.b-internal", (E, A, w, D) => {
							D[0] !== g[0] && l();
						});
					}, 0);
	...
	x.instanceSelector = (a, b) => {
		if (a === p || null === a) return d.map(b, (f) => f.inst);
		var c = [],
			e = d.map(b, (f) => f.name),
			h = (f) => {
	...
	d.extend(C, {
		collection: {
			text: (a) => a.i18n("buttons.collection", "Collection"),
		...
		split: {
			text: (a) => a.i18n("buttons.split", "Split"),
			className: "buttons-split",
			closeButton: !1,
			init: (a, b, c) => b.attr("aria-expanded", !1),
			action: function (a, b, c, e) {
				this.popover(e._collection, e);
			},
			attr: { "aria-haspopup": !0 },
	...
	u.Api.registerPlural("buttons().active()", "button().active()", function (a) {
		return a === p
			? this.map((b) => b.inst.active(b.node))
			: this.each((b) => {
	...
	u.Api.registerPlural("buttons().action()", "button().action()", function (a) {
		return a === p
			? this.map((b) => b.inst.action(b.node))
			: this.each((b) => {
	...
	u.Api.registerPlural(
		...
		function (a) {
			return a === p
				? this.map((b) => b.inst.processing(b.node))
				: this.each((b) => {
	...
	var K = (a) =>
			null === a || a === p ? null : "function" === typeof a ? a() : a,
		N = (a, b, c) => {
			b = K(b);
		...
		Q = (a, b) => {
			var c = d.extend(
				...
				{},
				{
					...
					trim: !0,
					format: {
						header: (t) => x.stripData(t, c),
						footer: (t) => x.stripData(t, c),
						body: (t) => x.stripData(t, c),
					},
					customizeData: null,
				},
				b,
	...
	d.fn.DataTable.Buttons = x;
	d(B).on("init.dt plugin-init.dt", (a, b) => {
		"dt" === a.namespace &&
