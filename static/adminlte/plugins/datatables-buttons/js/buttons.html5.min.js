...
((n) => {
	"function" === typeof define && define.amd
		? define(["jquery", "datatables.net", "datatables.net-buttons"], (u) =>
				n(u, window, document),
			)
		: "object" === typeof exports
			? (module.exports = (u, x, E, F) => {
					u || (u = window);
	...
	var K = ((a) => {
		if (
			...
			var c = a.document.createElementNS("http://www.w3.org/1999/xhtml", "a"),
				...
				p = function (h, q, v) {
					v || (h = g(h));
					var w = "application/octet-stream" === h.type,
						C = () => {
			...
			t.error =
				...
				t.onwriteend =
					null;
			return (h, q, v) => new p(h, q || h.name || "download", v);
		}
	})(
	...
	var P = {
		...
		U = [
			{ match: /^-?\d+\.\d%$/, style: 60, fmt: (a) => a / 100 },
			{ match: /^-?\d+\.?\d*%$/, style: 56, fmt: (a) => a / 100 },
			{ match: /^-?\$[\d,]+.?\d*$/, style: 57 },
			{ match: /^-?£[\d,]+.?\d*$/, style: 58 },
			{ match: /^-?€[\d,]+.?\d*$/, style: 59 },
			{ match: /^-?\d+$/, style: 65 },
			...
			{ match: /^-?[\d,]+\.\d{2}$/, style: 64 },
			{
				match: /^[\d]{4}-[\d]{2}-[\d]{2}$/,
				style: 67,
				fmt: (a) => Math.round(25569 + Date.parse(a) / 864e5),
			},
		];
	D.ext.buttons.copyHtml5 = {
		className: "buttons-copy buttons-html5",
		text: (a) => a.i18n("buttons.copy", "Copy"),
		action: function (a, c, d, b) {
			...
			n(x)
				.on("keydown.buttons-copy", (q) => {
					27 === q.keyCode && (h(), this.processing(!1));
				})
	...
	D.ext.buttons.csvHtml5 = {
		...
		available: () => u.FileReader !== B && u.Blob,
		text: (a) => a.i18n("buttons.csv", "CSV"),
		action: function (a, c, d, b) {
			this.processing(!0);
			a = S(c, b).str;
	...
	D.ext.buttons.excelHtml5 = {
		...
		available: () => u.FileReader !== B && (E || u.JSZip) !== B && !T() && M,
		text: (a) => a.i18n("buttons.excel", "Excel"),
		action: function (a, c, d, b) {
			this.processing(!0);
			var e = 0;
			a = (k) => n.parseXML(P[k]);
	...
	D.ext.buttons.pdfHtml5 = {
		...
		available: () => u.FileReader !== B && (F || u.pdfMake),
		text: (a) => a.i18n("buttons.pdf", "PDF"),
		action: function (a, c, d, b) {
			this.processing(!0);
