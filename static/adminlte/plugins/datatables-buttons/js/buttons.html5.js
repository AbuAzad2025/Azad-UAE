...
((factory) => {
	if (typeof define === "function" && define.amd) {
		...
		// CommonJS
		module.exports = (root, $, jszip, pdfmake) => {
			if (!root) {
				root = window;
			}
	...
	var _excelSpecials = [
		{ match: /^-?\d+\.\d%$/, style: 60, fmt: (d) => d / 100 }, // Percent with d.p.
		...
		{ match: /^-?[\d,]+\.\d{2}$/, style: 64 },
		{
			match: /^[\d]{4}-[\d]{2}-[\d]{2}$/,
			style: 67,
			fmt: (d) => Math.round(25569 + Date.parse(d) / (86400 * 1000)),
		}, //Date yyyy-mm-dd
	];
	...
	DataTable.ext.buttons.csvHtml5 = {
		...
		available: () => window.FileReader !== undefined && window.Blob,
	...
	DataTable.ext.buttons.excelHtml5 = {
		...
		action: function (e, dt, button, config) {
			...
			if (zip.generateAsync) {
				// JSZip 3+
				zip.generateAsync(zipConfig).then((blob) => {
	...
	DataTable.ext.buttons.pdfHtml5 = {
		...
		available: () => window.FileReader !== undefined && _pdfMake(),
