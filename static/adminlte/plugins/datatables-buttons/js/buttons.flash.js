...
((factory) => {
	...
	ZeroClipboard_TableTools.Client.prototype = {
		...
		destroy: function () {
			if (this.domElement && this.div) {
				...
				$.each(ZeroClipboard_TableTools.clients, (id, client) => {
					if (client === this) {
	...
	ZeroClipboard_TableTools.hasFlash = () => {
		try {
	 ...
	 */
	var _filename = (config, incExtension) => {
	...
	var flashButton = {
		available: () => ZeroClipboard_TableTools.hasFlash(),
	...
	function _excelColWidth(data, col) {
		...
		for (var i = 0, ien = data.body.length; i < ien; i++) {
			...
			if (str.indexOf("\n") !== -1) {
				lineSplit = str.split("\n");
				lineSplit.sort((a, b) => b.length - a.length);
	...
	if (typeof window.XMLSerializer === "undefined") {
		_serialiser = new (function () {
			this.serializeToString = (input) => input.xml;
		})();
	} else {
	...
	var _excelSpecials = [
		{ match: /^-?\d+\.\d%$/, style: 60, fmt: (d) => d / 100 }, // Precent with d.p.
		{ match: /^-?\d+\.?\d*%$/, style: 56, fmt: (d) => d / 100 }, // Percent
	...
	DataTable.Api.register("buttons.resize()", () => {
		$.each(ZeroClipboard_TableTools.clients, (i, client) => {
	...
	DataTable.ext.buttons.copyFlash = $.extend({}, flashButton, {
		...
		text: (dt) => dt.i18n("buttons.copy", "Copy"),
	...
	DataTable.ext.buttons.csvFlash = $.extend({}, flashButton, {
		...
		text: (dt) => dt.i18n("buttons.csv", "CSV"),
	...
	DataTable.ext.buttons.excelFlash = $.extend({}, flashButton, {
		...
		text: (dt) => dt.i18n("buttons.excel", "Excel"),
	...
	DataTable.ext.buttons.pdfFlash = $.extend({}, flashButton, {
		...
		text: (dt) => dt.i18n("buttons.pdf", "PDF"),
		...
		action: function (e, dt, button, config) {
			...
			var ratios = dt
				...
				.indexes()
				.map((idx) => dt.column(idx).header().offsetWidth / totalWidth);
