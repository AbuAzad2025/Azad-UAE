(() => {
  ...
  function normalizeMode(v) { return v === "dark" ? "dark" : "light"; }
  function normalizeVariant(v) { return v === "gulf" ? "gulf" : "palestinian"; }
  function normalizeSidebarSide(v) { return v === "left" ? "left" : v === "right" ? "right" : null; }
  function getDefaultSidebarSide() { return document.documentElement.getAttribute("dir") === "rtl" ? "right" : "left"; }

  function getInitialMode() {
    const stored = localStorage.getItem(STORAGE_MODE);
    if (stored) return normalizeMode(stored);
    try { return window.matchMedia?.("(prefers-color-scheme: dark)").matches ? "dark" : "light"; } catch (_e) { return "light"; }
  }
  ...
  function updateModeToggle(mode) {
    ...
    toggle.setAttribute("aria-label", mode === "dark" ? "التبديل إلى الوضع الفاتح" : "التبديل إلى الوضع الداكن");
    toggle.setAttribute("title", mode === "dark" ? "التبديل إلى الوضع الفاتح" : "التبديل إلى الوضع الداكن");
