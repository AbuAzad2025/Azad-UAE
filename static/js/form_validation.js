    ...
    if (input.required && !val) {
      ...
      switch (type) {
        ...
        case "url":
          ...
          break;
        case "number":
          if (isNaN(Number(val))) err = MSG.number;
          break;
        case "date":
          if (isNaN(Date.parse(val))) err = MSG.date;
          break;
      }

      // Name-based validation
      if (!err && name && (name.includes("phone") || name.includes("mobile") || name.includes("tel"))) {
      ...
      if (!err && type === "file" && input.files.length > 0) {
        ...
        const acceptTypes = (input.accept || "").split(",").map(t => t.trim()).filter(Boolean);
