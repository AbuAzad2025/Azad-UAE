(function ($) {
  ...
  function performSearch(query, target) {
    ...
    $.get('/api/search', { type: target, q: query })
      .done(data => {
        if (data.html) { $container.html(data.html); } else if (data.results && data.results.length) { $container.html('<ul class="list-group">' + data.results.map(r => '<li class="list-group-item d-flex justify-content-between align-items-center">' + (r.text || r.name || '') + (r.phone ? '<span class="badge badge-secondary">' + r.phone + '</span>' : '') + '</li>').join('') + '</ul>'); } else { $container.html('لا توجد نتائج'); }
      })
      .fail(() => {
