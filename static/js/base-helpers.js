  ...
  window.azad = {
    ...
    hideLoading() {
      ...
      if (loadingCount === 0) {
        ...
        el.style.opacity = "0";
        setTimeout(() => { if (loadingCount === 0) el.style.display = "none"; }, 300);
      }
    },
    formatNumber(value) {
    ...
    showError(msg) { azadToast(msg, "error"); },
    showSuccess(msg) { azadToast(msg, "success"); },
    showWarning(msg) { azadToast(msg, "warning"); },
    showInfo(msg) { azadToast(msg, "info"); },
    // Debounce utility
    debounce(fn, wait) {
      let timeout;
...
document.querySelectorAll('a[href^="/"]:not([href*="/logout"]):not([href^="//"])').forEach((link) => {
...
async function loadFxRates() {
  ...
  if (tbody) {
    tbody.innerHTML = '<tr><td colspan="3" class="text-center py-4"><div class="spinner-border spinner-border-sm text-muted" role="status"></div><span class="mr-2"> Ø¬Ø§Ø±Ù Ø§Ù„ØªØ­Ù…ÙŠÙ„...</span></td></tr>';
  }

  try {
    const baseCurrency = window._FX_BASE_CURRENCY || "{{ tenant_base_currency|default(\x27USD\x27) }}";
    const res = await fetch(`${window._FX_API_URL}?base=${encodeURIComponent(baseCurrency)}`, { cache: "no-store" });
...
function getFallbackFx() {
  return {
    ...
    rates: {
      USD: 1.0, ILS: 3.65, JOD: 0.709, AED: 3.67,
      EUR: 0.92, SAR: 3.75, EGP: 50.5, GBP: 0.79,
    },
...
function safeEval(expr) {
  ...
  function error() { throw new Error("invalid expression"); }
  function skipWs() { while (pos < len && isWhite(src[pos])) pos++; }
  ...
  function parseTerm() {
    ...
    for (;;) {
      ...
      else if (c === "/") { pos++; const d = parseFactor(); if (d === 0) error(); value /= d; }
      else return value;
    }
  }
  ...
  function parsePrimary() {
    skipWs();
    const c = src[pos];
    if (c === "(") { pos++; const v = parseExpression(); if (peek() !== ")") error(); pos++; return v; }
    if (isDigit(c) || c === ".") return parseNumber();
    if (isIdent(c)) {
      const name = parseIdent();
      if (name === "pi") return Math.PI;
      if (name === "e") return Math.E;
      const funcs = { sin: Math.sin, cos: Math.cos, tan: Math.tan, sqrt: Math.sqrt, log: Math.log10, ln: Math.log };
      if (funcs[name]) {
        if (peek() !== "(") error();
        pos++;
...
function wirePad(container, display, buttons) {
  ...
  container.innerHTML = buttons
    .map((b) => `<button type="button" class="btn btn-outline-secondary az-calc-btn" data-calc="${b}">${b}</button>`)
    .join("");
  ...
  if (btnLoanCalc) {
    btnLoanCalc.addEventListener("click", () => {
      ...
      if (!(p > 0) || !(months > 0)) {
        ...
        out.innerHTML = '<i class="fas fa-exclamation-triangle mr-1"></i>Ø£Ø¯Ø®Ù„ Ù‚ÙŠÙ… ØµØ­ÙŠØ­Ø©.';
        return;
      }
...
(() => {
  ...
  function toAbsoluteUrl(value) {
    try { return new URL(value, window.location.href).toString(); } catch (_) { return String(value || ""); }
  }
  ...
  function isApiRequest(url) {
    try {
      ...
      return parsed.pathname.indexOf("/api/") === 0 || parsed.pathname.indexOf("/api_enhanced/") === 0;
  ...
  window.addEventListener("error", (ev) => {
    ...
    sendError({
      ...
      lineno: ev.lineno || 0, colno: ev.colno || 0,
  ...
  window.addEventListener("unhandledrejection", (ev) => {
    ...
    else if (reason?.message) { msg = reason.message; stack = reason.stack || null; }
    sendError({ type: "promise", message: msg, source: "unhandledrejection", lineno: 0, colno: 0, stack, url: window.location.href });
  });
    ...
    window.fetch = function (input, init) {
      ...
      if (activeRequests >= CONCURRENCY_WARN_AT && !shouldSkipRequest(absoluteUrl)) {
        ...
        if (now - concurrencyNoticeAt > DUPLICATE_WINDOW_MS) {
          ...
          sendError({ type: "concurrency", message: `High concurrent browser requests: ${activeRequests}`, source: "fetch.concurrency", request_url: absoluteUrl, method, url: window.location.href });
        }
      }
      ...
      return originalFetch.call(this, input, init).then((response) => {
        ...
        if (response && !response.ok && !shouldSkipRequest(requestUrl)) {
          sendError({ type: requestIsApi ? "api" : "fetch", message: (requestIsApi ? "API failed: HTTP " : "Fetch failed: HTTP ") + response.status, source: requestIsApi ? "fetch.api" : "fetch", request_url: absoluteUrl, method, status: response.status, duration_ms: duration, active_requests: activeRequests, request_id: response.headers?.get ? response.headers.get("X-Request-Id") : "", url: window.location.href });
        } else if (response && duration >= SLOW_REQUEST_MS && !shouldSkipRequest(requestUrl)) {
          sendError({ type: requestIsApi ? "api_slow" : "fetch_slow", message: `${requestIsApi ? "Slow API request" : "Slow fetch request"}: ${duration}ms`, source: requestIsApi ? "fetch.api.slow" : "fetch.slow", request_url: absoluteUrl, method, status: response.status, duration_ms: duration, active_requests: activeRequests, request_id: response.headers?.get ? response.headers.get("X-Request-Id") : "", url: window.location.href });
        }
        ...
        if (!shouldSkipRequest(requestUrl)) {
          const duration = Math.round((window.performance && performance.now ? performance.now() : Date.now()) - startedAt);
          sendError({ type: requestIsApi ? "api" : "fetch", message: err?.message || (requestIsApi ? "API network error" : "Fetch network error"), source: requestIsApi ? "fetch.api" : "fetch", request_url: absoluteUrl, method, duration_ms: duration, active_requests: activeRequests, stack: err?.stack ? err.stack : null, url: window.location.href });
        }
        activeRequests = Math.max(0, activeRequests - 1);
        throw err;
      });
  ...
  if (window.jQuery) {
    window.jQuery(document).ajaxError((_event, xhr, settings, thrownError) => {
      ...
      const absoluteUrl = toAbsoluteUrl(requestUrl);
      const requestIsApi = isApiRequest(absoluteUrl);
      sendError({ type: requestIsApi ? "api" : "ajax", message: thrownError || (requestIsApi ? "API AJAX failed: HTTP " : "AJAX failed: HTTP ") + xhr?.status, source: requestIsApi ? "jquery.ajax.api" : "jquery.ajax", request_url: absoluteUrl, method: settings?.type, status: xhr?.status, response_size: xhr?.responseText ? xhr.responseText.length : 0, url: window.location.href });
    });
  }
      ...
      new PerformanceObserver((list) => {
        list.getEntries().forEach((entry) => {
          if (entry.duration >= LONG_TASK_MS) {
            sendError({ type: "longtask", message: `Main thread long task: ${Math.round(entry.duration)}ms`, source: "performance.longtask", duration_ms: Math.round(entry.duration), url: window.location.href });
          }
        });
      ...
      new PerformanceObserver((list) => {
        list.getEntries().forEach((entry) => {
          ...
          if (!layoutNoticeSent && cumulativeLayoutShift >= LAYOUT_SHIFT_WARN_AT) {
            ...
            sendError({ type: "layout", message: `High cumulative layout shift: ${cumulativeLayoutShift.toFixed(3)}`, source: "performance.layout_shift", cls: Number(cumulativeLayoutShift.toFixed(3)), url: window.location.href });
          }
        });
  ...
  function auditThemeState(reason) {
    try {
      ...
      if (VALID_THEME_MODES.indexOf(mode) === -1 || VALID_THEME_VARIANTS.indexOf(variant) === -1) {
        sendError({ type: "theme", message: "Invalid UI theme state", source: "ui.theme", reason: reason || "audit", ui_mode: mode, ui_variant: variant, url: window.location.href });
      }
    } catch (_) {}
  }
  ...
  try {
    new MutationObserver((mutations) => {
      for (let i = 0; i < mutations.length; i += 1) {
        if (mutations[i].attributeName === "data-ui-mode" || mutations[i].attributeName === "data-ui-variant") {
          auditThemeState("mutation");
          break;
        }
      }
    }).observe(document.documentElement, { attributes: true, attributeFilter: ["data-ui-mode", "data-ui-variant"] });
  } catch (_) {}
})();
