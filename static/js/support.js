...
(() => {
	...
	function buildSupportMessage(kind, amountOverride, recordId) {
		...
		const messages = {
			...
			donation_help:
				...
				t("Expected") +
				": " +
				(amount || t("I will specify with you")) +
				" " +
				t("Dollar") +
				"." +
				orderRef,
	...
	function buildSupportStatusHtml(recordId, paymentAmount, label) {
		return (
			...
			'<div><i class="fas fa-check-circle sp-status-icon-success"></i> ' +
			t("On success you will receive clear details or direct payment address") +
			".</div>" +
			'<div><i class="fas fa-hourglass-half sp-status-icon-warning"></i> ' +
			t("On manual review you can follow up immediately with Azad") +
			".</div>" +
			'<div><i class="fas fa-undo-alt sp-status-icon-info"></i> ' +
			t("For any refund or reconciliation use WhatsApp or official email") +
			".</div>" +
			"</div>"
		);
	}
	...
	function showSupportAssistanceModal(title, introHtml, kind, amountOverride, recordId) {
		Swal.fire({
			...
			html:
				...
				'<p class="sp-status-box-label"><strong>' +
				t("Official contact with Azad") +
				":</strong></p>" +
				'<p class="sp-status-box-label"><i class="fab fa-whatsapp sp-status-icon-success"></i> ' +
				cfg.whatsappDisplay +
				"</p>" +
				'<p class="sp-status-box-value"><i class="fas fa-envelope sp-text-primary"></i> ' +
				cfg.email +
				"</p>" +
				"</div>" +
				"</div>",
	...
	function selectMethod(method, event) {
		document.querySelectorAll(".sp-donation-form").forEach((f) => {
	...
	async function collectSupportContactData(title) {
		const result = await Swal.fire({
			...
			html:
				...
				t("Full Name") +
				'">' +
				'<input id="swal-email" class="swal2-input" placeholder="' +
				t("Email address") +
				'" type="email">' +
				'<input id="swal-phone" class="swal2-input" placeholder="' +
				t("Mobile Number") +
				'" type="tel">' +
				'<textarea id="swal-message" class="swal2-textarea" placeholder="' +
				(currentTab === "purchase"
					? t("Company name or additional note (optional)")
					: t("Short message (optional)")) +
				'"></textarea>',
			focusConfirm: false,
			showCancelButton: true,
		...
		if (currentTab === "purchase" && (!result.value.name || !result.value.email)) {
			Swal.fire({
				icon: "error",
				title: t("Error"),
				text: t("Name and email required to purchase plan"),
	...
	async function generateCryptoPayment() {
		...
		Swal.fire({
			title: t("Creating payment address..."),
		...
		if (currentTab === "purchase") {
			if (!selectedPackage) {
				...
				Swal.fire({
					icon: "warning",
					title: t("Alert"),
					text: t("Please select a package first"),
					confirmButtonColor: "#667eea",
				});
				return;
			}
			...
			if (!packageId) {
				...
				Swal.fire({
					icon: "error",
					title: t("Error"),
					text: t("Selected plan not recognized"),
					confirmButtonColor: "#667eea",
				});
				return;
			}
					...
					if (data.payment_address) {
						Swal.fire({
							...
							html:
								...
								payBtn +
								"</div>" +
								'<p class="sp-result-hint"><i class="fas fa-info-circle"></i> ' +
								t(
								...
								buildSupportStatusHtml(
									recordId,
									paymentAmount,
									t("Waiting for your transfer to payment address"),
								) +
								"</div>",
						...
						Swal.fire({
							...
							html:
								...
								buildSupportStatusHtml(
									recordId,
									paymentAmount,
									t("Order registered pending follow-up or confirmation"),
								) +
								"</div>",
	...
	function copyAddress() {
		...
		navigator.clipboard.writeText(addr).then(() => {
			Swal.fire({
				icon: "success",
				title: t("Copied!"),
				text: t("Address copied to clipboard"),
				timer: 2000,
				showConfirmButton: false,
			});
		});
	...
	async function handlePayPalPayment() {
		...
		const r = await Swal.fire({
			...
			html:
				...
				'" type="tel">' +
				(currentTab === "purchase"
					? '<input id="swal-company" class="swal2-input" placeholder="' +
						t("Company Name") +
						" (" +
						t("Optional") +
						')">'
			...
			preConfirm: () => ({
				...
				company:
					currentTab === "purchase"
						? document.getElementById("swal-company")
							? document.getElementById("swal-company").value
							: ""
						: "",
			}),
		});
		...
		if (currentTab === "purchase") {
			...
			if (!packageId) {
				Swal.fire({
					icon: "error",
					title: t("Error"),
					text: t("Please select a package"),
				});
				return;
			}
			requestData = {
				...
				if (data.payment_address) {
					Swal.fire({
						...
						html:
							"" +
							'<div class="sp-result-center">' +
							'<p class="sp-result-title"><i class="fab fa-paypal sp-result-paypal-icon"></i> <strong>' +
							...
							paymentAmount +
							"</p>" +
							'<p class="sp-result-contact"><i class="fas fa-exchange-alt"></i> ' +
							...
							"</button>" +
							"</div>" +
							'<p class="sp-result-confirmed">\u2705 ' +
							t("Your order has been saved successfully") +
							"</p>" +
							buildSupportStatusHtml(
								data.purchase_id || data.donation_id,
								paymentAmount,
								t("Pending payment completion or contact"),
							) +
							"</div>",
	...
	document.addEventListener("DOMContentLoaded", () => {
		...
		if (urlParams.get("tab") === "donation") {
			document.querySelectorAll(".sp-tab-btn").forEach((b, i) => {
			...
			cardForm.addEventListener("submit", async (e) => {
				...
				try {
						...
						if (data.payment_address) {
							Swal.fire({
								...
								html:
									...
									"</button>" +
									"</div>" +
									'<p class="sp-result-highlight"><i class="fas fa-check-circle"></i> ' +
									t("Send the specified amount to the address above") +
									"</p>" +
									buildSupportStatusHtml(
										data.purchase_id || data.donation_id,
										paymentAmount,
										t("Pending payment completion or follow-up with Azad"),
									) +
									"</div>",
								confirmButtonText: t("Follow up Payment"),
								confirmButtonColor: "#28a745",
					...
					showSupportAssistanceModal(
						...
						"<p>" +
							t("Could not reach server while processing the order") +
							".</p><p>" +
							t("Don't worry, you can follow up directly with Azad Company") +
							".</p>",
						"payment_help",
						paymentAmount,
					);
