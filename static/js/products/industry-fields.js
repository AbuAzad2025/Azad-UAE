(() => {
	...
	function loadFields(industryCode) {
		...
		if (!container) return;
		container.innerHTML =
			'<div class="text-muted small"><i class="fas fa-spinner fa-spin"></i> جاري التحميل...</div>';
		fetch(`${API_URL}?industry=${encodeURIComponent(industryCode)}`, {
			...
			.then((data) => {
				...
				let html =
					'<hr><h5 class="text-info"><i class="fas fa-cogs"></i> حقول إضافية (' +
					esc(data.industry) +
					')</h5><div class="row">';
				data.fields.forEach((f) => {
					...
					html +=
						'<label><i class="fas fa-sliders-h text-muted"></i> ' +
						esc(label) +
						reqStar +
						"</label>";
