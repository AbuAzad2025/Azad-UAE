...
(() => {
	...
	async function fetchProductInfo(pid, wid) {
		if (!(pid && wid)) return {};
		try {
			const res = await fetch(`/api/products/${pid}/info?warehouse_id=${wid}`, {
	...
	(function initForm() {
		...
		function currentMaxIndex() {
			...
			qsa(".sale-line", wrap).forEach((row) => {
				if (row.dataset.index && !Number.isNaN(+row.dataset.index)) {
					max = Math.max(max, +row.dataset.index);
					return;
				}
		...
		function removeLine(row) {
			const rows = qsa(".sale-line", wrap);
			if (rows.length <= 1) {
				alert("يجب ترك بند واحد على الأقل.");
				return;
			}
			row.remove();
		...
		function bindAll() {
			qsa(".sale-line", wrap).forEach(bindRow);
		}
		...
		function recalc() {
			...
			qsa(".sale-line", wrap).forEach((row) => {
				...
				let d = toNum(qs('[name$="-discount_rate"]', row)?.value);
				d = Math.max(0, Math.min(100, d));
				const lineTotal = q * p;
				sub += lineTotal * (1 - d / 100);
				totalDiscount += lineTotal * (d / 100);
			});
			let globalTax = toNum(qs("#taxRate")?.value);
			globalTax = Math.max(0, Math.min(100, globalTax));
			...
			const td = qs("#totalDiscount");
			if (td) {
				td.textContent = totalDiscount.toFixed(2);
			}
		}
