(() => {
	...
	function refreshCartDrawer() {
		...
		body.innerHTML =
			'<div class="ps-cart-drawer-loading"><i class="fas fa-spinner fa-spin"></i></div>';
		...
		apiGet("/cart").then(() => {
			...
			return fetch(`/s/${SLUG}/cart`, {
				...
				.catch(() => {
					body.innerHTML =
						'<div class="ps-cart-drawer-empty"><i class="fas fa-shopping-basket"></i><p>Unable to load cart</p></div>';
				});
		});
	}
	...
	function renderCartDrawerItems(items, _doc) {
		...
		if (!items || items.length === 0) {
			body.innerHTML =
				'<div class="ps-cart-drawer-empty"><i class="fas fa-shopping-basket"></i><p>' +
				(window._EMPTY_CART_TEXT || "Cart is empty") +
				"</p></div>";
			if (footer) footer.style.display = "none";
			return;
		}
		...
		items.forEach((item) => {
			...
			html +=
				...
				_esc(item.pid) +
				'">' +
				'<button class="ps-cart-item-remove" data-cart-remove="' +
				_esc(item.pid) +
				'"><i class="fas fa-times"></i></button>' +
				'<div class="ps-cart-item-info">' +
				'<div class="ps-cart-item-name">' +
				_esc(item.name) +
				"</div>" +
	...
	addToCart = (productId, quantity) =>
		_origAddToCart(productId, quantity).then((data) => {
	...
	document.addEventListener("click", (e) => {
		...
		if (e.target.closest(".ps-cart-link")) {
			e.preventDefault();
			openCartDrawer();
		}
