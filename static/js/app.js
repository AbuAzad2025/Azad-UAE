(($) => {
	...
	function initAjaxSelects(root) {
		...
		// تجنب التداخل مع customer-select.js و supplier-select
		$(root)
			.find("select.ajax-select:not(.customer-select):not(.supplier-select)")
			.each(function () {
				const $el = $(this);
				if ($el.data("s2-initialized")) return;
				$el.data("s2-initialized", 1);
	...
	function initConfirmForms(root) {
		$(root)
			.find("form[data-confirm]")
			.each(function () {
				const $form = $(this);
				if ($form.data("confirm-bound")) return;
				$form.data("confirm-bound", 1).on("submit", (e) => {
	...
	function initBtnLoading(root) {
		$(root)
			...
			.each(function () {
				...
				$btn.data("loading-bound", 1).on("click", () => {
	...
	function saveFormData($form) {
		const formData = $form
			...
			.not(
				'[type="password"], [type="hidden"], [name*="csrf"], [name*="token"], [name*="secret"], [name*="api_key"], [name*="password"]',
			)
			.serialize();
    ...
    `);
		...
		if ($container.length === 0) {
			$container = $(
				'<div id="system-alert-container" style="position: fixed; top: 20px; left: 20px; z-index: 9999; max-width: 400px;"></div>',
			);
			$("body").append($container);
		}
